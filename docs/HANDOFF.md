...
- 版本 **v0.51.0**｜分支 `opus` → `origin/opus`（收尾 `git push`）｜开发机 macOS（本仓库路径）
- 上一轮：**R51 移动行 / 复制行编辑命令（Line motion）✓ 已交付**（R32+ 候选池第四梯队 **#⑳**）。新 `features/editor/editorMotionCommands.ts`：`registerEditorMotionCommands(app, getView)` 把 `@codemirror/commands` 的 `moveLineUp`/`moveLineDown`/`copyLineUp`/`copyLineDown` 暴露成命名命令（`editor:move-line-up`/`-down`/`copy-line-up`/`-down`，name thunk，`available: getView()!==null`，callback=`cmd(view);view.focus()`）。键 move=`Alt+ArrowUp/Down`、copy=`Shift+Alt+ArrowUp/Down`。**关键认知**：CM `defaultKeymap` **已绑**这些键 → 行为今天已能用，本轮价值 = 命名化 + palette 可发现 + 可重绑；与 defaultKeymap 同键时 cmExtensions 的 Prec.highest `handleKeydown` 拦截器**先匹配** → preventDefault+return true → **单次触发无双发**（E2E 真实 `Alt+ArrowUp` 键击验证落点恰好一次 move）。App.tsx 接线（`getActiveFileEditorView` 双侧门控，与 format/composer 并列）+ `__geodeMotion` 探针（一次性 EditorState 跑纯变换）+ i18n。**零新依赖、无 Rust、无新 vault 写路径**（单 CM transaction → autosave）。`r51-e2e` **10/10**（6 纯变换 + 3 live + 1 键击路由）+ 桌面 `r51-probe` **6/6** + 回归 r33/r40/r24 不回退。**对抗评审 0 真缺陷 / 4 维全证伪**；主动处理评审点名 1 项：键位由初版 `Mod+Shift+Arrow` 改 `Alt+Arrow`（对齐 CM/Obsidian 约定、消 defaultKeymap 冗余、消 macOS 原生 `Cmd+Shift+↑` 遮蔽）+ 补真实键击断言。
- **下一项 = R51→R52 = 编辑命令补全 II（#⑳余项延续）**：把更多 `@codemirror/commands` 现成命令命名化 —— `toggleComment`（`editor:toggle-comment`，`Mod+/`，Obsidian 同款）、`insertBlankLine`、`indentMore`/`indentLess`、`deleteLine`、`selectLine` 等。切入：**完全复刻 R51 范式**（editorMotionCommands.ts 是现成模板——thin wrapper + getView 门控 + name thunk + Alt/Mod 键 + `__geode*` 纯变换探针 + i18n）。**零新依赖、无 Rust、低风险加性轮**。注意：toggleComment 依赖 markdown 语言的 commentTokens（CM lang-markdown 已配 `<!-- -->`，需确认 live/source 都生效）。**⚠️ 候选池近况**:第三梯队整块功能基本清空（#⑪⑫⑬ 整体完成,#⑩⑭⑮ 部分），第四梯队 **#⑲⑳ 完成**;**剩余实质大项 = #⑯ pop-out 弹出窗口 / #⑰ Canvas 白板（池内自标「远期大工程」,需用户显式拍板才启动）**。其余小余项:第四梯队 #⑱（live 渲染长尾,较硬,可拆，需块级 StateField）/#㉑（小众插件,逐个）+ #⑧ stacked tabs/linked view + #⑭ schema 桥接 + #⑮ OS deep-link[待拍板新 crate]。全队列见 ROADMAP R32+ 候选池。

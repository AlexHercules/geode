...
- 上一轮：**R57 Live preview 跨行 `$$` 数学 + 共享 HydratedBlockWidget ✓ 已交付**（R32+ 候选池第四梯队 **#⑱** live 渲染长尾）。① 把 R56 mermaid 的「占位+异步 hydrate」widget 抽到共享 `liveHydratedWidget.ts` 的 `HydratedBlockWidget`（重构 liveMermaid 复用，r56-e2e 13/13 护航零回退）。② 新 `liveMath.ts`：`$$…$$` **display 数学 live 渲染为 KaTeX**（R18 显式延后的「跨行 `$$` 仅淡显不渲染」）。**关键**：lezer-markdown **无 `$$` 节点** → `findMathBlockRanges` 用**行扫描**（镜像 `core/markdown` 块 math opener/closer/inner-close 规则）+ **renderMarkdownToHtml self-check**（slice 必须渲染出 `geode-math-block` 才纳入 → 渲染器为权威，检测永不与阅读视图分歧）。复用 hydrateEmbeds math pass（KaTeX）。接入 `livePreview()`（仅 live）+ `__geodeMath` 探针 + CSS。**零新依赖、无 Rust、纯 view 不改文档**。`r57-e2e` **19/19**（7 纯检测含边角 + 8 live widget[**异步 KaTeX headless 真渲染**/揭示/源码不变] + 4 D1 缩进守卫）+ 桌面 `r57-probe` **9/9** + 回归 r56/r55/r51/r52/r35/r33/r24/r29 不回退。**评审 1 minor 修 / 11 维证伪**（重构等价、扫描=渲染器一致[18 输入实测]、data-safety 零修改无 XSS）：**D1**=列表内缩进 `$$` 的 range 永远=`line.from`→行首守卫对 math 失效→被 widget 化→修=opener 要求 `indent === 0`（缩进 1-3 math 降级源码，与嵌套表格/mermaid 一致）。
- **下一项 = R57→R58**。⚠️ **#⑱ live 渲染长尾仅剩 `%%` 注释隐藏**（Setext 折叠 R54 / 表格 R55 / mermaid R56 / `$$` 数学 R57 已完成）。**建议 R58 = `%%` 注释 live 隐藏**（Obsidian 在 live/reading 隐藏 `%%comment%%`）：先 **derisk 阅读视图 markdown.ts 是否已处理 `%%`**（若已隐藏则镜像其判定；若未处理则按 Obsidian 隐藏）；inline `%%x%%` 用 mark-hide（`Decoration.replace` 非 block）、block `%%\n…\n%%` 用 `liveBlockWidget`（已支持任意 scan-based ranges），cursor reveal。做完 **#⑱ 基本收尾**。**🛑 此后候选池仅剩大轮/需用户拍板项（勿自动启动）**：#⑧ stacked tabs（内容级 cascade）/ #⑯ pop-out / #⑰ Canvas（均「远期大工程」）/ #⑮ OS deep-link（新 crate=硬边界#5）/ #⑭ schema 桥接。**报告须向用户强调：#⑱ 收尾后小项基本清空，下一步必须由用户指定优先级（Canvas/pop-out/stacked tabs 等大轮），否则无可自主推进的小项。** 全队列见 ROADMAP R32+ 候选池。

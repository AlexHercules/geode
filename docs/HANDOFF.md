...
- 版本 **v0.55.0**｜分支 `opus` → `origin/opus`（收尾 `git push`）｜开发机 macOS（本仓库路径）
- 上一轮：**R55 Live preview 表格 ✓ 已交付**（R32+ 候选池第四梯队 **#⑱** live 渲染长尾）。GFM 管道表格在 **live preview 渲染为真 `<table>`**（复用阅读视图 `renderMarkdownToHtml`）——**Geode 首个 cursor-aware block-replace widget**。新 `features/editor/liveTables.ts`：`findTableRanges`（纯，syntaxTree 取 Table 节点）+ `TableWidget`（innerHTML=renderMarkdownToHtml + mousedown→dispatch selection 揭示源码 + eq 比 source/from/html）+ `buildTableDecos`（**行首守卫**跳过非行首[blockquote/list/缩进]表格 + selection 相交揭示）+ `liveTables` **StateField**（provide decorations+atomicRanges）。接入 `livePreview()`（仅 live 模式）+ `__geodeTable` 探针 + CSS。**关键 CM 约束（踩坑）**：block-replace 装饰**必须经 StateField，不能经 ViewPlugin**（ViewPlugin 提供 block 装饰→初次 DocView update 时 `RangeSet.spans` 崩溃白屏）→ 镜像 frontmatter `fmField`。**纯 view 装饰不改文档=零数据丢失风险**（cursor/click 揭示源码可编辑）。`r55-e2e` **15/15**（4 纯 + 8 live widget + 3 nested-table guard）+ 桌面 `r55-probe` **6/6** + 回归 r51/r52/r35/r33/r24/r29 不回退。**评审 1 major + 2 minor 修 / data-safety 核心全证伪**（零文档修改、无编辑锁死、无 XSS 新面）：major=blockquote/list/缩进表格行中块装饰损坏→行首守卫降级显示源码；minor=eq 纳入 html。**确立 cursor-aware block widget 范式**（StateField + atomicRanges + selection-reveal + click-edit）。**scope 记一句**：原定 Setext dim 精修，derisk 证伪（Setext 下划线 `===` 与 ATX `#` 同为 `cm-md-h1 cm-md-mark` 已一致，无精修可做）→ 改取高价值 live 表格。
- **下一项 = R55→R56**。⚠️ **候选池「零依赖小加性轮」已清空**；但 R55 **确立了 cursor-aware block widget 范式**（`liveTables.ts` = 模板），#⑱ 余项现可复用它。**建议（Step 0 自选）**：① **Mermaid live 渲染**（mermaid 围栏代码块 live 渲染为图，复用 R55 block widget StateField 范式 + **R19 mermaid 两坑必先复核 ARCHITECTURE R19**；mermaid 是 FencedCode[info=mermaid] 节点，异步渲染）——高价值、可一轮；② **跨行 `$$` 数学 live**（display math 渲染，复用 block widget + 既有 `core/math.ts` loadKatex；注意 `$$` 是否被 lezer 解析为单节点需先 derisk）；③ **`%%` 注释 live 隐藏**（Obsidian 隐藏注释，复用 reveal 范式）。**🛑 需用户拍板/专门大轮（勿自动启动）**：#⑧ stacked tabs（内容级 cascade）/ #⑯ pop-out / #⑰ Canvas（均「远期大工程」）/ #⑮ OS deep-link（新 crate=硬边界#5）/ #⑭ schema 桥接。**继续在报告里向用户点明：剩余多为 #⑱ 长尾（可复用 R55 范式逐项做）或大轮（可指定优先级，想优先 Canvas/pop-out/stacked tabs 随时打断重定向）。** 全队列见 ROADMAP R32+ 候选池。

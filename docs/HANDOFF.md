...
- 版本 **v0.59.0**｜分支 `opus` → `origin/opus`（收尾 `git push`）｜开发机 macOS（本仓库路径）
- 上一轮：**R62 = 专用 Outgoing Links 出链面板（第五梯队 ㉓ → 出队）**——用户 `/loop /continue` 自定速推进第五梯队【小】项。**Step 0 grep 现状救场**：出链数据(`getOutgoingLinks`)+显示(`BacklinksPanel` 出链分区)早已存在 → 真缺口仅是「独立面板」（Obsidian Outgoing Links 是独立核心插件）→ 抽**独立右栏 tab** `features/outgoinglinks/`（镜像 OutlinePanel，复用 getOutgoingLinks，拆 Links/Unresolved 两分区 + `app:show-outgoing-links` 命令）。刻意不动组合 BacklinksPanel（出链同显两处=评审证伪可接受）。纯只读 view。对抗评审（Workflow 4 lens）19 finding→**7 确认全 nit/minor + 12 证伪**（createAndOpen 数据安全证伪）：修别名显示 `alias||target`/自有 i18n 键/e2e 补 ol-empty/本节。验证 typecheck 0 · r62-e2e 14/14 · r62-probe 5/5 真 WKWebView · 回归 r30/r41 面板切换不回退。再上一轮 **R61 = 图片嵌入尺寸 ㉒**（共享 `parseEmbedSize` reading+live 单一解析器→`<img width height>`，导出零改动继承；评审修巨数别名三端漂移=正则封 5 位；r26-bytes 41 案 0 违反/r61-e2e 15/15/probe 8/8）。**㉒/㉓ 连续两轮「候选池缺口」均部分已实现**（第 4、5 次 stale gap，Step 0 grep 已制度化挡返工）。更早历史见下方「三句话背景」+ ROADMAP。
- **下一项（用户 `/loop /continue` 口径：第五梯队【小】零依赖项逐项推进至清空）= ㉔ Smart typography 智能排版**。⚠️ **动手前先 `grep -rn "smarttypo\|弯引号\|transactionFilter\|inputHandler\|typewriter\|—\|…" src/` 核现状**（㉒/㉓/R54/R58/R60 反复栽 stale gap——「读 handoff 缺口清单」当线索而非事实，第二段永远先 grep 现状）。㉔ 范围：编辑器输入变换——弯引号 `"`/`'`、`--`→—、`---`→—、`...`→…；设置开关（默认可 OFF）；CM `EditorState.transactionFilter` 或 inputHandler；**代码块/数学内不变换 + IME 守卫（R33 先例）**。其后【小】队列：㉕ 多光标（补 `allowMultipleSelections`+`drawSelection`）→ ㉗ Outline 过滤 → ㉘ Footnotes 面板 → ㉙ 状态栏增强 → ㉛ 拖文件入编辑器。**候选池续命史**：R59 登记第五梯队（㉒–㊴），R60 登记第六梯队（㊵–㊿）+ compat 8 行；R61 出队 ㉒，R62 出队 ㉓。全队列见 ROADMAP「第五/第六梯队」。
  - **【小】loop 燃料（每个零依赖一轮，按价值取）**：~~㉒图片尺寸（R61 ✅）~~ / ~~㉓出链面板（R62 ✅）~~ / ㉔**Smart typography**（输入变换，注意代码/数学内不变换+IME 守卫）/ ㉕**多光标多选命令**（CM6 原生，细化：补 `allowMultipleSelections`）/ ㉗ Outline 过滤 / ㉘ Footnotes 面板 / ㉙ 状态栏增强 / ㉛ 拖文件入编辑器（**㉖ 粘贴URL / ㉚ Callout fallback 已出队=实为已完成**）。

...
- 版本 **v0.56.0**｜分支 `opus` → `origin/opus`（收尾 `git push`）｜开发机 macOS（本仓库路径）
- 上一轮：**R56 Live preview mermaid + 共享 block widget 抽取 ✓ 已交付**（R32+ 候选池第四梯队 **#⑱** live 渲染长尾）。① 把 R55 live 表格的 cursor-aware block widget 机制**抽到共享 `liveBlockWidget.ts`**（`liveBlockWidgets(spec: {ranges, widget})` = StateField + atomicRanges + 行首守卫 + selection-reveal），重构 `liveTables` 复用（r55-e2e 15/15 护航零回退）。② 新 `liveMermaid.ts`：```mermaid 围栏 **live 渲染为图**（R19 显式延后的「live mermaid 需 StateField 跨行 replace」正是 R55 范式）。`findMermaidRanges`（syntaxTree 取 `FencedCode` 且 `node.getChild("CodeInfo")` 首词==="mermaid"，镜像 fence renderer）+ `MermaidWidget`（innerHTML=`renderMarkdownToHtml` 出 `.geode-mermaid` 占位 + `void hydrateEmbeds(wrap, app, getPath())` 异步渲染 SVG，复用 R19 管线含 `mermaidBatchChain` 全局串行链→多 widget 不并发串图）。接入 `livePreview()`（仅 live）+ `__geodeMermaid` 探针 + CSS。**零新依赖、无 Rust、纯 view 不改文档**。`r56-e2e` **13/13**（5 纯检测/占位 + 8 live widget[**异步 SVG headless 真渲染**/揭示/源码不变/无报错]）+ 桌面 `r56-probe` **7/7** + 回归 r55/r51/r52/r35/r33/r24/r29 不回退。**评审 0 真缺陷 / 5 维全证伪**（重构等价、检测=渲染器一致[仅 HTML-实体 info 1 安全方向偏差]、**异步并发被 mermaidBatchChain 全局串行化**[实测 2 图无串色]、data-safety 零修改无 XSS）。
- **下一项 = R56→R57**。⚠️ 候选池「零依赖小加性轮」已清空，但 **`liveBlockWidget.ts` 现是共享范式**（`spec.ranges` 是任意函数，不限 syntaxTree 节点），#⑱ 余项继续复用。**建议（Step 0 自选）**：① **跨行 `$$` display 数学 live 渲染**（复用 `liveBlockWidgets` + 既有 `core/math.ts` loadKatex；**先 derisk `$$` 是否被 lezer 解析为节点**——若否则 `spec.ranges` 用 scan-based 扫 `$$...$$` 区段，liveBlockWidget 已支持任意 ranges 函数；阅读视图已渲染 `$$` 故复用 renderMarkdownToHtml+hydrateEmbeds 的 math pass）——高价值、可一轮；② **`%%` 注释 live 隐藏**（Obsidian 隐藏注释，inline `%%x%%` 用 mark-hide + block `%%\n…\n%%` 用 liveBlockWidget，cursor reveal）；③ 其它 #⑱ 长尾。**🛑 需用户拍板/专门大轮（勿自动启动）**：#⑧ stacked tabs（内容级 cascade）/ #⑯ pop-out / #⑰ Canvas（均「远期大工程」）/ #⑮ OS deep-link（新 crate=硬边界#5）/ #⑭ schema 桥接。**继续在报告里向用户点明：剩余 = #⑱ 长尾（复用 liveBlockWidget 逐项做）或大轮（可指定优先级，想优先 Canvas/pop-out/stacked tabs 随时打断重定向）。** 全队列见 ROADMAP R32+ 候选池。

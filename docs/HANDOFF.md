...
- 版本 **v0.76.0**｜分支 `opus` → `origin/opus`（收尾 `git push`）｜开发机 macOS（本仓库路径）
- 上一轮：**R79 = 外观补全 v1：强调色取色器 + 系统主题三态（候选池第六梯队 ㊺）**——用户 `/loop /continue` 自定速推进。**纯前端、不写 .md → data-safety 轻**（但改 workspace.ts 持久化树 → r45/r50 回归必跑）。① **强调色**：`<input type=color>` → 运行时覆盖 `--accent`+派生 `--accent-hover`/`--accent-muted`（color-mix）+ localStorage（hex 校验，非法 removeProperty 回默认）+ reset；② **系统主题三态**：`ThemeKind+="system"`，新纯函数 `resolveTheme(kind,systemPrefersDark)`（system→随 `prefers-color-scheme`），`workspace.watchSystemTheme()` matchMedia 监听实时跟随。**关键契约**：`state.theme` 载 kind（可 "system"），但 **`dataset.theme` + `theme:changed` 事件恒载 resolved 具体值 `"dark"|"light"`**（events.ts 类型不破、compat/mermaid/graph 消费者读 resolved）。`sanitizeState` 接 "system" 向后兼容（旧 blob 无损、未知回 dark）；captureLayout/applyLayout 对 "system" 仍 R45 全局态（删/保 current）。compat/themes.ts syncBodyClass 改读 dataset（resolved）。新 `core/appearance.ts accentColor` Store/setter + `core/workspace.ts resolveTheme`。**对抗评审 7 lens → 0 确认缺陷**（持久化向后兼容、事件载 resolved、watchSystemTheme 仅 system 动作、accent 校验回默认、分层）+ 采纳 2 nit（accent swatch 读 computed `--accent` 不硬编码色 / 补 R45×system 不变式断言）。验证 typecheck 0 · r79-e2e 21/21 · r79-probe 7/7 真 build · r26-bytes 0 · 回归 r45(10)/r50(15) 绿 · 简化门 clean。**v1 已知延期**：字体三族（需先引 `--font-*` 变量）/ Show inline title / ribbon 显隐。再上 **R78 = 图谱设置完整化 v1：显示+力 持久化设置面板（候选池第六梯队 ㊵）**——用户 `/loop /continue` 自定速推进。**纯前端、不写 .md、不动 markdown.ts → data-safety 轻**。把 GraphView 硬编码的 4 力（中心/斥力/链接力/链接距离）+ 4 显示（节点大小/连线粗细/文本淡出阈值/箭头）抽成齿轮设置面板（滑块）+ localStorage 持久化。新 `features/graph/graphPrefs.ts`（抽出可测纯逻辑：GraphPrefs 扩 forces/display + `GRAPH_RANGES`[滑块区间=clamp 边界单一真值] + `parseGraphPrefs` 逐字段 clamp/向后兼容旧 `{mode,depth,showAll}` blob + freeze DEFAULT_PREFS）。GraphView：prefs 镜像 stateRef（draw/rebuild 空 deps 闭包读最新）、`applyForces` 力滑块 **live-poke 运行中 d3 sim + alpha(0.3).restart()**（不重建，布局连续=Obsidian 手感）、显示滑块 requestDraw、`nodeRadius(n,scale)` 全 9 调用点传 nodeSize、箭头三角。**分层**：graph **绝不 import settings feature** → 滑块自建 `.graph-slider`。**对抗评审 7 lens → 0 确认缺陷**（prefs 镜像无 stale/打架、力 poke 力名一致 charge 取 `-repel`、clamp 对 NaN/超界/旧 blob/坏 JSON 全回安全值、9 调用点 nodeSize 一致、分层合规）+ 采纳 1 nit（freeze DEFAULT_PREFS 防就地 mutate）。验证 typecheck 0 · r78-e2e 16/16 · r78-probe 6/6 真 build · r26-bytes 0（未碰渲染）· 简化门 clean。**v1 已知延期**：过滤组（标签/附件作节点、孤立笔记——动 getGraph）/ 分组着色 / 局部深度>2。再上 **R77 = 块 ID 自动铸造 `^id`（候选池第五梯队【中】㊴；㊱–㊴ 全清，第五梯队空）**——用户 `/loop /continue` 自定速推进。**写 .md 轮**：「复制块引用/嵌入」命令对活动编辑器光标段落块铸短随机 `^id`（6 位 base36）append 行尾 + 复制 `[[Note#^id]]`/`![[Note#^id]]`。写回走 `view.dispatch`（R33/R40 既有 B-class autosave + getView 仅活动文件 DS-1，**非新写机制**，data-safety 继承）。新 `core/blockId.ts`（blockRefAt 复用 R13 frozen BLOCK_MARKER_RE/blocks 索引、collision-checked mint、空行/围栏/frontmatter 守 null）+ `features/editor/blockRefCommands.ts` 2 命令。**不碰 markdown.ts**（`^id` R13 已 strip → r26-bytes 0）。**对抗评审 7 lens（运行实例实测复现）→ 1 确认 MAJOR**：块链接硬编码 basename `[[name#^id]]` **绕过 R72 `formatLink` 单一真值**→三症状（重复名粘别处解析错笔记 / 文件名含 `]#|^` 元字符产坏链 / 忽略 link 设置）→ **修=走 `formatLink(metadata, path, path, {embed, subpath:"^"+id})`**（给 R72 formatLink **additive 加 subpath 形参**；resolve-back+unsafe 守卫+设置遵循；null→notice「无安全链接」，**id 仍正确铸入**）+ 2 minor（mint 恒 6 位 / collision 大小写对齐 R13）。验证 typecheck 0 · r77-e2e 14/14（含 unsafe 文件名不复制坏链 + unique shortest）· r77-probe 6/6 真 fs · r72-e2e 26 回归 · r26-bytes 0 · 简化门 clean。再上 **R76 = 任务自定义状态渲染（候选池第五梯队【中】㊳，reading v1）**——用户 `/loop /continue` 自定速推进。**字节敏感轮 + 字节保守设计**：阅读视图把 `- [/]`/`- [-]`/`- [>]`/`- [<]` 等非标准复选框态渲染为带 `data-task="<char>"` 的 checkbox + 区分样式（主题用 `li[data-task="/"]` 命中）。markdown.ts：`TASK_RE` `( |x|X)`→`[^\]]`（闭合 4 处 task 定义漂移）+ `is-checked` 仅 x/X + **data-task 仅非标准态 emit** → 标准 `[ ]`/`[x]` 阅读字节**逐字节不变**（r26-bytes `list-task` 0 violations，改前 `--baseline` 重捕 48 cases）。editor.css+export.css 自定义态样式。**对抗评审 7 lens → 1 确认 MAJOR（R70 元教训复发）**：渲染收敛让 `[/]` 成可点 checkbox，但**第 5 处 task 定义消费者**「阅读点击 toggle」`preview.ts toggleTaskOnLine` 仍窄正则 `( |x|X)` → 点自定义态 checkbox 死键；**修=放宽 `[^\]]` + flip 镜像 format.ts**（`x/X`→空、其余→`x`；标准态行为逐字节不变、自定义态 `[/]`→`[x]`）+ 补阅读点击 toggle e2e。**v1 已知延期**：**live preview 自定义态不渲染**（lezer `TaskList` 硬编码 `[ xX]` + Obsidian 自身 live 也不支持自定义态——阅读视图才是该特性主场）。验证 typecheck 0 · r76-e2e 17/17 · r76-probe 8/8 真 fs · r26-bytes 标准 0 violations · 回归 r26(12)/r35(25)/r68(40) 绿 · 简化门 clean。再上 **R75 = `query` 搜索结果嵌入代码块（候选池第五梯队【中】㊲）**——用户 `/loop /continue` 自定速推进。**字节敏感轮**（动 markdown.ts fence renderer → §C：改前 `r26-bytes --baseline` 重捕 → 改后仅 query fence 字节变、其余 47 cases 0 violations=隔离）。` ```query ` 代码块 → 实时搜索结果列表（复用 `core/search.ts` parseSearchQuery+evaluateSearch），reading + live 双态。镜像 mermaid fence→`.geode-query` 占位→hydrate 范式：新 `core/queryEmbed.ts`（runQueryBlock 全 vault 扫描 + renderQueryResult，结果项 `a.internal-link[data-target=path]`，全 textContent 杜绝 XSS）+ embeds.ts hydrateQuery pass（trackConnectivity 跳脱离节点、never-throws）+ 新 `features/editor/liveQuery.ts`（findQueryRanges + HydratedBlockWidget "cm-live-query"，复用 R55/R57 机器）。**共享 widget 改动**：liveHydratedWidget mousedown 加 `a.internal-link`→openWikilink 导航（**评审揭露冻结契约原方案 `.internal-link`+return 不可实现**——live 全局委托 liveClickHandler 只认 `.cm-live-mdlink`/`.cm-live-wikilink` 不认 `.internal-link` → 只 return 成死链；改 widget 内直接 openWikilink，且 `a.internal-link` 锚点限定不误命中 mermaid 的 SVG `<g>`）。**对抗评审 7 lens 各独立 + skeptic verify → 0 critical/0 major/1 minor（doc 一致性，已订正 As-built）**+ 证伪 8。**v1 已知延期**：行内 snippet（deriveLineHits 仍 SearchPanel 私有）/ 渲染选项 / 点击只跳文件不跳行。验证 typecheck 0 · r75-e2e 16/16 · r75-probe 8/8 真 fs · r26-bytes 0 violations · 回归 r34(15)/r55(15)/r56(13)/r26(12) 绿 · 简化门 clean。再上 **R74 = Slides 演示模式（候选池第五梯队【中】㊱）**——用户 `/loop /continue` 自定速推进。**零依赖极简自实现**（非 reveal.js→避硬边界#5），v1 纯静态只读：当前笔记按整行 `---` 水平分页 → 全屏 overlay 逐页渲染 + 键盘导航（←/→/Space/PageUp/PageDown）+ Esc 关 + 页计数。新 `features/slides/`（`slides.ts splitSlides`=先 `parseFrontmatter().to` 剥 frontmatter 再切 body 跳代码围栏 / `SlidesOverlay.tsx` / slides.css / dict.slides.ts）+ ModalKind+slides + 命令 `slides:start`。**分层关键**：slides 属 feature **绝不 import 别的 feature** → 用 **core** `renderMarkdownToHtml` + **core** `hydrateEmbeds`（非 editor 的），自建 imageSrc（`mimeForPath`+readBinary+blob，卸载 revoke）。**不写 .md、不动 markdown.ts**（r26-bytes 0 violations）。**对抗评审 6 lens 各独立 + skeptic verify（reviewer 自写 11 形态 splitSlides + focus 实测）→ 1 确认 MAJOR（数据安全底线#1）**：**overlay 不夺焦 → 演示时键入字符静默改写底层笔记**（overlay 是叠加层、editor 仍挂载持焦；window keydown 只拦箭头/空格不拦字符键）→ **修 = 挂载夺焦 `blur()`+overlay `tabIndex=-1` `focus()` + 拦 Tab 防走回编辑器 + CSS outline:none**；**套件假绿根因=原 e2e 走 editor-持焦+openModal 路径却不断言文档未变 → 补无变更不变式**。验证 typecheck 0 · r74-e2e 23/23 · r74-probe 7/7 真 fs · 回归 r26(12)/r30(25)/r26-bytes(0 violations) 绿 · 简化门 clean。再上 **R73 = `cssclasses` frontmatter 应用到笔记视图容器（候选池第六梯队 ㊼ slice）；㉟ Properties 类型化编辑经前置门判 R22 已实现而出队**——用户 `/loop /continue` 自定速推进。**两道前置门 gate① grep 揭露 ㉟「缺类型化编辑」描述失准**：R22 PropertiesPanel 已按 `effectivePropertyType` 分派 number/checkbox/date/datetime/chips 控件 + 类型菜单 `propertyTypes.assign` + `.obsidian/types.json` 注册表 + buildSetProperty 写回——㉟ **出队**（㉒–㉜ 同类失准已制度化的两道门正是为此，本轮成功拦截「重建已有控件」）。转做 ㊼（细化 ㉟）首条真缺口 `cssclasses` 应用。**纯读 additive**：笔记 frontmatter `cssclasses`（list/空格串/逗号串）+ 遗留单数 `cssclass` 作为 CSS 类加到笔记视图容器（preview `.markdown-preview-view` + live/source `.markdown-source-view`），供主题/CSS 片段定向单笔记（Obsidian 核心 + 迁移叙事）。新 `core/metadata.ts getCssClasses()`（复用 parseFrontmatter/asList/fmField，`\s+` 切 token+去重）；EditorPane `useMemo([handle,docRevision])` 即时响应（live+preview 双态，编辑即更新+stale 移除）。**不写 .md、不动 markdown.ts**（容器 className 不入 `previewHtml` 字节流 → r26-bytes 0 violations）。**对抗评审 6 lens 各独立 + skeptic verify（reviewer 自写 CM keystroke 复核）→ 0 确认缺陷**（CM 不覆写 host className 实测；className prop 非 innerHTML 无 XSS；docRevision 双态 bump）+ 采纳 1 nit（补 live 态容器响应性断言）。**桌面 probe §D 教训**：首版用 `document.querySelector` 读 className → WKWebView App-Nap 全 null → 改 always-on sync 探针 `__geodeCssClasses(path)` 读真 fs→token（DOM 应用 browser-E2E only，对齐 r71-probe）。验证 typecheck 0 · r73-e2e 23/23 · r73-probe 8/8 真 fs · 回归 r30(25)/r26(12)/r26-bytes(0 violations) 绿 · 简化门 clean。再上 **R72 = 新链接格式设置 wikilink↔markdown × 最短/相对/绝对（第五梯队【中】㉞-c → 出队；㉞ 整项 a/b/c 完成）**——用户 `/loop /continue` 自定速推进。**纯前端 additive**（改链接**创建**，不动改写引擎/渲染，未碰 markdown.ts，无字节套件风险）。新 `core/linkFormat.ts` `formatLink()` = 全部「文件→链接」5 构造点（noteComposer/attachments/cmExtensions/**unlinkedMentions**/SettingsModal）的**单一真值**（wiki↔md × 路径形式 × embed/alias，resolve-back 验证→无安全形 null=调用方跳过）+ 两道硬守卫（a wiki+相对→退最短；b embed→恒 wikilink）。本会话工作：补 unlinkedMentions 接线 + 对抗评审修根因。**对抗评审（Workflow 9 agent / 4 lens + 逐条 skeptic verify）5 确认 0 证伪 → 2 根因 + 1 注释**：① **unlinkedMentions 改写后校验（:385）用 wikilink resolver `resolveLink` 验 markdown 链接 → 笔记名含空格的 `%20` href 静默 skip（headline 功能对常见名失效）→ 修 `resolveByKind`（对齐 R70；评审揭露 r72-e2e 只测 `Note.md` 唯一未编码 basename 遮蔽此 bug → 补 spaced/ambiguous linkAll 用例）**；② **`formatLink` markdown 分支 display 含 `]` 产不可重解析 `[a]b](..)` 违反自身 null 契约 → display 含 `]`→null（对齐 WIKILINK_UNSAFE）**；③ extract 注释纠错。**顺手修**：ARCHITECTURE.md 在 R70 文档轮被写入 1 个 NUL 字节（line 132 map-key 分隔符，致 grep/rg 视二进制截断）→ `tr '\000' ' '` 复原。验证 typecheck 0 · r72-e2e 26/26 · r72-probe 15/15 真 WKWebView+真 fs · 回归 r24/r44/r67/r70/r71 绿 · 简化门 clean。再上 **R71 = markdown 内部链接渲染 + 点击导航（第五梯队【中】㉞-b → 出队）**——用户 `/loop /continue` 自定速推进。**改 markdown.ts 阅读视图管线=字节敏感**（数据安全 §C：r26-bytes `--baseline` 改前重捕 → 改后 0 violations，仅 2 md-internal 例预期变）。**关键设计**：md 内部链接**复用 wikilink 渲染+点击机器**——anchor 用 `data-target=已解析全路径`（非裸 href），点击委托喂 openWikilink（完整路径 resolveLink 必命中→openFile，**永不误建笔记**）；只对解析到 `.md` 笔记的产内部链接（附件会让 openWikilink 误建笔记，故附件/unresolved/external 保持旧 `<a href>` 字节不变）。新 `RenderMarkdownOptions.resolveMdLink` 由 5 渲染调用点注入。**对抗评审（3 lens + verify）9 确认 → 修 5 根因**：① live Ctrl-点内部链接 window.open 裸路径→无视修饰键导航；② live 内部 md 链接无 hover→hoverController 加 `.cm-live-mdlink` 分支；③ embeds+卡片渲染未传 resolveMdLink→补；④ 角括号 `<note.md>` 双端不一致→live 剥 `<>`；⑤ **根-绝对 `/Z.md` basename 模糊解析到错笔记（修 R70 resolveMarkdownLink MAJOR）→ position-bearing href 走精确路径**。验证 typecheck 0 · r71-e2e 17/17 · r71-probe 8/8 真 fs · r26-bytes 0 violations · 回归 r23/r25/r26/r35/r55/r63/r70 绿。再上 **R70 = md 链接重命名改写 ㉞-a**——用户 `/loop /continue` 自定速推进。**数据安全最高敏感轮**（动 R16 改写引擎核心）。关键设计：把 R16 verified-rewrite 五步引擎**按 `LinkRef.kind` 分叉**（补 `MARKDOWN_LINK_RE` 索引 + `resolveMarkdownLink`/`normalizeMdHref`，md href 走专用解析），重命名时 `[text](note.md)` 随 wikilink 一并改写（anchor/%20/相对/folder 移动；external/code 跳过；新 href 用 vault 绝对形）。**对抗评审（3 lens + verify）12 确认 → 修 5 根因 + 自查补 1 崩溃**：① `encodeMdHref` 扩到 `%()#?` 编码（否则重命名进括号/井号名留悬空链接）；② 新 `resolveByKind` 让 backlinks/graph/outgoing/compat resolvedLinks 正确解析 md href（否则锚点/编码 md 链接成图谱幽灵节点+反链缺失）；③ compat original 按 kind；④ ?query 尾保留；⑤ normalizeMdHref 相对解析一致；**+ 自查 e2e 抓到 getGraph 对 md→附件解析崩溃（非笔记解析当幽灵）**。验证 typecheck 0 · r70-e2e 23/23 · r70-probe 9/9 真 fs · 回归 r44/r47/r28/r62/r66/r24 绿。再上 **R69 = 全库标签重命名 ㉝**——用户 `/loop /continue` 自定速推进。**数据安全最高敏感轮**（批量改写 .md）。两道前置门：grep 确认 R41 TagsPanel 无重命名 + R16/R30 引擎已就绪；WebSearch 确认 Obsidian 核心。**关键设计**：新建 `core/tagRewrite.ts` **镜像 R30 propertyRewrite 五步 verified-rewrite**（NOT R16 偏移式——TagRef 只有 from 无 to、inline/frontmatter 标签不可区分→必须逐文件重构 edit）：runTail 串行 + flushAll/ensureFresh capture + never-cache 逐文件读 + inline 复用 metadata 导出的 `TAG_RE` 重扫现算偏移 splice + frontmatter 走 `buildSetProperty`（绝不手写 YAML）+ inline/fm edit 合并一组一个 undo 步 + post-rewrite 复解析断言（count 不变/旧消失/新到位）+ per-file skip。前缀匹配 `old`/`old/*` 大小写敏感。TagsPanel 右键 rename UI（照搬 AllProperties 菜单）。**对抗评审（3 lens + verify）9 确认（多 minor/nit）→ 修 3 根因**：① frontmatter 重写集对齐 parseNote 索引判据 ⊆ 索引集（`/^#/` strip + 丢空白项，不动用户看不见的垃圾项）；② 全跳过仍显 skip 数（R47）；③ 删死 import。验证 typecheck 0 · r69-e2e 36/36 · r69-probe 10/10 真 fs · 回归 r41/r30/r24 绿。再上 **R68 = 搜索运算符 ㉜**——用户 `/loop /continue` 自定速推进。两道前置门：grep 确认 R21 已做 `file/path/content/tag/line/case`（task*/property 当年延后）+ WebSearch 确认 task:/[property] 均 Obsidian 核心（`section:`/`block:` 故意延后=与 line: 无差异）。实现 = 扩 R21 冻结手写解析器（不破 grammar）：task 家族复用 `line:` 行级机制 + `TASK_LINE_RE`，property 查新 `SearchInput.frontmatter`（大小写不敏感键 + 字符串/数组值子串）。纯只读零依赖无 Rust。**对抗评审（3 lens parser/evaluator/contract）2 根因修**：① `TASK_LINE_RE` `[ xX]`→`[^\]]` 收敛 R40 `TASK_BOX_RE`（自定义复选框态 `[/]`/`[-]`/`[>]` 算 task，非空格=done）— 避免 task 定义第 4 次漂移；② `[key:]` 空值降级 key-exists（否则空子串匹配任意值）。**已知偏差**：裸 `[link]` 现按属性谓词解析（非字面文本），转义口 `content:[…]`/`"[…]"`。验证 typecheck 0 · r68-e2e 40/40 · r68-probe 16/16 真 WKWebView · 回归 r34/r41/r46/r38 绿。再上 **R67 = 拖文件入编辑器 ㉛（【小】项清空）**、R66 状态栏 ㉙、R65 Footnotes ㉘、R64 Outline 过滤 ㉗、R63 多光标 ㉕、R62 出链 ㉓、R61 图片尺寸 ㉒。**㉒–㉜ 连续多轮候选池描述失准/评审抓回归** → **两道前置门（grep 是否已实现 + WebSearch 是否核心，分项核）已制度化、每项必走**。更早历史见下方「三句话背景」+ ROADMAP。
- **下一项 = 【第六梯队】㊸ 搜索 UI**（ROADMAP「第六梯队取用顺序建议」：㊵✓→㊺✓→**㊸ 搜索 UI**→㊿ 标签页右键菜单）。**纯前端、不写 .md → data-safety 轻**。**两道前置门先走**：① `grep -rn "SearchPanel\|searchScope\|sortBy\|matchCase\|SEARCH_HISTORY\|collapse" src/features/search/ src/core/search.ts` 核现状——R21/R34/R68 已做搜索解析器+面板，看 SearchPanel UI 缺哪些 Obsidian 搜索面板交互（排序下拉 / 折叠全部结果 / 搜索范围 path 限定 / 显示更多上下文 / 复制结果 / 搜索历史等）；② WebSearch 确认 Obsidian 搜索面板 UI 核心控件（sort by [file name/modified/created]、collapse results、show more context、explain search term）。㊸ 范围预判：补 SearchPanel 缺的 UI 控件（优先**排序下拉** + **折叠/展开全部结果** + **show more context**——纯前端，复用 R21 deriveLineHits/FileResult），择高 ROI 子集一轮。**备选**：㊿ 标签页右键菜单（交互主入口）/ ㊵ 续（过滤/分组着色）/ ㊺ 续（字体三族）。**先看 ROADMAP ㊸ 行确认缺口清单**（前置门 gate① 防失准）。**候选池续命史**：R61–R79 出队 ㉒㉓㉕㉗㉘㉙㉛㉜㉝㉞-a㉞-b㉞-c㊼-cssclasses㊱㊲㊳㊴㊵-v1㊺-v1（㉔ 移除、㉟ R22 出队；第五梯队全清，第六梯队 ㊵/㊺ v1 完成）。全队列见 ROADMAP「第六梯队」。**纯前端、不写 .md → data-safety 轻**。**两道前置门先走**：① `grep -rn "appearance\|accentColor\|accent\|theme\|systemTheme\|colorScheme\|fontSize\|appearance.json" src/core/appearance.ts src/features/settings/` 核现状——R20 已有 appearance.json（fontSize/theme 等），看缺哪些 Obsidian 外观项（强调色自定义 / 跟随系统主题 / 字体族 / 行宽——R50 已做可读行宽? 核 ROADMAP ㊺ 行）；② WebSearch 确认 Obsidian Appearance 设置项（accent color picker / "Adapt to system" theme / interface+text font / CSS snippets 开关等核心）。㊺ 范围预判：补 Obsidian Appearance 缺项，优先**强调色自定义**（color picker → `--accent` CSS 变量覆盖 + appearance.json 持久化）+ **跟随系统主题**（`prefers-color-scheme` 媒体查询 → 自动 light/dark）。复用 R20 appearance Store + R50 行宽/缩放先例。**备选**：㊸ 搜索 UI / ㊿ 标签页右键菜单。**注意**：先看 ROADMAP ㊺ 行确认具体缺口清单（别重做 R20/R50 已有项——前置门 gate① 防失准）。**候选池续命史**：R61–R78 出队 ㉒㉓㉕㉗㉘㉙㉛㉜㉝㉞-a㉞-b㉞-c㊼-cssclasses㊱㊲㊳㊴㊵-v1（㉔ 移除、㉟ R22 已实现出队；**第五梯队全清，第六梯队 ㊵ v1 起步**）。全队列见 ROADMAP「第六梯队」。
  - **【小】loop 燃料**：~~全部清空~~（㉒R61 / ㉓R62 / ㉔移除 / ㉕R63 / ㉗R64 / ㉘R65 / ㉙R66 / ㉛R67；㉖粘贴URL/㉚callout 早出队）。
  - **【中】次优先（现进行中）**：~~㉜ 搜索运算符（R68）~~ / ~~㉝ 标签重命名（R69）~~ / ~~㉞-a md 链接改写（R70）~~ / ~~㉞-b md 链接渲染+导航（R71）~~ / ~~㉞-c 新链接格式设置（R72，㉞ 整项完成）~~ / ~~㉟ Properties 类型化编辑（R22 已实现，前置门出队）~~ / ~~㊱ Slides 演示模式（R74，零依赖 overlay）~~ / ~~㊲ query 嵌入（R75，动 markdown.ts fence，r26-bytes 隔离）~~ / ~~㊳ 任务自定义状态渲染（R76，reading v1，字节保守）~~ / ~~㊴ 块 ID 自动铸造（R77，写 .md，复用 R13+B-class）~~。**第五梯队全清。** 第六梯队（㊵–㊿，ROADMAP）：~~㊵ 图谱设置 v1（R78）~~ / ~~㊺ 外观补全 v1（R79，强调色+系统主题）~~ / **㊸ 搜索 UI（下一项，纯前端）** / ㊿ 标签页右键菜单 / ㊼ Properties 增强续缺口（cssclasses 已 R73）/ ㊵ 续（过滤/分组着色）/ ㊺ 续（字体三族）。**另：㊼ Properties 增强**——~~cssclasses 应用（R73）~~ / File properties 右侧栏 / 键盘导航 / tags chip 点击搜索 / date 链日记 / hover+embeds 容器应用。**【中】多为 core 逻辑/改写引擎，注意 data-safety（㊴ 写 .md）+ 复用既有引擎不另起。**

...
- 版本 **v0.54.0**｜分支 `opus` → `origin/opus`（收尾 `git push`）｜开发机 macOS（本仓库路径）
- 上一轮：**R54 Setext 标题折叠 ✓ 已交付**（R32+ 候选池第四梯队 **#⑱** live 渲染长尾）。**先纠过时判断**：探明发现 Setext 标题（`text\n===`/`text\n---`）的 **live 样式早已存在**（`syntaxHighlighting(mdHighlight)` 把 lezer 的 `t.heading1/2` 映射成 `cm-md-h1/2`，lezer 给 Setext 文本也打这些 tag；阅读视图 markdown-it `lheading` 也早渲染）→ **真实缺口 = 折叠**（`folding.ts` 自 R17 注释明写「SetextHeading out of scope」）。本轮扩展 foldService：新增 `SETEXT_HEADING_RE` + `headingLevel(name)`（ATX 1-6 / Setext 1-2 统一层级）；`headingSectionEnd` + `markdownFoldRange` 改用它 → Setext 在**文本行**加折叠点（折隐藏下划线+section）、ATX+Setext 互为 section 终止符。export `markdownFoldRange` + `__geodeFoldRange` 探针（一次性 EditorState+ensureSyntaxTree，纯几何）。**顺带修 latent bug**：「ATX section 后跟 Setext 同级标题」之前折穿（把 Setext 折没），现在在它前停（纯 ATX 文档折叠**逐行不变**）。**零新依赖、无 Rust、不碰 markdown.ts**（阅读视图已对）。`r54-e2e` **11/11**（7 纯 fold-range + 4 live 折叠）+ 桌面 `r54-probe` **8/8** + 回归 r29[fold 持久化]/r51/r52/r35/r33/r24 不回退。**评审 0 真缺陷 / 全维证伪**（折叠几何 / `---`·`===`·frontmatter·fence 歧义 / 探针-live 解析树逐字节一致）。
- **下一项 = R54→R55**。⚠️ **候选池「零依赖小加性轮」已基本清空**——剩余多为更大/更硬的项或需用户定优先级。**建议（Step 0 自选，按可一轮干净交付排序）**：① **Setext 下划线 dim 视觉精修**（`===`/`---` 现 live 渲染为 heading 字号偏大，Obsidian dim/隐藏标记）——cursor-aware ViewPlugin 装饰，**单子项、零依赖、可一轮**（但 view 依赖、probe 难驱动，靠 E2E）；② 余下 **#㉑ 小插件**中较清爽的（Format converter 纯转换 / Footnotes 处理，零依赖，但部分 ROADMAP 条目是否对应真 Obsidian 核心插件需先 WebFetch 核实）；③ **#⑱ StateField 长尾**（live 表格 / 跨行 `$$`·`%%` / mermaid live widget）——**较硬，需块级 StateField，专门一轮**。**🛑 需用户拍板/专门大轮（勿自动启动）**：#⑧ stacked tabs（内容级 cascade，多 EditorPane）/ #⑯ pop-out / #⑰ Canvas（均「远期大工程」）/ #⑮ OS deep-link（新 crate=硬边界#5）/ #⑭ schema 桥接。**小项将尽——继续在报告里向用户点明：剩余项多为大轮，可由用户指定优先级（如想优先 Canvas/pop-out/stacked tabs 随时打断重定向）。** 全队列见 ROADMAP R32+ 候选池。

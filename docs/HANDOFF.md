...
- 版本 **v0.47.0**｜分支 `opus` → `origin/opus`（收尾 `git push`）｜开发机 macOS（本仓库路径）
- 上一轮：**R47 笔记合并（Note composer merge）✓ 已交付**（R32+ 候选池第三梯队 **#⑬ 另一半 → #⑬ 完成**）。**数据安全关键轮**。`linkRewrite.ts` refactor 加 `{move}` 选项 + `rewriteLinksForMerge`（`move:false` 跳过 rename + remap=identity + 始终改写，**复用同一 R16 verified capture→splice→post-rewrite-reassert 引擎**）+ `core/noteMerge.ts` `mergeNotes`（flush[try/catch]→**live-buffer 读**→append→ensureFresh→rewrite→trash[R42 可恢复]，**append-before-trash 无损**）+ `mergeTargetMode` 一次性 Store + QuickSwitcher merge 模式（StrictMode 安全快照 + consume skipped→notice + .catch）+ `editor:merge-file` 命令（switcher 已开 no-op 防泄漏）+ `__geodeMerge` 探针。**零新依赖、无 Rust**。`r47-e2e` **11/11** + 桌面 `r47-probe` **7/7**（**on-disk 数据安全终态**）+ 回归 r28/r24/r44/r42 不回退（frozen 引擎不破）。**3 维对抗评审（数据安全+frozen 引擎重点）12 finding → 4 确认（全 major）逐条修 + 8 证伪**（merge 丢弃 result.skipped→悬空链无告警→consume+notice / 链无 .catch→静默失败→catch / flushAll 失败读旧磁盘丢未保存编辑→live-buffer 读 / merge-file 命令 switcher 已开泄漏 mergeTargetMode→守卫）。
- **下一项 = R47→R48 = 补全 #⑫ 可配置日记设置 UI**（R43 做了日历 + 前后日导航，**可配置设置 UI = #⑫ 另一半**，当前格式/文件夹/模板写死 "Daily Notes/YYYY-MM-DD"）。Obsidian Daily notes 设置：**日期格式**（moment token，如 YYYY-MM-DD）/ **新文件位置**（文件夹）/ **模板文件**。切入:① 设置存储（镜像既有偏好持久化——查 `core/foldStore`/localStorage 或 `.obsidian/daily-notes.json`）;② SettingsModal 加 daily-notes section（格式/文件夹/模板三字段，镜像现有 SECTIONS 模式）;③ `core/dailyNote.ts` 的 `DAILY_FOLDER`/格式由写死改为读设置（`dailyStamp`/`dailyNotePath` 接受 format/folder 参数或读设置 Store）;④ openOrCreateDailyNote 应用模板（复用 `core/templates.ts expandTemplate`）;⑤ 日历 + 命令跟随设置。**⚠️ 改 `core/dailyNote.ts` 注意**:R43 的 `parseDailyStamp`/`isDailyNotePath` 锚定了 YYYY-MM-DD，格式可配后解析要跟着变（或限制可配范围）。**零新依赖预判可行**（moment 已是依赖）。**远期大工程**（池内自标，建议用户显式拍板再做）:#⑯ pop-out 弹出窗口（Tauri 多 WebviewWindow，跨窗状态同步）/ #⑰ Canvas 白板（JSONCanvas 无限画布引擎）。其余余项:#⑮ OS deep-link（待拍板新 crate）/ #⑭ Obsidian schema 桥接 / #⑪ 快照 / #⑧ stacked-linked + 第四梯队（编辑器实时渲染长尾）。全队列见 ROADMAP R32+ 候选池。

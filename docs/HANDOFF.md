...
- 版本 **v0.45.0**｜分支 `opus` → `origin/opus`（收尾 `git push`）｜开发机 macOS（本仓库路径）
- 上一轮：**R45 保存的工作区布局（Workspaces）✓ 已交付**（R32+ 候选池第三梯队 **#⑭**）。新 `core/workspaces.ts`（命名布局存到 `<vault>/.obsidian/workspaces.json`，**裸 `"workspaces.json"`**——adapter 已相对 `.obsidian/` 解析；镜像 R27 bookmarks 串行 RMW + race guard + 保留 Obsidian 原 entry/top-level key + **空-store 守卫不抹盘**）+ `Workspace.captureLayout`（复用 persist 序列化，**去 theme/fontSize**）/`applyLayout`（`sanitizeState` 容错 + `closeMissingFileTabs` 剪缺失 + `pruneTabHistory`/`emitActiveFile` reconcile + 保留当前外观）+ `WorkspacesModal`（save/list/load/delete）+ `workspace:manage` 命令 + `__geodeWorkspaces` 探针 + `ModalKind+="workspaces"`。**零新依赖、无 Rust**。`r45-e2e` **10/10** + 桌面 `r45-probe` **6/6**（on-disk 验证）+ 回归 r37/r39/r36/r43/r27 不回退。**3 维对抗评审 10 finding → 6 确认（1 major + 5 minor，含 1 doc）逐条修 + 4 证伪**（captureLayout 误存 theme/fontSize→delete+保留当前外观 / applyLayout 不 reconcile→pruneTabHistory+emitActiveFile / enqueue 缺 try/catch+warn→拒写静默 / init 失败+persist 抹盘→空-store 守卫 / WORKSPACES_CONFIG 字面量回写；**证伪** applyLayout 不 flush[EditorPane unmount 已 flush]）。
- **下一项 = R45→R46 = R32+ 候选池第三梯队 #⑮ `obsidian://` URI / 深链**（Obsidian:`obsidian://open?vault=…&file=…`、`obsidian://new?…` 等深链;当前 compat 仅 gap-stub）。切入:**桌面 Tauri deep-link**（`tauri-plugin-deep-link` 或 `tauri.conf.json` 注册自定义 scheme `obsidian://` → Rust 收 URL → 转发前端路由）+ **浏览器降级**（`obsidian://` 链接在浏览器模式无 OS scheme，仅同源 in-app 解析）+ URI 解析器（core 纯函数:parse `obsidian://open?vault&file&heading&block` → 动作）+ 执行（open file/heading/创建）。**⚠️ 先评估是否需新依赖**：若用 `tauri-plugin-deep-link` crate=**新运行时依赖→撞硬边界#5,须停下问用户**;若能用 `tauri.conf.json` 内置 scheme 注册（Tauri 2 可能内置支持）则零新依赖。**Step 1 先判依赖,撞到就停。** 纯前端 URI 解析器（core）可先做、可测。**#⑭ 余项**（Obsidian schema 桥接）+ **#⑬ 余项**（merge）+ **#⑫ 余项**（日记设置 UI）+ **#⑪ 余项**（快照）+ **#⑧ 余项**（stacked/linked）仍延后。其后队列:⑯ pop-out（大）→ ⑰ canvas（大）→ 第四梯队（全队列见 ROADMAP）。

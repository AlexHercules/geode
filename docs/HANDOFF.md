...
- 版本 **v0.62.0**｜分支 `opus` → `origin/opus`（收尾 `git push`）｜开发机 macOS（本仓库路径）
- 上一轮：**R65 = Footnotes 脚注面板（第五梯队 ㉘ → 出队）**——用户 `/loop /continue` 自定速推进第五梯队【小】项。两道前置门：grep 确认脚注解析存在（R18 阅读视图）但无 metadata 索引/无面板（真缺口）+ WebSearch 确认 Obsidian 1.9 把 Footnotes view 做成**核心插件**（核心非社区 → 做，Gate 2 存疑解除）。实现 = metadata 索引（镜像 headings：`FOOTNOTE_DEF_RE` on masked 排围栏，content 取原文，`getFootnotes`）+ 新右栏 tab `features/footnotes/`（列 id+content，点击 jumpTo 定义复用 `geode:scroll-to-heading`）。**对抗评审 1 根因（解析器是阅读视图脚注解析的子集→分歧，R56/R57 再现）→ 部分修+部分记限制**：修 body 首段内联代码丢失（从 `]:` 边界在原文 slice）；记限制 1-3 空格缩进定义被漏（保 col-0 锚定=与 HEADING_RE/TAG_RE 同约定）+ 多行续行不合并（面板单行）。验证 typecheck 0 · r65-e2e 12/12 · r65-probe 6/6 真 fs · metadata 回归全绿。再上 **R64 = Outline 过滤 ㉗**、**R63 = 多光标 ㉕（㉔ 移除非核心）**、**R62 = 出链 ㉓**、**R61 = 图片尺寸 ㉒**。**㉒–㉘ 连续多轮候选池描述失准** → **两道前置门（grep 是否已实现 + WebSearch 是否核心）已制度化、每项必走**。更早历史见下方「三句话背景」+ ROADMAP。
- **下一项（用户 `/loop /continue` 口径：第五梯队【小】零依赖项逐项推进至清空）= ㉙ 状态栏增强**（光标行:列 / 选中字数 / 当前笔记后链数）。⚠️ **两道前置门先走**：① `grep -rn "status-bar\|statusbar\|状态栏\|word-count\|wordCount\|cursor.*line\|cm-statusbar" src/` 核现状（**word-count 插件已在状态栏**——R 某轮；要确认状态栏框架 + 现有项，本项是「增强」非「新建」）；② WebSearch 确认 Obsidian **核心**状态栏显示这些吗（字数=核心 Word count 插件；光标行列/后链数=？可能部分非核心，分项核）。㉙ 范围：状态栏加 光标行:列、选中字数、当前笔记后链数；复用 backlinks 索引 + CM selection；逐块加，零依赖。其后【小】队列：㉛ 拖文件入编辑器（**㉖ 粘贴URL / ㉚ Callout fallback 已出队**）→ 【小】项基本清空后进【中】㉜–㊴。**候选池续命史**：R61 出队 ㉒、R62 ㉓、R63 ㉕（㉔ 移除）、R64 ㉗、R65 ㉘。全队列见 ROADMAP「第五/第六梯队」。
  - **【小】loop 燃料（每个零依赖一轮，按价值取）**：~~㉒图片尺寸（R61✅）~~ / ~~㉓出链面板（R62✅）~~ / ~~㉔Smart typography（R63 移除=非核心）~~ / ~~㉕多光标（R63✅）~~ / ~~㉗Outline 过滤（R64✅）~~ / ~~㉘Footnotes 面板（R65✅）~~ / ㉙ **状态栏增强**（⚠️分项核是否核心 + word-count 已在状态栏=增强非新建）/ ㉛ 拖文件入编辑器（**㉖ 粘贴URL / ㉚ Callout fallback 已出队=实为已完成**）。

...
**R16 重要提示：P2 特性池已见底。** P1（发布渠道+Authenticode 证书）是仅剩的高价值
事项且需要用户两个决策（渠道选择、证书购买）。若用户仍无输入：可做的只剩全文搜索
专项（PERFORMANCE.md R3 遗留：防抖自适应/倒排索引）或体验长尾（PDF 嵌入、[[#h]]
同文链接）——做之前先评估"等待用户输入 + 暂停"是否比硬找活更负责任，并把这个判断
明确告诉用户。

...
- 版本 **v0.46.0**｜分支 `opus` → `origin/opus`（收尾 `git push`）｜开发机 macOS（本仓库路径）
- 上一轮：**R46 `obsidian://` URI 深链（零依赖 in-app 切片）✓ 已交付**（R32+ 候选池第三梯队 **#⑮**）。新 `core/obsidianUri.ts`（纯解析器 `parseObsidianUri` → open/new/search/unknown）+ `features/editor/obsidianUriHandler.ts`（`handleObsidianUri` 执行:open→`resolveLink` gate **只开不建**+`openWikilink` reveal、new→`vault.create`、search→`requestSearch`）+ EditorPane 笔记内 `obsidian://` 点击路由 + `__geodeUri` 探针 + **core `Vault.create` 加 `assertSafeRelPath`**（拒空/绝对/`..`/控制符,双端一致）。**🛑 OS 级 deep-link 待用户拍板**（`tauri-plugin-deep-link` 新 crate=硬边界#5,本轮不引入）。**零新依赖、无 Rust**。`r46-e2e` **18/18** + 桌面 `r46-probe` **9/9**（on-disk new 建文件）+ 回归 r44/r43/r28/r24/r25/r33/r45 不回退。**3 维对抗评审（安全重点）12 finding → 5 确认（全 minor）逐条修 + 7 证伪**（安全攻击面全证伪:穿越被 safe_join、SEC-1 强化、scheme 口径一致、search 管线硬化;修:open 经 openWikilink 静默建笔记→resolveLink gate / Memory 无路径守卫→core assertSafeRelPath / new 文件名控制符→同守卫 / create 失败仍 openFile 幽灵 tab→fileExists 守卫 / `??` 遮蔽空 file=→`||`）。
- **下一项 = R46→R47 = 补全第三梯队 partial 项 #⑬ 笔记合并（Note composer merge）**（R44 已做 extract;合并 = #⑬ 另一半）。Obsidian「Merge current file with another file」:把当前笔记内容**追加进目标笔记**、**把所有指向当前笔记的链接改指目标**、**删当前笔记**（入 `.trash`）。切入:① 选目标的 UI（复用 QuickSwitcher 风格选择器 或新模态）;② 追加（读两者内容,`vault.modify(target, target+"\n\n"+source)`）;③ **link 改写 A→B**——R16 引擎是 rename-with-link-update（`vault.rename(A,B)` 在 B 存在时覆盖,不能直接用）→ 需**抽出 link-rewrite-only 核**或新写一遍 capture→verify→splice（务必带 R16 的 post-rewrite 复解析断言,见 ARCHITECTURE R16/R24）;④ 删源走 R42 `vault.trash`（可恢复,非永久）。**⚠️ 数据安全关键轮**:多文件写 + link 改写 + 删文件 → **必加载 data-safety skill 全清单**（R16 五步、改写复解析断言、flush-before-move、删除竞态）。**零新依赖预判可行**。**远期大工程**（池内自标）:#⑯ pop-out 弹出窗口（Tauri 多 WebviewWindow,跨窗状态同步）/ #⑰ Canvas 白板（JSONCanvas 无限画布引擎）——建议留作池末或用户显式拍板再做。其余余项:#⑮ OS deep-link（待拍板）/ #⑭ Obsidian schema 桥接 / #⑫ 日记设置 UI / #⑪ 快照 / #⑧ stacked-linked + 第四梯队（编辑器实时渲染长尾）。全队列见 ROADMAP R32+ 候选池。

...
- 版本 **v0.60.0**｜分支 `opus` → `origin/opus`（收尾 `git push`）｜开发机 macOS（本仓库路径）
- 上一轮：**R63 = 多光标/多选 foundation（第五梯队 ㉕ → 出队）+ ㉔ 移除（非核心）**——用户 `/loop /continue` 自定速推进第五梯队【小】项。**两道前置门**：① faithfulness 门：㉔ Smart typography（弯引号/em-dash/…）WebSearch 证实是社区插件非 Obsidian 核心 → 与 `{{date+3d}}` 同类**出队不做**；② grep/derisk 门：㉕ 真实缺口 = Geode 根本无法持有/渲染 >1 光标（无 allowMultipleSelections/drawSelection），命令早全在 keymap 只是静默 no-op → **修=2 行 foundation**（`allowMultipleSelections`+`drawSelection`+`rectangularSelection`+`crosshairCursor`）。Cmd+D 保留 daily-note（select-next 也是社区插件非核心）。零依赖。**对抗评审抓到 1 真数据安全回归→已修**：fmField Backspace 守卫只查 `selection.main`，多光标副光标可删 frontmatter 闭合换行（INT-3 回归）→ 改查所有 range + e2e 锁；余 nit（format 命令多光标仅作用主选区=预存）。验证 typecheck 0 · r63-e2e 9/9 · r63-probe 4/4 真 WKWebView · 回归 r35/r51/r34/r55/r30/r50/r32 全绿。再上一轮 **R62 = 专用 Outgoing Links 出链面板 ㉓**（独立右栏 tab，复用 getOutgoingLinks；评审 19→7 确认全 nit/minor）。再上 **R61 = 图片嵌入尺寸 ㉒**（共享 parseEmbedSize 单一解析器；评审修巨数别名三端漂移=正则封 5 位）。**㉒/㉓/㉔/㉕ 连续四轮均有「候选池描述失准」**（㉒㉓ 部分已实现 stale gap、㉔ 非核心、㉕ 的 Cmd+D 描述错）→ **两道前置门（grep 是否已实现 + WebSearch 是否核心）已制度化**。更早历史见下方「三句话背景」+ ROADMAP。
- **下一项（用户 `/loop /continue` 口径：第五梯队【小】零依赖项逐项推进至清空）= ㉗ Outline 内搜索过滤**。⚠️ **两道前置门先走**：① `grep -rn "OutlinePanel\|outline.*filter\|过滤" src/features/outline/` 核现状（是否已实现）；② 若描述涉及「Obsidian 有 X」先确认 X 是核心非社区插件（㉔/㉕ 教训）。㉗ 范围：大纲面板顶部加过滤输入框，实时过滤标题（substring/fuzzy），改 `features/outline/OutlinePanel.tsx`，纯前端。其后【小】队列：㉘ Footnotes 面板 → ㉙ 状态栏增强（光标行列/选中字数/后链数）→ ㉛ 拖文件入编辑器。**候选池续命史**：R59 第五梯队（㉒–㊴），R60 第六梯队（㊵–㊿）+ compat 8 行；R61 出队 ㉒、R62 ㉓、R63 ㉕（㉔ 移除）。全队列见 ROADMAP「第五/第六梯队」。
  - **【小】loop 燃料（每个零依赖一轮，按价值取）**：~~㉒图片尺寸（R61✅）~~ / ~~㉓出链面板（R62✅）~~ / ~~㉔Smart typography（R63 移除=非核心）~~ / ~~㉕多光标（R63✅）~~ / ㉗ **Outline 过滤** / ㉘ Footnotes 面板 / ㉙ 状态栏增强 / ㉛ 拖文件入编辑器（**㉖ 粘贴URL / ㉚ Callout fallback 已出队=实为已完成**）。

...
- 版本 **v0.48.0**｜分支 `opus` → `origin/opus`（收尾 `git push`）｜开发机 macOS（本仓库路径）
- 上一轮：**R48 可配置日记设置（Configurable daily notes）✓ 已交付**（R32+ 候选池第三梯队 **#⑫ 另一半 → #⑫ 完成**）。`core/dailyNote.ts` 加 `dailyNoteFolder`/`dailyNoteFormat`/`dailyNoteTemplate` Store + setter（localStorage，镜像 templateFolder）+ `dailyStamp = moment.format(effFormat)` / `parseDailyStamp` 改 **moment STRICT parse**（**完整保 R43 over-match 守卫**）/ `isDailyNotePath` 用 effFolder + openOrCreate 应用模板（expandTemplate）+ SettingsModal「Daily notes」3 字段 + Calendar 反应式 + `__geodeDaily` setFormat/setFolder 探针。**零新依赖、无 Rust**。`r48-e2e` **13/13** + 桌面 `r48-probe` **9/9** + 回归 r43-e2e 22 / r43-probe 13 / r23 22 / r28 23 / r42 17 不回退。**3 维对抗评审 9 finding → 6 确认（1 major + 5 minor）→ 修 2 + 3 已知偏差 + 3 证伪**（修：effFolder 只 trim 未 strip 斜杠→尾斜杠双斜杠路径→strip+validateDir / `Vault.createFolder` 缺 assertSafeRelPath[R46 同根漏补]→加守卫；延期 minor：无日粒度格式碰撞、FS 非法字符 Windows 静默、插件描述硬编码）。
- **下一项 = R48→R49 = 第三梯队余项 #⑪ 文件恢复快照（File recovery snapshots）**（R42 做了回收站[删除可恢复]，**周期内容快照 = #⑪ 另一半**，当前无快照——编辑历史不可回溯）。Obsidian File recovery：**周期性保存笔记内容快照**（每 N 分钟 / 每次保存），可在「文件恢复」里浏览/还原旧版本。切入:① 快照存储（`.geode/snapshots/<file-path-encoded>/<timestamp>.md` 或单 `.geode/snapshots.json` 内容库，镜像 bookmarks/workspaces 的 `vault.adapter` 配置写;**评估 per-file 多版本 vs 单库**）;② 触发（hook 自动保存 flush 后 / 或定时器,**保留最近 N 版/M 天**,旧的剪枝）;③ 恢复 UI（命令 + 模态:列某文件的快照时间戳 → 预览 → 还原写回当前文件,还原前先快照当前防丢）;④ 探针 on-disk 验证。**⚠️ 数据安全相关**（写 `.geode/`、还原=覆盖当前文件——还原前必先快照当前;剪枝别误删）→ **加载 data-safety skill**。**零新依赖预判可行**（既有 vault.adapter 写 + 定时器）。**建议先评估快照存储形态再冻契约**（per-file 目录 vs 单 JSON 库;编码文件路径为快照 key 注意 `/` 与碰撞）。**远期大工程**（池内自标,待用户显式拍板）:#⑯ pop-out（Tauri 多窗）/ #⑰ Canvas（JSONCanvas 无限画布）。其余余项:#⑮ OS deep-link（待拍板新 crate）/ #⑭ Obsidian schema 桥接 / #⑧ stacked-linked + 第四梯队（编辑器实时渲染长尾）。全队列见 ROADMAP R32+ 候选池。

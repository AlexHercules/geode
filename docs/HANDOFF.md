...
- 版本 **v0.49.0**｜分支 `opus` → `origin/opus`（收尾 `git push`）｜开发机 macOS（本仓库路径）
- 上一轮：**R49 文件恢复快照（File recovery snapshots）✓ 已交付**（R32+ 候选池第三梯队 **#⑪ 另一半 → #⑪ 完成**）。**数据安全相关轮**。新 `core/snapshots.ts`（每 note 单 JSON `.obsidian/snapshots/<encodeURIComponent(path)>.json`；`file:modified` hook throttle 60s → 串行 RMW append + prune 25；**写路径 STRICT-parse 拒覆盖坏 JSON**[读路径容错]；restore **先 `documents.flushAll()` 再 force-快照当前再 `vault.modify` 写回**=脏 buffer 未保存编辑无损）+ RecoveryModal（浏览/预览/还原）+ `editor:file-recovery` 命令 + `ModalKind+="recovery"` + `__geodeSnapshots` 探针。复用 `vault.adapter.writeConfig`（配置写不触发 tree 刷新）。**零新依赖、无 Rust**。`r49-e2e` **12/12** + 桌面 `r49-probe` **9/9**（on-disk 数据安全终态）+ 回归 r24/r43/r27/r45/r47 不回退。**3 维对抗评审（数据安全重点）9 finding → 7 确认（2 major + 5 minor）逐条修 + 2 证伪**（坏 JSON→空 list 覆盖丢历史→STRICT-parse 拒覆盖 / restore 不 flush→脏 buffer 覆盖丢编辑→flushAll / enqueue 静默吞错→guarded+warn / lastSnapTs 写失败毒化节流→绑成功 / vault 切换不清 lastSnapTs / modal restore 无 .catch / revision bump 偷换 sel）。
- **下一项 = R49→R50 = 第四梯队 #⑲ 可读行宽 + 拼写检查 + 应用级缩放（Appearance 高频设置）**（Obsidian Appearance「Readable line length」很常用;当前所有 input `spellCheck={false}`、无 readableLineLength、缩放仅图谱内）。切入:① **Readable line length**（设置开关 + 编辑器/阅读视图正文 `max-width` CSS class 切换,镜像 R48 设置 Store 范式）;② **拼写检查**（设置开关 → 编辑器 `spellCheck` 由写死 false 改读设置;注意 CM 的 contentEditable spellcheck 属性）;③ **应用级字号缩放**（`Cmd/Ctrl + =/-/0` 命令调 `fontSize`（既有 `workspace.setFontSize`），全局 CSS 变量驱动;现仅图谱缩放）。**零新依赖、无 Rust**,镜像 R48/R45 设置 + 命令范式。**低风险加性轮**（无 vault 写、无数据安全）。**⚠️ 候选池近况**:第三梯队整块功能 #⑩-⑮ 已大幅完成（#⑪⑫⑬ 整体完成,#⑩⑭⑮ 部分）;**剩余实质大项 = #⑯ pop-out 弹出窗口 / #⑰ Canvas 白板（池内自标「远期大工程」,需用户显式拍板）**。其余小余项:第四梯队 #⑱（live 渲染长尾,较硬）/#⑳（move-line cmd,小）/#㉑（小众插件）+ #⑧ stacked/linked + #⑭ schema 桥接 + #⑮ OS deep-link[待拍板新 crate]。全队列见 ROADMAP R32+ 候选池。

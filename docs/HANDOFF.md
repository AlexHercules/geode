...
- 版本 **v0.50.0**｜分支 `opus` → `origin/opus`（收尾 `git push`）｜开发机 macOS（本仓库路径）
- 上一轮：**R50 可读行宽 + 拼写检查 + 应用级缩放（Appearance）✓ 已交付**（R32+ 候选池第四梯队 **#⑲**）。新 `core/appearance.ts`（`readableLineLength`/`spellcheckEnabled` Store + setter，localStorage，镜像 autoUpdateLinks）。Readable line length = 正文 cap（`.cm-content`/`.preview-content`/`.editor-loading`/reading-view properties-panel）由写死 46em 改 `var(--readable-line-width, 46em)`，setter 切 documentElement var（OFF=none=全宽）。Spellcheck = EditorPane `useStore` + effect 设 CM contentDOM spellcheck（反应式 + view 重建跟随）。Zoom = `app:zoom-in/out/reset`（Mod+=/-/0 → 既有 `setFontSize`）。SettingsModal 2 toggle + `applyAppearanceSettings()` boot + `__geodeAppearance` 探针 + i18n。**零新依赖、无 Rust、低风险加性轮**（默认保持现状）。`r50-e2e` **15/15** + 桌面 `r50-probe` **6/6** + 回归 r33/r24/r25/r49 不回退。**2 维对抗评审 2 finding → 1 确认（minor）修**（阅读视图 properties-panel 硬编码 46em 漏跟随 readable-line→改同源 var）。
- **下一项 = R50→R51 = 第四梯队 #⑳ 移动行上下 + 其它编辑命令**（Obsidian 有 move-line-up/down、CM `defaultKeymap` 经 Mod 已给 deleteLine 等但 move-line CM 默认无键）。切入:① `@codemirror/commands` 的 `moveLineUp`/`moveLineDown`/`copyLineUp`/`copyLineDown` 接命令（`editor:move-line-up`/`-down` 等,Obsidian 默认 `Mod+Shift+↑/↓`）;② 注册进 App.tsx 命令层（镜像 R36 tab 快捷键 + R33 format 命令的 getActiveFileEditorView 注入范式）;③ 可选 `insertBlankLine`/`toggleComment` 等 CM 现成命令。**零新依赖、无 Rust、低风险加性轮**（纯 CM 命令接线,无 vault 写）。**⚠️ 候选池近况**:第三梯队整块功能基本清空（#⑪⑫⑬ 整体完成,#⑩⑭⑮ 部分,第四梯队 #⑲ 完成）;**剩余实质大项 = #⑯ pop-out 弹出窗口 / #⑰ Canvas 白板（池内自标「远期大工程」,需用户显式拍板才启动）**。其余小余项:第四梯队 #⑱（live 渲染长尾,较硬,可拆）/#㉑（小众插件,逐个）+ #⑧ stacked/linked + #⑭ schema 桥接 + #⑮ OS deep-link[待拍板新 crate]。全队列见 ROADMAP R32+ 候选池。

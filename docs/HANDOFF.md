...
- 版本 **v0.52.0**｜分支 `opus` → `origin/opus`（收尾 `git push`）｜开发机 macOS（本仓库路径）
- 上一轮：**R52 编辑命令补全 II（toggle-comment / indent / 行操作）✓ 已交付**（R32+ 候选池第四梯队 **#⑳余项**，#⑳ 视为完成）。新 `features/editor/editorEditCommands.ts`：`registerEditorEditCommands(app, getView)` 把 `@codemirror/commands` 的 5 个 StateCommand（`toggleComment`/`indentMore`/`indentLess`/`insertBlankLine`/`selectLine`）暴露成命名命令（`editor:toggle-comment`[Mod+/]/`editor:indent`/`editor:unindent`/`editor:insert-blank-line`/`editor:select-line`，复刻 R51 范式：name thunk + `available: getView()!==null` + callback=`cmd(view);view.focus()`）。**headline = toggle-comment 产出 Obsidian `%%…%%` 注释**：markdown 自身**无** commentTokens（toggleComment 会 no-op）→ cmExtensions 加一行 `markdownLanguage.data.of({ commentTokens: { block: { open: "%%", close: "%%" } } })`，toggleComment 落 block 路径包/解 `%%`。**`deleteLine` 排除**（它是 `Command` 需真实 `view.moveVertically`，纯变换探针驱动不了；Obsidian 亦无此默认命令）。App.tsx 接线（`getActiveFileEditorView` 双侧门控）+ `__geodeEdit` 探针（一次性 EditorState 含 markdown+commentTokens，返回 `{doc,from,to}`，selectLine 只改选区故必返选区）+ i18n 5 键。**零新依赖、无 Rust、无新 vault 写路径**（单 CM transaction → autosave）。`r52-e2e` **11/11**（7 纯变换 + 3 live + 1 真实 `Meta+/` 键击）+ 桌面 `r52-probe` **6/6** + 回归 r51/r35/r33/r40/r24 不回退。**对抗评审 0 真缺陷 / 5 维全证伪**（一条 open===close=`%%` toggle 固有歧义备注：字面含 `%%` 的行会被判已注释而 uncomment——任何同分隔符 toggle[含 Obsidian]的本质语义，可逆，by-design 非缺陷）。
- **下一项 = R52→R53 = 候选池下一个零依赖项**。#⑳ 已完结，编辑器命令小项基本清空。**建议两条路线（Step 0 自选其一）**：① **#⑧ 余项** = stacked tabs 标签堆叠 / linked view 链接面板（反链·大纲·局部图谱「跟随活动文件」已有，缺「链接到指定 pane」开关 + 堆叠渲染）——动 workspace `PaneLeaf` 层 + 持久化，**触发 data-safety skill**，中等工程；② 想保持小步则挑 **#㉑ 小插件**（如 Unique note creator 唯一笔记前缀 = 纯 core 函数生成时间戳/唯一 id 名 + 命令 + 可选设置，零依赖、清爽，镜像 Obsidian 核心插件）。**⚠️ 候选池近况**:第三梯队整块基本清空（#⑪⑫⑬ 整体完成,#⑩⑭⑮ 部分），第四梯队 **#⑲⑳ 完成**;**剩余实质大项 = #⑯ pop-out 弹出窗口 / #⑰ Canvas 白板（池内自标「远期大工程」,需用户显式拍板才启动）**。其余小余项:#⑱（live 渲染长尾,较硬,需块级 StateField,可拆）/#㉑（小众插件,逐个）+ #⑧ stacked/linked + #⑭ schema 桥接 + #⑮ OS deep-link[待拍板新 crate]。全队列见 ROADMAP R32+ 候选池。

...
- 版本 **v0.61.0**｜分支 `opus` → `origin/opus`（收尾 `git push`）｜开发机 macOS（本仓库路径）
- 上一轮：**R64 = Outline 内搜索过滤（第五梯队 ㉗ → 出队）**——用户 `/loop /continue` 自定速推进第五梯队【小】项。两道前置门：grep 确认无过滤框（真缺口）+ WebSearch 确认 Obsidian 核心 Outline **确有过滤栏**（核心非社区，与 ㉔ 相反 → 做）。纯 view：`filterRows` 纯函数（镜像 buildRows/visibleRows）大小写不敏感 substring → 命中 + 祖先（淡显）不含后代（对齐 Obsidian）；query 切文件清空；过滤忽略 collapse、无匹配空态。**对抗评审 2 确认（本轮引入）**：① 切文件 query 在被动 useEffect（paint 后）重置 → 先渲一帧旧 query 可能闪「无匹配」→ **修=useLayoutEffect**；② is-ancestor 淡显 specificity 压过 hover → 加 hover 规则。验证 typecheck 0 · r64-e2e 15/15 · 纯 view 桌面 boot smoke 4/4。再上一轮 **R63 = 多光标 foundation ㉕ + ㉔ 移除（非核心）**（2 行 CM 扩展解锁已有 keymap；评审抓到并修 fmField 多光标 Backspace 数据安全回归）。再上 **R62 = 出链面板 ㉓**、**R61 = 图片尺寸 ㉒**。**㉒–㉗ 连续多轮候选池描述失准**（部分已实现 / 非核心 / 描述错）→ **两道前置门（grep 是否已实现 + WebSearch 是否核心）已制度化、每项必走**。更早历史见下方「三句话背景」+ ROADMAP。
- **下一项（用户 `/loop /continue` 口径：第五梯队【小】零依赖项逐项推进至清空）= ㉘ Footnotes view 脚注面板**。⚠️ **两道前置门先走**：① `grep -rn "footnote\|脚注\|\\[\\^" src/` 核现状（脚注解析/渲染是否已在，阅读视图 R18 已渲染脚注 → 大概率复用既有解析）；② WebSearch 确认 Obsidian **核心**有「Footnotes」面板吗？——⚠️**存疑**：Obsidian 核心可能**没有**独立脚注面板（脚注是 markdown 语法，但「脚注 pane」未必是核心插件，可能是社区插件如 "Footnotes"）。**动手前务必 Gate 2 确认是核心**，否则同 ㉔ 出队。㉘ 范围（若核心）：新右栏面板列当前笔记脚注定义 + 点击跳转，复用 metadata/markdown 脚注解析。其后【小】队列：㉙ 状态栏增强（光标行列/选中字数/后链数）→ ㉛ 拖文件入编辑器。**候选池续命史**：R61 出队 ㉒、R62 ㉓、R63 ㉕（㉔ 移除）、R64 ㉗。全队列见 ROADMAP「第五/第六梯队」。
  - **【小】loop 燃料（每个零依赖一轮，按价值取）**：~~㉒图片尺寸（R61✅）~~ / ~~㉓出链面板（R62✅）~~ / ~~㉔Smart typography（R63 移除=非核心）~~ / ~~㉕多光标（R63✅）~~ / ~~㉗Outline 过滤（R64✅）~~ / ㉘ **Footnotes 面板**（⚠️先 Gate 2 核是否 Obsidian 核心）/ ㉙ 状态栏增强 / ㉛ 拖文件入编辑器（**㉖ 粘贴URL / ㉚ Callout fallback 已出队=实为已完成**）。

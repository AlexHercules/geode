...
- 版本 **v0.58.0**｜分支 `opus` → `origin/opus`（收尾 `git push`）｜开发机 macOS（本仓库路径）
- 上一轮：**R61 = 图片嵌入尺寸 `![[img.png|200]]` / `|200x100`（第五梯队 ㉒ → 出队）**——用户 `/loop /continue` 自定速推进第五梯队【小】项。共享 `parseEmbedSize`（reading+live 单一解析器，去漂移）→ `<img width height>`；导出 `export.ts` 零改动继承（hydrate 只设 src）；非数字别名仍当 alt（字节级 r26-bytes 不变）。对抗评审（Workflow 4 lens + verify）：**1 根因确认修**（无上界 `Number()` → 巨数别名三端漂移：阅读/导出发 `"1e+21"`/`"Infinity"` 无效 HTML→intrinsic，live 走 ToUint32→clamp → **修=正则封 5 位 `\d{1,5}`**）+ ~9 证伪（video/pdf 仅图片 / CSS 不加 height:auto / 导出继承均安全）。验证 typecheck 0 · r26-bytes 41 案 0 违反（重捕基线+锁死）· r61-e2e 15/15 · r61-probe 8/8 真 WKWebView · cargo release 真重建。**顺手剥 ARCHITECTURE/HANDOFF 各 1 NUL+1 US 控制字节**（R44/R46 那批，破 grep/diff）。再上一轮 **R60 = Obsidian 差距深挖 II（零代码调研/文档轮）**——用户「再挖一轮差距，按文档格式补充」。Workflow 编排 14 域 explorer + 147 项 code-verify（**survey→对抗 verify 两段，制度化防 stale gap**）：152 去重→131 真缺口（89 NEW）。登记 **ROADMAP 第六梯队 ㊵–㊿**（图谱设置/编辑器设置面板/反链出链/搜索UI/wikilink补全/外观/标签面板/Properties/文件浏览器/命令快捷键/标签页右键）+ **OBSIDIAN-COMPAT 缺口表 R60 8 行**（商业主轴：`file-menu`钩子/`registerMarkdownPostProcessor`/`vault.readBinary` 等）。**状态纠正**：㉖ 粘贴URL、㉚ callout fallback **实为已完成出队**（第三/四次 stale gap）；㉕ 多光标细化、㊳ 任务渲染「待核」改「缺」；设置「重复」= 忠实 Obsidian（不合并，仅补 setting-desc）。再上一轮 **R58 = `%%` 注释探明=stale gap，无代码**。原定 R58 = `%%` 注释 live 隐藏（#⑱ 最后一项），探明发现 **`%%` inline 隐藏 R18 早已完成**：livePreview.ts 既有「same-line `%%comment%%` hiding + cross-line block line-tinting」+ 自己的 `commentDelimOffsets`（镜像 markdown.ts）。实测既有处理 inline `%%comment%%` 光标 off 隐藏 / inside 揭示 / doc 不变全绿。本轮写的冗余 `liveComments.ts` **已全部 revert**（tree=R57 状态，版本 0.57.0，无 feature commit）。**第三次「候选池缺口」实为已完成**（Setext 样式 R54、Setext-dim R55、`%%` R58）。上上轮 R57：`$$` 数学 live 渲染 + 抽共享 `HydratedBlockWidget`（r57-e2e 19、probe 9，评审 1 minor[D1 indent===0 守卫]修）。
- **下一项（用户 `/loop /continue` 口径：第五梯队【小】零依赖项逐项推进至清空）= ㉓ Outgoing links 出链面板**。⚠️ **动手前先 `grep -rn "outgoing\|出链\|getMetadata.*links" src/` 核现状**（R54/R58/R60 反复栽 stale gap——「读 handoff 缺口清单」当线索而非事实，第二段永远先 grep 现状）。㉓ 范围：新右栏面板 `features/outgoing/`，复用 `metadata.getMetadata(path).links`（已有索引）列当前笔记出链 + 未解析链接（红色），镜像 OutlinePanel/BacklinksPanel；纯前端零依赖。其后【小】队列：㉔ Smart typography → ㉕ 多光标（补 `allowMultipleSelections`+`drawSelection`）→ ㉗ Outline 过滤 → ㉘ Footnotes 面板 → ㉙ 状态栏增强 → ㉛ 拖文件入编辑器。**候选池续命史**：R59 登记第五梯队（㉒–㊴），R60 登记第六梯队（㊵–㊿）+ compat 8 行；R61 出队 ㉒。全队列见 ROADMAP「第五/第六梯队」。
  - **【小】loop 燃料（每个零依赖一轮，按价值取）**：~~㉒图片嵌入尺寸（R61 ✅）~~ / ㉓**Outgoing links 出链面板**（独立核心插件，复用 metadata.links）/ ㉕**多光标多选命令**（CM6 原生，细化：补 `allowMultipleSelections`）/ ㉔ Smart typography / ㉗ Outline 过滤 / ㉘ Footnotes 面板 / ㉙ 状态栏增强 / ㉛ 拖文件入编辑器（**㉖ 粘贴URL / ㉚ Callout fallback 已出队=实为已完成**）。

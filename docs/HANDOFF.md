...
- **下一项 = 用户指定优先级**（自主小池已满，候选池扩容到第六梯队）。最高 ROI 小项可选：㉒ 图片嵌入尺寸 / ㉕ 多光标（细化：补 `allowMultipleSelections`+`drawSelection`）/ ㊵ 图谱设置 / ㊺ 外观补全（强调色·系统主题三态）/ ㊸ 搜索 UI / ㊿ 标签页右键菜单 / ㊷ 出链独立面板。**或商业主轴**（compat，落 OBSIDIAN-COMPAT）：`registerMarkdownPostProcessor`（Dataview 命脉，大工程）/ `file-menu` 右键钩子。**候选池续命史**：R59 登记第五梯队（㉒–㊴），R60 登记第六梯队（㊵–㊿）+ compat 8 行。全队列见 ROADMAP「第五/第六梯队」。
  - **【小】loop 燃料（每个零依赖一轮，按价值取）**：㉒**图片嵌入尺寸** `![[img\|200]]`（高频高价值，纯渲染）/ ㉓**Outgoing links 出链面板**（独立核心插件，复用 metadata.links）/ ㉕**多光标多选命令**（CM6 原生，细化：补 `allowMultipleSelections`）/ ㉔ Smart typography / ㉗ Outline 过滤 / ㉘ Footnotes 面板 / ㉙ 状态栏增强 / ㉛ 拖文件入编辑器（**㉖ 粘贴URL / ㉚ Callout fallback 已出队=实为已完成**）。

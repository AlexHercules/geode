...
- 版本 **v0.64.0**｜分支 `opus` → `origin/opus`（收尾 `git push`）｜开发机 macOS（本仓库路径）
- 上一轮：**R67 = 拖拽 vault 文件入编辑器 → 链接/嵌入（第五梯队 ㉛ → 出队；【小】项清空）**——用户 `/loop /continue` 自定速推进。两道前置门：grep 确认外部文件摄入(R17)+树拖拽移动(R28)已在但 vault 内文件拖入编辑器是缺口 + WebSearch 确认 Obsidian 核心。实现 = 编辑器 drop handler 识别 `EXPLORER_MIME`（核心共享 MIME）→ note→[[Name]]/附件→![[name.ext]]，文件夹/未知跳过（fileExists）；dragover 接管；effectAllowed move→copyMove。sync 无 await 不写 vault。**对抗评审 1 主根因（3 lens）+1 major → 修**：裸包 basename 的两病——重名消歧缺失（[[Spec]] 链错文件）+ wikilink-unsafe 字符（Foo#Bar→渲染按 # 切错目标）→ **修=复用全代码库 fileToLinktext 规则（解析回本文件的最短形 basename/全路径 + 含 unsafe 字符则不插）**。验证 typecheck 0 · r67-e2e 9/9 · r67-probe 4/4 真 Tauri fs · 回归 r28/r35 绿。再上 **R66 = 状态栏 ㉙**、**R65 = Footnotes ㉘**、**R64 = Outline 过滤 ㉗**、**R63 = 多光标 ㉕（㉔ 移除）**、**R62 = 出链 ㉓**、**R61 = 图片尺寸 ㉒**。**㉒–㉛ 连续多轮候选池描述失准/评审抓回归** → **两道前置门（grep 是否已实现 + WebSearch 是否核心，分项核）已制度化、每项必走**。更早历史见下方「三句话背景」+ ROADMAP。
- **下一项 = 【中】㉜ 搜索运算符扩展**（【小】项已全部清空，进【中】梯队）。⚠️ **两道前置门先走**：① `grep -rn "task:\|line:\|section:\|searchOperator\|parseQuery\|搜索运算符" src/core/` 核现状（**R21 已做搜索运算符**：path:/file:/tag:/content:/[组合等；候选池备注「显式延期：block:/section:/task:/属性搜索」——本项是把延期的补上，要先核 R21 实现了哪些、延期了哪些）；② WebSearch 分项确认 Obsidian **核心**搜索支持哪些运算符（`task:`/`task-todo:`/`task-done:`/`line:`/`section:`/`[property]`/`[property:value]` 等——逐个核哪些是核心 vs 社区，可能部分非核心）。㉜ 范围：扩展 R21 的搜索查询解析（core 搜索模块），加核心运算符；纯 core 逻辑 + 测试，注意复用 R21 的 query parser 不另起一套。**候选池续命史**：R61–R67 出队 ㉒㉓㉕㉗㉘㉙㉛（㉔ 移除）。全队列见 ROADMAP「第五/第六梯队」。
  - **【小】loop 燃料**：~~全部清空~~（㉒R61 / ㉓R62 / ㉔移除 / ㉕R63 / ㉗R64 / ㉘R65 / ㉙R66 / ㉛R67；㉖粘贴URL/㉚callout 早出队）。
  - **【中】次优先（现进行中）**：**㉜ 搜索运算符扩展（task:/[property]/line: — 先核 R21 延期项 + 分项 Gate 2）** / ㉝ 标签重命名 / ㉞ 链接格式策略 / ㉟ Properties 类型化编辑 / ㊱ Slides / ㊲ query 嵌入 / ㊳ 任务自定义状态渲染 / ㊴ 块 ID 自动铸造。**【中】多为 core 逻辑/改写引擎，注意 data-safety（㉞/㊴ 写 .md）+ 复用既有引擎不另起。**

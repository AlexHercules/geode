...
## Round 260 additions — 真插件迁移 pre-Dataview gap：obsidian CM6 editor StateFields（editorInfoField / editorEditorField / editorLivePreviewField + editorViewField 别名）·逻辑档·data-safety·零新依赖【As-built·v0.253】

> **状态：As-built（已交付·v0.253）。** R257 roadmap 的 pre-Dataview 无依赖 gap。真插件（Dataview inline fields / decoration 插件）从自己的 CM6 扩展里读 `view.state.field(editorInfoField).file` / `editorLivePreviewField`(gate live-preview-only 渲染) / `editorEditorField`(从裸 EditorState 取回 view)。**Editor.getDoc 早 done（R220）。**
> **🔑 类型纠误（scout）**：`editorEditorField: StateField<EditorView>`（裸 CM6 view·**非** obsidian Editor）→ 无需稳定 per-view obsidian Editor；`editorInfoField: StateField<MarkdownFileInfo>` = 轻接口 `{ app, get file(), editor?, hoverPopover }`（非完整 MarkdownView）。
> **设计（三层桥·additive）**：
> - **`core/editorContext.ts`（新）**：2 个 Facet 桥——`editorPathFacet`(() => string·rename-safe 闭包)·`editorLivePreviewModeFacet`(boolean)。core 定义（可 import @codemirror）·features/editor provide·compat read（同 `core/editorExtensions.ts` registry 桥·compat 绝不 import features）。
> - **`features/editor/cmExtensions.ts`**：`editorModeExtensions` 返回 `[editorLivePreviewModeFacet.of(mode==="live"), livePreviewOr空]`（**放 mode compartment 内→3 个 EditorPane reconfigure 站点全经它·live↔source 切换免费 re-provide**）；基列表加 `editorPathFacet.of(getPath)`。
> - **`compat/obsidian/editorFields.ts`（新）**：3 个 StateField + 别名。`editorLivePreviewField`=StateField<boolean>(create/update 读 `editorLivePreviewModeFacet`·**reactive**：mode reconfigure transaction→update 读新 facet→翻转)；`editorEditorField`/`editorInfoField`=StateField<X|null>(setEffect 设)；`editorViewField`=editorInfoField(deprecated 别名·d.ts 对齐)。`editorFieldsExtension(app,registry)`=[3 field, ViewPlugin]·ViewPlugin 构造期 `new Editor(view)` 一次 + 建 info{app, get file(){registry.getFile(getPath())}, editor, hoverPopover:null} + **queueMicrotask 派发 effect-only transaction 填充**(CM6 禁构造期 dispatch·destroy guard 防卸载后派发)。
> - **`compat/obsidian/context.ts`**：`registerCoreEditorExtension(editorFieldsExtension(app, registry))`(disposer 管理·reload 重注册·app/registry 不陈旧·镜像 R200 editorMenu)；**`index.ts`** export 4 field。
> **data-safety（底线①·逻辑档·满跑·坐实零丢失）**：填充 dispatch **effect-only**(setEditorView/setEditorInfo·无 doc change)→无 docChanged→**无 autosave/dirty**(r260 实测填充后 doc 磁盘字节不变)；`editorModeExtensions` 仅 ADD facet 值在既有 livePreview 扩展旁·**无 decoration 逻辑改**·§C **markdown.ts 未碰**(这是 live-preview CM·非阅读视图 markdown.ts)·r18-diff N/A·live-preview 套件 r106/107/108/109/134/135/137/128 全绿(decoration 不退)。**桌面 probe N/A-by-equivalence**(纯前端 StateField/facet/ViewPlugin·真插件经共享 loader/registry 读·editor pipeline 跨端同)。**简化门 clean**·**ultracode 4-lens 对抗评审（12 agents）= 0 confirmed + 8 refuted**（microtask-null-window 被 3 lens 提但全证伪：真插件在 update()/decoration provider 读·seed transaction 重触发·拿到值·constructor-read-and-cache 非现实模式·非 regression；editorViewField 别名 = 现代 Obsidian 真行为；info.editor vs activeEditor.editor 非身份保证；data-safety lens 无丢失路径）。**已知设计**：editorEditorField/editorInfoField 在 seed microtask 前为 null（CM6 禁构造期 dispatch·真插件 reactive 读不受影响）。**测试**：`.calibration/r260-e2e.mjs` **12/12**（真注入插件注册 CM6 ViewPlugin 读 3 field·editorInfoField.file=ext1.md+app+editor[getValue]·editorEditorField===view·**editorLivePreviewField true→false on setTabMode source[reactive]**·editorViewField 别名·**effect-only seed 后 doc 字节不变[底线①]**）·回归 r106-128 live-preview/r115 7/7/r33 37/37/r24 12/12/r258 12/12 全绿·生产构建 ✓·cargo ✓。**下一片**：R261 STOP-POINT（js-yaml@^4.1.0 授权 + faithful parseYaml/stringifyYaml + 试载 Dataview/Templater）；或 adapter.stat[Rust]/getResourcePath[Tauri asset-protocol]/suggest-modal ES5 支持（无依赖余 gap）。

## Round 259 additions — 真插件迁移：第二 committed 真插件证明（read-time「Words to Read Time」1.0.0·MIT·ES6·零新依赖·零 src 改动）·validation/fixture 轮【As-built·v0.252】

> **状态：As-built（已交付·v0.252）。** R258 后续·**零 src 代码改动**（read-time 是 ES6 esbuild bundle·走既有 loader·不需 ES5 fix）= validation/fixture 轮（vendor + e2e·验 harness 泛化到第二真插件 + 拓宽 API 覆盖面）。**vendor**：`compat-fixtures/obsidian-read-time-plugin/`（main.js+manifest.json 1.0.0 release 原物[sha 校验逐字节同上游]+LICENSE[MIT © ryanschiang]+README 归属）。**互补覆盖面（vs R258 Sort&Permute 的 commands+editor）**：read-time **无命令**·exercises `addStatusBarItem` + `registerInterval`(500ms) + `registerDomEvent(document,"click")` + `getActiveViewOfType(MarkdownView)` + 真 `PluginSettingTab`(wpm 设置)·读 active note 词数→写状态栏 `"<m>m <s>s read time"`(默认 130wpm)。`isDesktopOnly:true` 但 loader 不 gate（浏览器 E2E 是 desktop app 的 dev surface·非 mobile·正确不 gate）。**data-safety 非 scope**（read-only 插件·只读 editor 词数+写状态栏 DOM+saveData 持 wpm 到 data.json·**零 vault 写**·grep 证无 vault.create/modify/delete）。**分档=validation 轮**（零 src·非数据安全）→ 跳简化门 → focused review（e2e 假阳性 + fixture 完整性 + 数据安全 sanity）= **CLEAN + 1 nit[e2e 注释误称 click 触发 recompute·实为 interval·已校]**·review 坐实 e2e 真绑插件执行（status=enabled 仅 onload 不抛才达·状态栏文字是插件 setText 非 Geode·非零值仅能来自插件读 live 词数·dynamic recompute 真切 note）。**测试**：`.calibration/r259-e2e.mjs` **7/7**（真 MIT 插件 enabled·状态栏初始 "0m 0s read time"·**260 词→~2 min[260/130]·字节级**·**13 词→0 min·dynamic recompute 真按 active note**·无 page error）·回归 r258 12/12[共享 loader/harness]·typecheck 0·桌面 N/A-by-equivalence。**下一片**：R260 = pre-Dataview 无依赖 gap（adapter.stat[Rust fs::metadata]/CM6 editorInfoField·LivePreviewField·EditorField + Editor.getDoc StateFields/getResourcePath URI 桥）→ **R261 STOP-POINT**（js-yaml@^4.1.0 授权 + faithful parseYaml + 试载 Dataview）；或 suggest/modal 族 ES5 支持（延迟 ctor listener 注册）。

## Round 258 additions — 真插件迁移：首 committed 真插件证明 + ES5/tslib base-class 修复 + loader 硬化（Sort & Permute lines 0.7.0·MIT·零新依赖）·逻辑档·data-safety【As-built·v0.251】

> **状态：As-built（已交付·v0.251）。** R257 立项首项。**vendor 真插件**：`compat-fixtures/obsidian-sort-and-permute-lines/`（main.js + manifest.json 0.7.0 release 原物 + LICENSE[MIT 复制] + README 归属·**非 src/·不 ship**·e2e fs 读取 + `window.__geodeObsidianPlugins` 注入·镜像 r221）。**关键：首个 committed 真插件证明立即抓到真 gap**——
> **① ES5/tslib base-class 修复（核心发现）**：真插件编译为 **ES5 + tslib `__extends`** 时，派生 ctor 经 `Base.apply(this, args)` 继承（`function MyPlugin(){ return _super.apply(this,arguments)||this }`·_super=obsidian.Plugin）。shim 的 base 是 **ES6 class** → `.apply` 抛 "Class constructor cannot be invoked without 'new'"。5 个既有真插件能跑是因它们 ES6-target（`super()`）；ES5/tslib 插件全挂。**修复 = `compat/obsidian/es5compat.ts` `es5Callable(Base)` Proxy**：`apply` trap `Object.assign(thisArg, new Ctor(...args)); return thisArg`（构造真实例·把其 own 字段嫁接到已带原型链的 `this`·`|| this` 期望 truthy 返回）。`new`/`super()`（ES6 插件）走 Proxy 默认 [[Construct]] 透明转发·**instanceof 双向保持**（getPrototypeOf 转发）。**接入点 = `loader.ts` require("obsidian") bridge**：`obsidianForPlugins = {...obsidianModule, Plugin: es5Callable(...), ...}` 包到 `HOST_MODULES.obsidian`（**仅插件可见·shim 内部/index.ts 仍用 raw ES6 class**）。
> - **⚠️ 评审纠（minor·已修）**：throwaway-harvest 仅对【字段-init / detached-DOM-in-fields】base 正确；**ctor 绑 `this`-捕获 listener 的 base（Modal/SuggestModal/FuzzySuggestModal/EditorSuggest/AbstractInputSuggest）拷贝后 listener 仍绑废弃 throwaway→静默失效**。故**包装集收窄至 throwaway-safe only**（Plugin/Component/MarkdownRenderChild/PluginSettingTab/View/ItemView/FileView）·suggest/modal 族**故意不包**→ ES5 插件继承它们时 hard-fail-to-load 带清晰 getLastError 错误（非静默坏实例·不比 R258 前差）·留未来轮把它们 ctor listener 注册延迟出 ctor。注释已校正（原注释谎称全集 clean-copy）。
> **② loader 硬化**：`core/plugins.ts` `PluginRecord.lastError?` + `enable()` 清+捕获 onload 错误 + `getLastError(id)` accessor（全加性·native+obsidian 共用 enable）；`loader.ts` onload-fail report `detail: plugins.getLastError(id) ?? "onload failed (see error above)"`（真错误进 obsidianLoadReport·非吞进 console）。
> **data-safety（底线①·逻辑档 满跑·坐实零丢失）**：真插件经 `editor.setValue/replaceRange`（obsidian Editor shim）→ `cm.dispatch`（无 sync annotation）→ docChanged → autosave updateListener（documents.ts）→ 防抖落盘·**R258 未碰任何写路径**（editor.ts/documents.ts/vault 零改·仅 loader+plugins+es5compat）。§B 无新 .md 写路径·§C markdown.ts 未碰 r18-diff N/A·**桌面 probe N/A-by-equivalence**（loader/shim/es5compat 跨端同码·真 bundle 经真 loader 在浏览器载入证明·5 个 desktop 插件 ES6 不受影响）。**简化门 clean**·**ultracode 4-lens 对抗评审（13 agents）= data-safety PASS + 1 confirmed[throwaway-suggest·已修收窄+注释]+8 refuted**（含 MIT license 复制·已补 LICENSE）。**测试**：`.calibration/r258-e2e.mjs` **12/12**（真 MIT 插件 enabled[load report]·11 命令注册·**sort-alphabetically→"apple|banana|cherry" 编辑器+磁盘字节级**·**permute-reverse→"cherry|banana|apple" 磁盘·底线① 真插件编辑落盘**·**坏插件 report 带真 onload 错误[R258_BOOM]·硬化坐实**）·回归 r221 14/14[ES6 插件+instanceof]·r171/r166/r167/r175 fixture·r33 37/37·r24 12/12 全绿·生产构建 ✓·cargo ✓。**下一片**：R259 = 第二代表插件（read-time·证 PluginSettingTab + statusbar + registerInterval）或 suggest/modal 族 ES5 支持（延迟 ctor 注册）；R260 = adapter.stat/CM6 StateFields/getResourcePath（无依赖 gap）；R261 STOP-POINT = js-yaml 授权 + Dataview。

## Round 257 additions — 真插件迁移闭环【🔒 立项·docs-only·零依赖路线·实现 R258 起】（用户拍板「选项1 商业主轴」·ultracode 3-scout read-only 立项）

> **状态：立项（plan frozen·实现未落地）。** 用户 R256 收尾拐点拍板「真插件迁移闭环（商业主轴）」。**🔑 立项决定性发现（read-only scout 纠误）**：**Geode 早已加载真·Obsidian-格式社区插件**——`compat/obsidian/loader.ts` 是完整 host（列 `<vault>/.obsidian/plugins/*`·验 manifest·`new Function("require","module","exports", mainJs)` 评 CJS·`require("obsidian")`→shim·`require("@codemirror/*")`→共享 host 实例[instanceof 跨 plugin/host 成立]·注册同 community-plugins.json）；**5 个真插件今天就在跑**（nldates 1.3M / calendar 140K / recent-files 56K / url-into-selection 8K / better-word-count 2.5M·gitignored `compat-vault/`·desktop probe 验）。**故「真插件迁移」非从零——T0-T2 闭环已成**。剩两块：① **committed/自动化证明 + loader 硬化**（现 5 插件仅手动 desktop probe·自动化测用 synthetic fixture `compat/obsidian/fixture.ts`·非真 bundle）；② **重型生态 tier 的 breadth gaps**（Dataview/Templater/Excalidraw 卡在 named gaps：**parseYaml/stringifyYaml**[唯一真·依赖-gated 阻断=js-yaml]·getResourcePath URI 桥[Tauri asset-protocol·非 npm dep]·Lucide setIcon[已优雅降级·非阻断]·adapter.stat[Rust]·CM6 editorInfoField StateFields）。
>
> **🎯 首目标 = Sort & Permute lines**（Vinzent03/obsidian-sort-and-permute-lines·**MIT·0 runtime dep·~350 LOC**）：exercises addCommand×10 + 全 Editor API(getValue/setValue/replaceRange/getCursor/getLine/lastLine) + metadataCache.getFileCache(ListItemCache) + workspace.getActiveViewOfType(MarkdownView)·import 仅 `{ListItemCache,MarkdownView,Plugin}`·**不调 parseYaml**→现零依赖码即可证。胜过 toggle-case(deprecated·仅 smoke)与 read-time(无 addCommand)。
>
> **🛑 依赖授权（硬边界#5）= 现在无需任何授权**：首目标(Sort&Permute)+ R258-R260 全程**零新 runtime dep**（moment R5 已加·setIcon 已 shim·CM6/@lezer 共享 host）。**唯一将来需授权的 = `js-yaml@^4.1.0`**（faithful 背 parseYaml/stringifyYaml·core/properties.ts 是 frontmatter-子集解析器·不能当 parseYaml[会静默误析嵌套/anchor/multi-doc=correctness/data 风险]·self-roll YAML 子集可零依赖但同误析风险）·**仅当 R261 真打 Dataview 时才花此硬边界**·届时对着 demonstrated need 授权而非现在投机批。
>
> **roadmap（~4-5 轮·loader 已成故短）**：**R258**=loader 硬化 + 首 committed 真插件证明（vendor Sort&Permute[MIT]为 committed fixture·repeatable desktop-probe + browser-fixture run·载入+执行 2-3 命令+断言**字节级** editor 输出·收紧 manifest/eval 错误报告·**零依赖**）→ **R259**=第二代表(read-time·证 PluginSettingTab + addStatusBarItem + registerInterval/registerDomEvent)→ **R260**=pre-Dataview 无依赖 gap close(adapter.stat[Rust fs::metadata]·editorInfoField/LivePreviewField/EditorField + Editor.getDoc CM6 StateFields·getResourcePath URI 桥)→ **R261(STOP-POINT)**=js-yaml 授权 + faithful parseYaml/stringifyYaml + 试载 Dataview/Templater。
>
> **⚠️ data-safety（R258 = 强制逻辑档·底线①）**：真插件**无沙箱·full Vault 写权**（Sort&Permute 经 `editor.setValue`/`replaceRange` 整文档重写=底线① 热路径）。R258 必守：① **只对 `compat-vault`/calibration fixture 跑·绝不真用户 vault**；② 验插件 `editor.replaceRange` 走与原生编辑同一 autosave-debounce/flush 路径(不旁路关窗 flush)；③ 复核 live-handle-vs-stale-cache offset 陷阱(R223·`sort-list-recursively` 读 metadataCache list offset 再写)；④ 保留显式 community-plugins.json enable gate(用户 opt-in 才跑)；⑤ 字节级断言任何误写在「完成」前即被抓。**触发 data-safety skill 满跑。**

## Round 256 additions — view-header「…」more-options 聚合菜单（Obsidian native tab/view 顶部「更多选项」·tab 栏右缘三点按钮·聚合 视图模式 + 文件操作 + tab/pane 操作·全 wire 既有 vetted 命令）·机械档·零依赖【As-built·v0.250】

> **状态：As-built（已交付·v0.250）。** 用户 R255 拐点拍板「收尾最后 bounded 小件」→ 取本项（autonomous-safe 池最后一件）。**verify-first（ultracode 3-scout workflow·web-native + codebase + reference）= native-confirmed**：Obsidian 每个 view/tab 顶部右缘的「…」more-options 按钮是真 vanilla-core（官方 Tabs help + 版本特定 forum bug 实证·Appearance「Native menus」影响其渲染），由 leaf 的 pane-menu 构建 = tab 右键菜单(pin/split/close) ∪ 文件操作 ∪ 视图模式 toggle。**🔑 pointer 纠误**：HANDOFF/ROADMAP 指的「reference 08:35」其实是**编辑器右键菜单**（reference 08 grep 全树无「更多选项/view-header/聚合菜单」字样·spec 从未截过「…」按钮）→ 但 web 证实功能真 native → **非 R253-A3 式 phantom·是 mislabeled pointer·BUILD**（faithfulness 使命 > reference 覆盖缺口·R222/R223 先例）。
> **设计（机械档·镜像 tab-context-menu / R252 editor 菜单·App.tsx shell 不 import compat）**：
> - **`app/App.tsx` TabBar**：menu state 加 `kind:"tab"|"viewopts"` 判别（右键=tab·「…」按钮=viewopts·共用 dismiss/runMenu 机制·一次只渲一个）；`.tab-bar` 右缘加「…」按钮（`data-testid="view-header-more-<leafId>"`·`more-vertical` 图标[icons.tsx:121 既有]·`margin-left:auto` 推到右缘·无 activeTabId 时 disabled·onClick 从 `getBoundingClientRect` 锚定菜单·按钮在 `.pane` section 内→其 mousedown-capture 先 `setActivePane(leaf)`→wired 命令 target 该 leaf active file）；`cmdMenuItem(id,testid,danger?)` helper（`app.commands.list().find(id)`·missing 或 `available?.()===false` 则 return null[桌面 reveal/open 浏览器隐藏]·label `getCommandName(cmd)`·onClick `runMenu(()=>commands.execute(id))`）；viewopts 菜单块（branch `menu.kind==="viewopts"`·testid `view-options-menu`）。
> - **菜单项**（active tab·gating：markdown-only items on `viewType==="markdown"`·file items on `filePath!==null`·非空组间分隔线）：视图模式 `app:toggle-mode`(md)｜文件 `bookmarks:bookmark-file`·`editor:add-property`(md)·`app:export-pdf`(md)·`file-explorer:copy-path`·`workspace:copy-url`·**`file-explorer:reveal-active-file`(跨端·评审补)**·`file-explorer:reveal-in-system`(桌面)·`file-explorer:open-in-default-app`(桌面)·`workspace:edit-file-title`(重命名)·`app:delete-file`(删除·is-danger)｜tab/pane 直调 workspace `toggleTabPin`·`splitActivePane`(fileless 禁用)·`closeTab`·`closeOtherTabs`。**DEFER**：「Open linked view」子菜单（id 全在·但 Geode 菜单是 flat·子菜单基建是独立轮·drop 不假扁平）。
> - **`core/i18n/dict.app.ts`**：`app.moreOptions`（EN「More options」/ZH「更多选项」·按钮 aria-label·其余项 label 走 `getCommandName` 命令名·tab/pane 项复用既有 app.tabPin/tabUnpin/tabSplitRight/tabSplitDown/tabClose/tabCloseOthers）。
> - **`styles/app.css`**：`.tab-more-options`（镜像 `.tab-new`·`margin-left:auto`）+ `.tab-context-menu button.is-danger { color: var(--danger) }`。
> **分档=机械档（菜单 wiring 既有 vetted 命令·零新写路径·未碰 editor/vault/markdown）→ 跳简化门 → scoped review（ultracode 3-lens：mechanical/faithfulness/regression × 逐 finding 对抗证伪·17 agents）= 1 confirmed + 13 refuted**。confirmed=「Reveal file in navigation」(`file-explorer:reveal-active-file`·跨端·已注册) 遗漏（**浏览器下两个桌面 reveal 经 available() 隐藏→零 reveal 选项**·跨端 reveal-active-file 补上即解）→**已修**(+1 行 + e2e 断言)。13 refuted（all-clear 确认 + 正确降级 nit：pin 排序/toggle-mode 静态名/点按钮重开/version-bump 在收尾·均可接受 flat-menu/机械档 行为)。**测试**：`.calibration/r256-e2e.mjs` **30/30**（按钮在场·菜单开/含全项·**桌面项 reveal-in-system/open-default 浏览器隐藏[available() gate]**·delete is-danger·label 走命令名非裸 key·pin/toggle-mode 真执行+pin label 翻转·与 tab-context-menu 判别·Escape dismiss）·回归 r81 14/14·r131 8/8·r252 20/20·r33 37/37·r229 9/9·typecheck 0·生产构建 ✓·cargo ✓·**桌面 N/A-by-equivalence**（纯前端菜单·所 wire 命令 R218/R252 已桌面验证）。**收尾后 autonomous-safe bounded 池真清空 → 盘点交用户**（剩 boundary-gated：删除去向 parked / Canvas·Bases·真插件迁移 须拍板立项或批依赖）。

## Round 254 additions — Stacked tabs（堆叠标签页·一个 tab 组内同时渲染全部 tab 为横向 cascade·Obsidian native「Stack tabs / Unstack tabs」·原 Andy Matuschak Mode 内化为核心）·逻辑档·data-safety【As-built·R255 实现交付·v0.249】

> **状态：As-built（已交付·v0.249·立项 R254 冻结契约 + 实现 R255）。** 用户 R253 收尾拐点拍板「Stacked tabs 立项(推荐)」（R250 已表态想优先·零新依赖·boundary-gated 集里最适合自主推进者）。**Obsidian 行为（WebSearch 实证）**：Stacked tabs 是 tab 组（=Geode `PaneLeaf`）内的替代视图——组内**全部 tab 同时渲染**为横向并排的列（像桌上扇开的纸张），active tab 展开、其余折叠成窄竖标题条；点条或横向滚动切焦点。每组独立 toggle（tab 组「⌄」菜单「Stack tabs/Unstack tabs」+ 命令 `workspace:toggle-stacked-tabs`）。
>
...
>
> **As-built（R255 实现·v0.249·按契约逐项落地·偏离/补充记录）**：
> 1. **实现 = 加性·零现有字段改动**（契约 100% 落地）：`PaneLeaf.stacked?:boolean`（types.ts）·`workspace.toggleStacked(paneId)`[mapLeaf 翻 flag·镜像 toggleTabPin]·`sanitizeNode` leaf 分支 `stacked: n.stacked === true`[持久化 parity]·App.tsx PaneLeafView 抽 `renderTab(tab,autoFocus)`[既有 6-way viewType dispatch 提取·stacked/非 stacked 共用=减重复非加抽象] + stacked 分支 map 全 tab 渲 `.stacked-column`[header button + body]·命令 `workspace:toggle-stacked-tabs`·tab 右键菜单 Stack/Unstack 项·EditorPane `autoFocus?:boolean`[默认 true·门控 mount focus]·app.css 横向 cascade·i18n 6 键×中英（官方串 Stack/Unstack tabs·堆叠/取消堆叠标签页）。
> 2. **data-safety 坐实（split 先例 + R16 release→flush 继承·非新建面）**：多 EditorPane 同挂 autosave 各自独立（per-DocumentHandle·非 per-pane）·**unstack 卸载非 active 列 → `documents.ts:293-306` `release()`→`queueMicrotask`→`flush().finally(drop)` 先 flush 待落盘的防抖编辑再 drop**[R16 fix·继承]→ 编辑不丢。ultracode 4-lens 对抗评审 **data-safety lens = PASS（0 新丢失路径·5 向量全被 split+R16+report-effect 先例覆盖）**。r255-e2e 实测：双 tab 各自落盘 / 非 active 列待保存编辑 unstack-unmount 仍 flush / 切换不丢 / `stacked` reload round-trip。
> 3. **评审 1 minor 修（focus on activation）**：`autoFocus` 原仅 mount 门控→stacked 下编辑器常驻不重挂·经 header/tab-bar/命令 activate 时 mount focus 不重跑→键入落到上一聚焦列（**非数据丢失·autosave 仍正确路由到各自文件·但键盘优先 UX 缺陷+键入落错可见列**）。**修 = EditorPane 加 `autoFocus` false→true transition focus effect**（modal-guarded R23·非 stacked 下 autoFocus 恒 true 每 mount 故不触发=零回归）+ **header button `onMouseDown preventDefault`**（防 button mouseup 抢回 effect 刚给编辑器的焦点）。r255-e2e +2 断言锁（header-activate 聚焦编辑器·键入落对列）。
> 4. **2 nit（评审确认·不阻断）**：① 注释误称 `setActiveTab + scrolls`（实无 scrollIntoView）→已改正注释；② tab-drop overlay 在 stacked 横向滚动 + 拖拽时几何错位（`.pane-drop-overlay inset:0` 锚 scroll origin·niche·须 stacked+足够列溢出+预滚动+拖拽·零数据安全影响·tab 重排走独立 .tab-bar handler 不受影响）= **已知 v1 限制·记档不修**。
> 5. **9 refuted**（data-safety PASS·off-screen-active 不滚=minor 已转 nit·inactive 列全宽非折叠 spine=可接受表示·TabBar+列 header 双标题=忠实前提错·菜单/串忠实·remount 清 undo=与既有切 tab 一致非缺陷[且 undo 存 handle historyHost]·header 双 dispatch=无害·empty/single stacked=benign v1）。**简化门 clean**（renderTab 提取=减重复）。**测试**：r255-e2e **20/20**·回归 r81 14/14·r39 17/17·r88 13/13·r232 10/10·r33 37/37[字节锁]·r51 10/10·r229 9/9 + 生产构建 ✓ + cargo ✓·桌面 N/A-by-equivalence。**教训**：可点击 header 激活元素若需把焦点交给别处（编辑器），button 默认 mousedown 抢焦必 `preventDefault`，否则 effect/程序化 focus 被 mouseup 夺回（[[geode-keymap-stack-bare-key-shadow]] 同类「默认行为悄悄夺权」）。
...
> **设计（逐字镜像 R232 智能列表 compartment-toggle·data-safety 🟢[Vim 编辑仍走 CM dispatch→docChanged→autosave updateListener 顺序无关·零影响]）**：
> - **`core/appearance.ts`**：`vimMode` Store（默认 **OFF**·`geode.vimMode`·镜像 R226 rightToLeft·`readBool`/`persistBool`）+ `setVimMode`。
> - **`features/editor/cmExtensions.ts`**：`vimExtension(on) = on ? vim() : []`（镜像 `smartListExtension`）+ `vimCompartment` 声明（签名）+ **基列表注入**（`vimCompartment.of(vimExtension(vimMode.get()))`·**放基列表非 modeCompartment·live↔source 切换不销毁**·位置在 R33 `Prec.highest` 命令拦截之后、defaultKeymap 之前/附近·实测热键不冲突）。
> - **`features/editor/EditorPane.tsx`**：`vimCompartmentRef` + `new Compartment()` + cleanup null（镜像 smartListCompartment）+ `const vim = useStore(vimMode)` 订阅 + reconfigure effect（`view.dispatch({effects: compartment.reconfigure(vimExtension(vim))})`·deps `[vim]`）。
> - **`features/settings/SettingsModal.tsx` EditorSection「行为」组**（R247 分组·Vim 属行为）：Vim toggle（`data-testid="settings-vim-mode-toggle"`·镜像 R226 RTL toggle·smartLists 旁）。
> - **`core/i18n/dict.views.ts`**：`settings.vimMode`（EN「Vim key bindings」/ZH「Vim 键位」）+ `settings.vimModeDesc`（「在编辑器中使用 Vim 键位绑定。」）×中英。
> - **`:w`→flush**（faithful·Obsidian Vim :w 保存）：`Vim.defineEx("write","w", …)` 调既有 vetted `documents.flush()`（不丢编辑·裸 :w 否则 no-op）·若 handle 接入复杂则降级 autosave 兜底（记一句）。
> **分档=逻辑档（碰 editor 管线·新依赖·底线①一票否决）→ data-safety skill 满跑**（重点：① Vim 编辑走 CM dispatch→docChanged→autosave/flush/document:changed 零影响[updateListener 顺序无关·documents.ts:85]·② Escape 路由[modal 开→关 modal·无 modal→vim insert→normal·须实测命令层不吞 Escape]·③ live↔source 存活[vimCompartment 基列表]·④ 多光标/history[u/Ctrl-r 走 CM history·dd/p 触发 autosave]·⑤ 关窗 flush/重命名 retarget vim 编辑期不丢）→ 简化门 → 多维对抗评审。**测试**：`.calibration/r253-e2e.mjs`（vimMode toggle 默认 OFF·持久化·**ON→编辑器入 normal 模式[typing 不插入·i 入 insert·Esc 退出]**·**vim 编辑[i 插字 Esc]触发 autosave→文件保存[底线① 核心]**·dd 删行触发 save·live↔source 切换 vim 存活·OFF→普通编辑）+ data-safety 套件 + 回归 r232/r226/r225 编辑器 toggle 不退。**桌面 probe**：vim 纯前端 CM6·跨端同·浏览器全覆盖。**下一片**：A 系列 + Vim 完成 → 候选池 autonomous-safe 实质见底（剩 boundary-gated 大件 / 删除去向 parked）→ 届时盘点交用户。
>
> **As-built 修订（实测/评审后·偏离冻结契约处·根因记录）**：
> 1. **Escape 困死 insert 模式 — 根因修复（实测发现·底线① 边缘）**：`vimExtension(on) = on ? **Prec.high(vim())** : []`（非裸 `vim()`）。裸 `vim()` 时 defaultKeymap 的 `Escape→simplifySelection`（R63 多光标）**抢过** vim 的 insert→normal Escape，把用户困在 insert 模式（编辑可继续但退不出·伪正常）。r253-e2e 调试探针确认（i/Z/Esc 后仍 "insert"）。`Prec.high` 让 vim keymap 压过 defaultKeymap；**仍在 R33 `Prec.highest` 命令拦截之下**（Cmd+P 等热键仍赢）。**教训**：往既有 keymap 栈插新 keymap，必实测 Escape/Enter 等被 defaultKeymap 占用的裸键，Prec 不显式抬高即被既有绑定 shadow。
> 2. **`:w` 降级 autosave 兜底（契约「记一句」已授权·未接 defineEx）**：codemirror-vim 6.3.0 的 `write` handler 需 `CM.commands.save`/`cm.save`（Geode 二者均 `undefined`）→ 裸 `:w`/`:wq` 静默 no-op（**不抛错·不破坏编辑器**）。**数据安全无损**：vim 编辑走标准 CM dispatch→docChanged→autosave updateListener（documents.ts·无 userEvent/composing 门控），r253 实测 i+Esc 编辑落盘。Obsidian 本身亦持续 autosave，:w 无独立保存反馈不算缺失。
> 3. **toggle 放置修正（对抗评审 1 confirmed·faithfulness minor）**：冻结契约写「行为组·Vim 属行为」**有误** → `reference/01-编辑器.md:42-47` 明确 Vim 模式属 Editor**「高级（Advanced）」**（行为组列智能列表·不含 Vim）。EditorSection 原无 Advanced subheader（既有 `settings-subheader-advanced` 在 **FilesAndLinksSection**·非 Editor·评审误判可复用）→ **新增 Editor 专属** `<h3 data-testid="settings-subheader-editor-advanced">`（复用 `settings.subheaderAdvanced` i18n 键[标签同「高级」]·testid 与 FilesAndLinks 区分免重复），Vim toggle 移入其下。**教训**：保真放置须核 reference 的**具体 section/subsection 归属**，不能凭「某处已有同名 subheader」就放——它可能在另一个设置页（[[geode-visual-polish-verify-and-per-role]]）。
> 4. **ZH 标签对齐官方（评审 surface·obsidian-translations 权威源）**：「Vim 键位」→**「Vim 模式」**、desc「在编辑器中使用 Vim 键位绑定。」→**「允许在编辑器中使用 Vim 模式。」**（obsidianmd/obsidian-translations `zh.txt` key `setting.editor.option-vim-key-bindings`）。EN「Vim key bindings」保持（官方原文）。
> 5. **对抗评审（ultracode 4-lens·13 agents）**：data-safety/faithfulness/edge-cases/regression × 逐 finding 对抗证伪 = **1 confirmed**（toggle 放置·已修[#3]）+ **8 refuted**（data-safety all-clear·R33 precedence faithful-by-design[Obsidian 同样 app 热键压 vim]·`:w` autosave-fallback 契约已授权·static-import bundle nit·无视图指示器=Obsidian native 默认亦无·均正确驳回）。**CM6 单份 deduped**（`@codemirror/state@6.6.0`·`@codemirror/view@6.43.1`·vim peer 全 `^6`·无 instanceof 破坏）。
> 6. **简化门 clean**（vim 逐字镜像 R232·无可减面）；附带重接被 vimExtension 插入孤立的 R232 smartList JSDoc（各注释贴各函数·净零行为）。

## Round 252 additions — A2 编辑器右键菜单加 file-action 项（收藏/属性/导出/复制/系统操作/重命名/删除·全 wire 既有 vetted 命令·对齐 Obsidian reference 08 编辑器菜单）·逻辑档·零新依赖【契约冻结 v0.247】

> **状态：As-built（已交付·v0.247）。** **背景**：编辑器右键菜单（`compat/obsidian/editorMenu.ts` `editorContextMenuExtension`·CM6 contextmenu·经 R115 core registry 注入·R200 已加原生 Cut/Copy/Paste）现只有剪贴板项·缺 reference 08 的 file-action 项。**A2 = 给编辑器右键补 file-action 项·全是对【既有 vetted 命令】的菜单 wiring**（`handle.commands.execute(id)`·命令内部读 `workspace.getActiveFile()`·无需传 path）。**autonomous-safe（纯菜单接线·零新写路径）·底线①：删除走既有 `app:delete-file`[R242 deleteConfirm+flush+trash+R244 孤儿]·重命名走既有 `workspace:edit-file-title`[R16]·绝不另起删除/重命名路径**。
> **设计（1 文件主体 + context.ts 1 行·非数据安全[复用 vetted 命令·editorMenu.ts 不写 vault·只 execute 既有命令]·分层 OK[editorMenu.ts 属 compat·可 import core]）**：
> - **`compat/obsidian/editorMenu.ts`**：新 `addFileActionItems(menu, commands: CommandRegistry)`——3 组（组间 `addSeparator()`·**仅非空组才加分隔线·防 desktop 项隐藏时悬空分隔**）：① 收藏[`bookmarks:bookmark-file`·label getCommandName 自动翻收藏/取消] + 增加属性[`editor:add-property`] + 导出 PDF[`app:export-pdf`] + 复制库内路径[`file-explorer:copy-path`] + 复制 Obsidian URL[`workspace:copy-url`]；② 在系统中显示[`file-explorer:reveal-in-system`] + 在默认应用打开[`file-explorer:open-in-default-app`]（桌面·**经 `cmd.available?.() !== false` 门控**[reveal/open 的 available=isTauri·浏览器自动隐藏·因 `execute()` 不查 available 故 build 时按 available 过滤]）；③ 重命名[`workspace:edit-file-title`] + 删除[`app:delete-file`·`setWarning(true)` 危险样式]。每项 `setTitle(getCommandName(cmd)).onClick(()=>commands.execute(id))`·**icon-less**（与既有 clipboard 项一致·compat BUILTIN 图标集仅 11 个不全·delete 用 setWarning 标危险）。`editorContextMenuExtension(workspace, commands)` 在 `addClipboardItems` 后调 `addFileActionItems`·`nativeCount` 取两者之后（插件项仍在下方）。
> - **`compat/obsidian/context.ts:240`**：`editorContextMenuExtension(workspace, handle.commands)`（`handle` = context 函数参数·在作用域内·`handle.commands` = CommandRegistry）。
> - **i18n 无需新增**（label 复用命令名 getCommandName）。**deviation 记一句**：复制库内路径 + 在系统中显示 是 reference 08 文件树菜单项·本片就近并入编辑器菜单（与 Obsidian 新版编辑器菜单含 copy-link/reveal 一致·useful·全 vetted 命令）。
> **分档=逻辑档（新菜单项 + 控制流 wiring 删除/重命名·虽复用 vetted 命令但碰 editor 右键 + 删除/重命名关联）→ data-safety skill（确认 删除走 vetted app:delete-file 无旁路·重命名走 R16·菜单不写 vault）→ 简化门 → 多维对抗评审（重点：① 删除/重命名 wire 到 vetted 命令无另起路径·② available 门控正确[桌面项浏览器隐藏·active-file 项菜单仅在 activeEditor 弹]·③ 分隔线非空组才加无悬空·④ bookmark label 翻转·⑤ 命令不存在防御 skip）**。**测试**：`.calibration/r252-e2e.mjs`（镜像 r200·编辑器右键弹菜单·file-action 项在 clipboard 后·收藏/重命名/删除等项存在·点删除项→走 app:delete-file[deleteConfirm dialog]·桌面项浏览器不现·分隔线无悬空）+ 回归 r200[编辑器右键 clipboard]/r242[删除确认]/r131 不退。**桌面 probe**：reveal/open = R218 桌面 vetted·浏览器 e2e 断 available 隐藏。
> **As-built（v0.247·根因记录·ultracode 4-lens 评审）**：**简化门 clean**（菜单 wiring helper + 常量单一内聚·无可简化面）。**Step 4 用 Workflow 4-lens 并行对抗评审（data-safety/faithfulness/edgecases/regression·每 finding 对抗验证）= 0 confirmed 缺陷**——**data-safety lens 坐实底线①**：删除项 onClick=`execute("app:delete-file")` = R242 deleteConfirm gate + R244 孤儿 + flushAll + recoverable trash（editorMenu.ts 零 vault 写·无另起删除路径）·重命名=`execute("workspace:edit-file-title")`→R16·菜单零 .md 字节写。唯一 finding（命令名当 label 偏长 vs Obsidian 短标签）经对抗验证**证伪**（契约冻结 getCommandName 复用·避免新 i18n 键 + 自动翻转收藏 label·nit 非 must-fix）。r252-e2e **20/20**（file-action 项在 clipboard 后·桌面项浏览器隐藏[available 门控]·删除走 deleteConfirm dialog→recoverable trash·收藏 label 翻转·分隔线无悬空）·**回归 r200 21/21·r131 8/8**（stale 断言更新=正当·行为确实加了 file-actions·新断言 first-3-clipboard + plugin-LAST 更精确非掩盖）·r242 11/11·typecheck 0·截图实证（删除项红/危险·分组分隔）。**已知 minor 非缺陷**：菜单 label 用命令名（偏长·DRY tradeoff）·commands.list() 每次右键排序（非热路径·trivial）。**下一片**：A3 默认打开文件 → Vim 模式（codemirror-vim 已授权）。

## Round 251 additions — A1 热键页保真：命令名加来源前缀（插件命令「书签: xxx」·核心命令无前缀·扁平非分组·对齐 Obsidian reference 04 热键-01）·逻辑档·零新依赖【契约冻结 v0.246】

> **状态：As-built（已交付·v0.246）。** **背景（用户 R250 拍板续 bounded）+ verify-first 纠误**：Obsidian 热键页是**扁平列表**（**非分组 section**·热键-01 截图实证），命令名带**来源前缀**——核心插件命令显示「源: 命令名」（如「书签: 收藏当前搜索」「白板: 导出图片」），核心 app/editor/workspace 命令无前缀。Geode `HotkeysSection`(`SettingsModal.tsx:1949`) 现扁平**无来源前缀**。**绝不造分组 section（phantom·防 R145/R242 教训复发）**——只加名字前缀。Geode `Command`(`types.ts`) 无 `source` 字段 → 按 **id 前缀**派生来源（核心前缀 `editor`/`app`/`workspace`/`file-explorer` 无前缀·9 个插件前缀 `bookmarks`/`daily-note`/`backlink`/`unique-note`/`slides`/`random-note`/`outline`/`outgoing-links`/`graph` 加本地化「源: 」）。**已知局限（记一句）**：Geode 把笔记重组等部分核心插件命令注册在 `editor:`/`app:` 前缀下（非 `note-composer:`）→ 那些命令保持无前缀（少数·宁缺勿错·不误前缀）。
> **设计（hotkeys 页局部前缀·非改 getCommandName[避免波及命令面板/搜索]·逻辑档[新映射+控制流]·非数据安全[纯设置 UI·零 .md 写]）**：
> - **`features/settings/SettingsModal.tsx`**：模块级 `CMD_SOURCE_KEYS: Record<string, I18nKey>`（9 插件前缀→i18n 键·核心前缀不在表内→无前缀）。HotkeysSection 内 `cmdLabel(cmd)`：取 `name = getCommandName(cmd)`·核心/未知前缀返 name 原样·否则 `label = t(key)`·**幂等守卫**：`name.startsWith(label+":") || name.startsWith(label+"：")`（ASCII + 全角冒号）则返 name 原样[防双前缀]·否则 `label+": "+name`。用于 **行名显示 + 冲突名 + reset aria-label + 过滤匹配**（过滤含前缀→可按「书签」搜）。
> **As-built（v0.246·根因记录·ultracode 多维评审）**：**简化门 clean**（来源前缀机制单一·无可简化面）。**Step 4 用 Workflow 4-lens 并行对抗评审（faithfulness/i18n/edgecases/regression·每 finding 对抗验证）**——i18n/regression lens 0 缺陷·**揪出 1 confirmed major（faithfulness + edgecases 两 lens 独立发现·runtime 实证）**：`backlink:`/`outline:`/`outgoing-links:` 的 4 个命令注册名**已含来源标签**（R211/R212/R219 为命令面板加的「Backlinks: …」·ZH 全角「反向链接：…」），其 id 前缀又在 CMD_SOURCE_KEYS → 初版 `commandSourcePrefix(id,t)+getCommandName` **双前缀**（「Backlinks: Backlinks: …」·ZH 混合冒号更糟）·我的初版 r251-e2e 只测干净命令[bookmarks:/graph:] 漏掉这族。**修=幂等守卫**（折叠 commandSourcePrefix 进 cmdLabel·名字已以来源标签开头则不再加·**净删一个 helper = 顺带简化**）·r251-e2e +4 断言锁 backlink:/outline:/outgoing-links: 单前缀。**colon-width nit 被对抗验证证伪**（ASCII「: 」是忠实选择·reference 04 半角·全角是既有 deviation）。r251-e2e **14/14**·回归 r145 15/15[热键]·r142 18/18[命令面板]·r23 22/22[设置]·typecheck 0·桌面 N/A。**教训**：加【派生前缀/变换】前必查【已存在的 baked-in 版本】做幂等·测【碰撞用例】（id 前缀在表 ∧ 名字已前缀）而非只测干净用例——多维 lens 评审揪出单 lens/单 e2e 会漏的此类。
> - **`core/i18n/dict.views.ts`**：`cmdSource.bookmarks`(书签/Bookmarks)·`cmdSource.dailyNotes`(日记/Daily notes)·`cmdSource.backlinks`(反向链接/Backlinks)·`cmdSource.uniqueNotes`(唯一笔记创建器/Unique note creator)·`cmdSource.slides`(幻灯片/Slides)·`cmdSource.randomNote`(随机笔记/Random note)·`cmdSource.outline`(大纲/Outline)·`cmdSource.outgoingLinks`(出链/Outgoing links)·`cmdSource.graph`(关系图谱/Graph view)×中英。
> **分档=逻辑档（新映射 + 控制流[prefix 派生] + 过滤逻辑改·但非数据安全[纯设置 UI·零 vault/editor/markdown]）→ 简化门 → scoped/多维评审（重点：① 前缀映射只覆盖真插件前缀·核心命令无前缀[faithful]·② 笔记重组 editor: 局限记档·③ 过滤含前缀仍可按 id/名搜·④ i18n 双语真实·⑤ 不造分组 phantom）**。**测试**：`.calibration/r251-e2e.mjs`（插件命令[bookmarks:/graph:/...]行名带「源: 」前缀·核心命令[editor:/app:]无前缀·过滤「书签」命中 bookmarks 命令·i18n 文案·**不存在分组 section header**[非 phantom]）+ 回归 r145 热键套件不退。**桌面 probe N/A**（纯设置 UI·跨端同）。**下一片**：A2 编辑器 view-header「…」+ 右键 file-action → A3 默认打开文件 → **Vim 模式（codemirror-vim 已授权）**。
...
> **设计（1 CSS 规则改 2 属性·纯视觉·非数据安全[不碰 .md/vault/editor]）**：`features/settings/settings.css` `.settings-nav-item.is-active`：`background: var(--accent-muted)` → `var(--bg-active)`（= `--background-modifier-active-hover`·Obsidian 设置 nav 选中所用同名变量·浅 `rgba(0,0,0,.09)`/深 `rgba(255,255,255,.1)`）·`color: var(--accent)` → `var(--text-normal)`（图标走 currentColor 一并转中性）。**审计 settings.css 全部 `--accent` 用处**：toggle ON / slider thumb / 主按钮(update-install) / focus 边框 / 控件 active 态(funnel filter·capturing·segmented) 均**正确保留 accent**（= §四 accent 语义）·**唯独 nav 选中项是 nav 选择态该中性** → 仅改此一处。
> **分档=机械档（diff = 单文件 settings.css 2 属性·纯样式·无新逻辑/无新契约/未碰数据安全面）→ 简化门机械档跳过 → scoped review（窄域：CSS 变量真实存在[`--text-normal` 在 app.css 双主题·`--bg-active` 在 app.css + theme-bridge.css:125 映射 Obsidian `--background-modifier-active-hover`]·选中态 vs hover 态层叠正确[同特异性源序后者胜]·无其他 accent 误伤）**。**测试**：`.calibration/r245-e2e.mjs`（computed-style：active nav 文字 === `--text-normal` 且 !== `--accent`·active nav bg === `--bg-active`[中性] 且非 accent-tinted·探针元素解析变量为 rgb 精确比对）+ 截图留证。**桌面 probe N/A**（纯 CSS·跨端同样式·浏览器全覆盖）。**下一片 G2-b backlog**：内容区分组 section 子标题（界面/字体 等·reference §三「分组 section header 灰色小标题」·Obsidian 页内分组·Geode 当前扁平）= 结构档[需 SettingsModal JSX 改]·单独排。

## Round 244 additions — 回收站组余 slice 2a「删除附件处理」下拉（Obsidian native Files&Links·删除文件时删除附件·reference 02:30·默认「每次都询问」）·删笔记时按设置 询问/一并删/保留 其【独占引用】的附件·走 recoverable `vault.trash`·**绝不自动永久删**·零新依赖【契约冻结 v0.240·data-safety 最高危轮·底线①】

> **状态：As-built（已交付·v0.240）。** **背景**：R243 已把 markdown 嵌入纳入索引、并显式记下 getAttachmentMap 的 **frontmatter 盲区**（parseNote blank 整段 frontmatter → frontmatter `[[att]]` 引用不进 meta.links → getAttachmentMap 是引用**下界非完整集**）。R244 = 在此之上做删笔记联动删孤儿附件。**忠实性（reference 02:30 ground-truth 截图确认 native·默认「每次都询问」；官方 help 页 404 → 次级选项标签从语义忠实推定·记 deviation）**：删一条笔记时，对【仅被该笔记引用】的附件（删后即孤儿），按设置三档处理：`ask`「每次都询问」[默认·弹清单二次确认]/`delete`「一并删除」[直接 trash 孤儿]/`keep`「保留」[现状·不动附件]。
> **🛑 data-safety crux（R243 评审降级的前置约束·本轮兑现）**：孤儿判定**绝不能只信 getAttachmentMap**（漏 frontmatter）——否则别的笔记 frontmatter `cover: "[[pic.png]]"` 引用的附件会被误判孤儿→误删=破坏别的笔记（违底线①）。**解法 = 在 core 侧补一个跨「正文 + frontmatter」的完整引用查询**（不 widen getAttachmentMap 本体·避免动 graph「Attachments」消费者[R231 教训]）+ **纵深防御**：默认 `ask` 把孤儿清单摆给用户二次确认·删除一律走 recoverable `vault.trash`（永久删属 R245 硬边界·本轮绝不碰）。
>
> **冻结契约（跨模块签名）**：
> - **`core/metadata.ts`（MetadataIndex 新方法·data-safety）**：`getOrphanedAttachments(deletedNotePaths: Set<string>): string[]` —— 返回【完整引用集】⊆ deletedNotePaths 且非空的附件路径（= 删掉这些笔记后会变孤儿的附件）。**完整引用集 = 正文（复用 `getAttachmentMap()`）∪ frontmatter `[[att]]`（新 core 侧扫描）**。frontmatter 扫描：遍历 `byPath` 每条 `meta.frontmatter?.fields`（`Record<string,string|string[]>`·string 直扫·string[] 逐元素扫），**复用既有 `metadata.ts:20` 的 `WIKILINK_RE`**（与 compat metadata.ts:273 的 FM_WIKILINK_RE 功能等价·复用 = 单一真源·正文/frontmatter 的 `[[]]` 解析逐字一致·**core 不 import compat**，分层铁律）抽 `[[target]]`（m[1] 去 `#sub`/`|alias`），`resolveAttachment(target, meta.path)` 命中且 `!byPath.has(att)`（排除笔记·只算附件）即记 meta.path 为 referrer。裸串 `image: pic.png` **不计**（忠实 Obsidian：property link 仅识别 `[[]]`）。不缓存（删除低频·按需算）。
> - **🛑 覆盖边界（评审记 deviation·纵深防御兜底·非回退）**：① **仅识别 canonical 引号式 frontmatter 链接 `k: "[[x]]"`**——未加引号 `k: [[x]]` 被 core 极简 frontmatter parser 误读为内联表 `["[x]"]`（无 `[[`→不计）·**与 Obsidian（YAML 把 `[[x]]` 解析成嵌套 flow-seq·叶子裸 `x`·亦不识别）+ compat R126 既有限制一致**（compat metadata.ts:281 早有此 docstring）·非 R244 引入。② 孤儿判定读 flush **之前**的索引——某存活编辑器里**尚未自动保存**的新 `[[att]]` 引用看不见 → 该 att 可能被误判孤儿 trash（**可恢复·极端窗口·Obsidian 类似**）。**两条都由纵深防御兜底：默认 `ask` 把孤儿清单摆给用户二次确认 + 删除一律 recoverable `.trash`（可从回收站恢复·永久删属 R245 硬边界）**——即便漏看一条引用，用户能在对话框拦下或事后从 `.trash` 恢复。
> - **`core/appearance.ts`（enum Store·镜像 R234 extractReplaceMode）**：`type AttachmentDeleteMode = "ask" | "delete" | "keep"`·`attachmentDeleteMode = new Store<AttachmentDeleteMode>(...)` 默认 **`"ask"`**（忠实 Obsidian 默认「每次都询问」·键 `geode.attachmentDeleteMode`·校验 `v==="delete"||v==="keep"?v:"ask"`·`persistString`）·`setAttachmentDeleteMode(mode)`。
> - **`core/attachmentDeletion.ts`（新文件·删除编排助手·3 删除点共用[rule-of-three]·core 可 import confirm + i18n + metadata 类型）**：`async resolveAttachmentDeletion(metadata: MetadataIndex, deletedNotePaths: Set<string>, deletedRoots: string[], mode: AttachmentDeleteMode): Promise<string[]>` —— `mode==="keep"` 返 `[]`；否则 `orphans = metadata.getOrphanedAttachments(deletedNotePaths)`，**过滤掉位于任一 deletedRoot 之内的附件**（`a===r || a.startsWith(r+"/")`·避免与文件夹一并 trash 时双删）；`orphans` 空返 `[]`；`mode==="ask"` 弹 `confirmAction`（消息列孤儿清单·declined 返 `[]`）；否则（`delete` 或 ask-confirmed）返 orphans。**纯计算 + 一次 confirm·不写 vault**（trash 由 caller 控 flush 顺序）。
> - **3 删除点接线（data-safety·复用 vetted gate→flush→trash 序·R242 镜像）**：每点序变为 `deleteConfirm gate（R242 不变）→ const orphans = await resolveAttachmentDeletion(...)（R244·ask 在 flush 前）→ flushAll → vault.trash(note)（不变）→ for(att of orphans) vault.trash(att)（R244·孤儿在笔记后 trash·走 vetted recoverable .trash）`：
>   - `app/App.tsx` `app:delete-file`：单笔记·`deletedNotePaths = path.endsWith(".md") ? {path} : {}`·`deletedRoots=[path]`。
>   - `features/explorer/Explorer.tsx` `deleteNode`：文件→`{path}`（非 .md 则空·删附件本身不级联）·文件夹→`vault.getMarkdownFiles().filter(under folder)`·`deletedRoots=[node.path]`。
>   - `features/explorer/Explorer.tsx` `bulkDelete`：`deletedNotePaths` = 各 root 展开（文件夹→其下 .md·.md 文件→自身）之并·`deletedRoots=roots`·flushAll 仍循环外一次。
> - **`features/settings/SettingsModal.tsx`**：`FilesAndLinksSection` 删除确认 toggle 旁加「删除附件处理」下拉（`data-testid="settings-attachment-delete-mode"`·镜像 R234 extract select·绑 attachmentDeleteMode）。
> - **i18n**：`dict.views.ts` `settings.attachmentDelete`/`Desc` + `settings.attachmentDeleteAsk`/`Delete`/`Keep`（EN+ZH）；`dict.panels.ts` `explorer.attachmentDeleteTitle` + `explorer.attachmentDeletePrompt`（含 count + 清单·EN+ZH）。
> **分档=逻辑档（碰 core/metadata.ts + vault 删除写路径·改写多文件删除·底线①最高危）→ data-safety skill 满跑 + 多维对抗评审**（重点：① 孤儿判定含 frontmatter[别的笔记 frontmatter 引用的附件**绝不**被误删]·② 删除一律 recoverable trash·绝无永久删·③ 文件夹/批删 deletedRoots 过滤防双删·④ flush-before-trash 序不破[R242 教训]·⑤ ask declined→不删附件·⑥ 默认 ask 纵深防御）。**测试**：`.calibration/r244-e2e.mjs`（孤儿被删[ask accept / delete 模式]·非孤儿[别处正文引用 + **别处 frontmatter 引用**]保留·ask declined 保留·keep 模式不动·附件走 .trash 可恢复·文件夹删不双删）+ 回归 r242[删除确认]/r101[attachment map]/r126[frontmatter links]/r42[trash] 不退。**R245 = 回收站组余 slice 2b：删除文件去向 三档**（系统回收站/.trash/永久·**🛑 永久删除分支不可恢复=底线①最高危·实现前必再向用户确认·系统回收站若需 Rust=硬边界#5**）。
> **As-built（v0.240·根因记录）**：**简化门 clean**（3 删除点接线 similar-not-identical[2nd-use guard 禁合并]·单一数据安全算法·frozen-contract 方法·单调用点薄接线——无可简化面）。**多维对抗评审 0 crit/0 major·data-safety 四性坐实**（① 别的笔记正文/frontmatter 引用的附件绝不被误删[完整 referrer 集 = getAttachmentMap ∪ frontmatter `[[]]`·`!byPath.has` 排除笔记]·② 全程 recoverable trash·枚举全仓 5 个 trash/remove 点确认 R244 路径无永久删[`vault.remove` 仅底层原语/compat shim/卸插件目录·R244 不触达]·③ flush-before-trash 序不丢编辑[ask 在 flush 前同 R242 precedent]·④ 默认 ask 不破坏既有删除套件[orphans 空先短路 return·r242/r140/r161 删的笔记无附件→不弹]）。评审 4 minor 处理：① **评审驱动 code 硬化**——`deleteNode` + `app:delete-file` 的孤儿 trash 循环加 per-att try/catch（镜像 bulkDelete·一个孤儿 trash 失败不中止其余·笔记已安全 trash）；②③ 覆盖边界 deviation 已上记（见本节 🛑 覆盖边界·未加引号 frontmatter / 未保存编辑·纵深防御兜底）；④ 契约文本对齐实现（WIKILINK_RE 复用·见上）。**测试**：`.calibration/r244-e2e.mjs` **29/29**（getOrphanedAttachments 正文孤儿/**frontmatter 保护[getAttachmentMap-alone 会误删]**/裸串非引用/多笔记/无 .md 误判·resolveAttachmentDeletion keep-空/delete/**文件夹双删过滤**·集成 app:delete-file delete/keep/ask-accept/ask-dismiss/**frontmatter 集成保护**·`.trash` 可恢复实证）·回归 r242 11/11·r101 23/23·r126 11/11·r42 17/17·r243 11/11·r70 23/23·r33 37/37·typecheck 0·cargo check 通过（无 Rust delta）。**桌面 probe N/A-by-equivalence**（孤儿检测纯 TS 平台无关·trash 写 = R42/R161 FS-vetted·零 Rust delta·`__geodeResolveAttachmentDeletion` 浏览器 probe 全覆盖编排）·**r18-diff N/A**（不碰 markdown.ts·零 .md 字节写[trash 移动整文件非内容写]）。

## Round 243 additions — 索引 markdown 图片嵌入 `![](x)`（回收站组余 2 的 data-safety 前置·用户拍板 Option 1）·修复重命名 markdown 嵌入不改写的既有 bug + 解锁准确孤儿判定·零新依赖【契约冻结 v0.239】

> **状态：As-built（已交付·v0.239）。** **背景（用户 R243 AskUserQuestion 拍板 Option 1）**：R243 原定「删除附件处理」(回收站组余 slice 2a)·Step 0 发现 data-safety 阻断——孤儿判定需「准确判断附件是否还被别处引用」，但 `parseNote`（metadata.ts:250）**显式跳过 markdown 图片嵌入 `![](x)`**（`if (m[1]==="!") continue;`·㉞ 延后项）→ `meta.links` 不含它们 → `getAttachmentMap` 漏判 → 误删仍被 `![](x)` 引用的附件=破坏别的笔记。**用户拍板：先补 markdown 嵌入索引（顺带修复 rename 不改写 + 让孤儿判定准确），之后再做删除附件处理。**（注：**wikilink 嵌入 `![[x]]` 早已索引**·只 markdown 嵌入 `![](x)` 被跳过。）
> **设计（1 处 parse 改·span 排除 `!` 使 R16 改写零变·非新逻辑只补覆盖）·data-safety[碰 metadata + R16 rename·改 .md 字节面]**：
> - **`core/metadata.ts`**（parseNote MARKDOWN_LINK_RE 循环 :249-261）：删 `if (m[1]==="!") continue;`·markdown 嵌入也 push 进 `links`（kind `"markdown"`·与 markdown 链接同型）·**关键：embed 的 LinkRef `from = m.index!+1`（排除前导 `!`）**·`to` 不变。这样 `content.slice(from,to)="[text](href)"`（不含 `!`）→ R16 改写的校验正则 `^\[…\)$`（linkRewrite.ts:321）与重建 `[text](newhref)` **逐字不变即可处理**·`!` 在 span 外永不被触碰 → 嵌入语义自动保留（`![](old)`→`![](new)`·非降级为链接）。**makeSnippet 仍用全 match 起点**（仅显示）。`href` 的 external/anchor/`//` 过滤（:252）保留。
> - **R16 rename 引擎（linkRewrite.ts）零改**：embed 经 span 排除 `!` 后与 markdown 链接走同一 step-4 分支（`stillResolvesMd`/校验/重建）·**不动改写算法**。
> - **下游消费者全验**（[[geode-collection-widen-recheck-consumers]]·meta.links 多消费）：`getBacklinks`(:751)·`getOutgoingLinks`(:768)·`getGraph`(:936)·`getAttachmentMap`(:825·**目标**)·`unlinkedMentions`(:144 blank span) **全部正确改善**（嵌入引用计入反链/出链/图谱/孤儿映射·unlinkedMentions blank 嵌入 span 防 alt 文本误判）。**评审补漏（widen-recheck 教训·初版漏了 compat 消费者）**：`compat/obsidian/metadata.ts` buildCache(:538) 的 embed 分桶门原 gate 死写 `kind==="wikilink"` → markdown 嵌入会误落 `cache.links`（丢 `!`·position 偏 1）·**已修为 `content[l.from-1]==="!"` 不限 kind**（非嵌入 markdown 链接的 `[` 前不可能是 `!`，否则正则早吞进 `(!?)`）→ `getFileCache().embeds` 正确含 md 嵌入·`!`-inclusive original·r119 加 md 嵌入分桶断言锁回归（15/15）。
> **分档=逻辑档（碰 core/metadata.ts + 影响 R16 rename·改 .md 写字节面·数据安全红线）→ data-safety skill 满跑（含 r18-diff 字节）→ 多维对抗（重点：rename 嵌入 byte 正确[`!` 保留·非降级]·所有 meta.links 消费者不退·孤儿映射现含嵌入）**。**桌面 probe**：rename 写 = R16 vetted·R243 仅扩覆盖经 span 排除 `!`·byte 经 r33/r16 套件 + 新嵌入 rename 测验证·浏览器全覆盖。**测试**：`.calibration/r243-e2e.mjs`（① 含 `![](att.png)` 的笔记·rename att.png → 嵌入改写为 `![](new.png)`[**byte 精确·`!` 保留**]·② getAttachmentMap 现含 markdown 嵌入引用[孤儿判定准确]·③ 反链/出链含嵌入·④ 嵌入 alt 文本不被 unlinkedMentions 误判）+ 回归 r16/r33[字节锁]/r70[markdown 链接]/r101[attachment map]/r24[unlinked] 不退。
> **⚠️ getAttachmentMap 覆盖范围（评审降级措辞·R244 前置约束·防误删）**：R243 后 getAttachmentMap 覆盖**正文** wikilink 嵌入 `![[x]]` + wikilink `[[x]]` + markdown 链接 `[t](x)` + markdown 嵌入 `![](x)`——但 **parseNote 在扫链接前把 frontmatter 整段 blank（metadata.ts:226 `withoutFm`）→ frontmatter/property 里的附件引用（如 `cover: "[[pic.png]]"`）永不进 `meta.links` → getAttachmentMap 不计入**（compat 另有独立 `buildFrontmatterLinks`(:283·R126) 抽 frontmatter wikilink·证明该类引用真实存在且 core 索引未覆盖）。故 getAttachmentMap 是引用的**下界（lower bound）而非完整集**。**R244 = 删除附件处理**：销毁前**必须**补 frontmatter/property 引用覆盖**或**把 getAttachmentMap 当下界 + 删除前二次确认——**不可**直接信其为权威去删（否则误删仍被 frontmatter 引用的附件 = 破坏别的笔记·违底线①）。
...
> **设计（新 composer 模板展开 + 1 string Store + extract 套模板·改 extract 字节）**：
> - **`core/templates.ts`**：新 `expandExtractTemplate(template, ctx:{content,fromTitle,newTitle,now})` —— **逐字镜像 `expandTemplate` 的单趟 regex-callback**（replacement 值永不重扫·提取内容含字面 `{{newTitle}}` 不会被二次展开）。regex `/\{\{(content|fromTitle|newTitle|date)(?::([^}]*))?\}\}/gi`：content/fromTitle/newTitle 字面替换（带 `:fmt` 则非变量·返回原文同 expandTemplate `{{title:x}}` 处理）；date → `moment(now).format(fmt || templateDateFormat || YYYY-MM-DD)`（复用既有 date 设置）。**不扩 `expandTemplate`**（templates feature 零触碰·composer 变量集 {content/fromTitle/newTitle/date} 与之不同）。
> - **`core/appearance.ts`**：`extractTemplatePath` string Store（默认 **""**·`geode.extractTemplatePath`·镜像 accentColor `readString` idiom）+ `setExtractTemplatePath`。默认 "" = R236 前 verbatim = **零回归**。
> - **`features/editor/noteComposerCommands.ts`**（extractRange·inline 单调用点不抽 helper）：`notePath` 算出后、`vault.create` **前**——`extractTemplatePath.get().trim()` 非空 → `vault.read(tmplPath).catch(()=>null)`；读到则 `expandExtractTemplate(tmpl, {content: content.replace(/\s+$/,""), fromTitle: stripExtension(basename(activePath)), newTitle: stripExtension(basename(notePath)), now: new Date()})`·**{{content}} 缺省追加到底部**（`tmpl.toLowerCase().includes("{{content}}") ? expanded : expanded.trimEnd()+"\n\n"+content.trimEnd()`·**case-insensitive 匹配展开 regex**·评审 minor 修）·末尾归一 `.replace(/\s+$/,"")+"\n"`。**默认空 / 读取失败 → `extractedContent(content)`（verbatim·现状·不丢数据）**。import extractTemplatePath/expandExtractTemplate/stripExtension。
> - **`features/settings/SettingsModal.tsx`**：NoteComposerSection += text input 行（testid `settings-extract-template`·镜像 templateFolder 输入框·受控 value=useStore·onChange=setter·**存 RAW·消费时 trim**）。
> - **`core/i18n/dict.views.ts`**：`settings.extractTemplate`/`Desc` × 中英。
> **data-safety（逻辑档·改 extract 写新笔记字节·底线①）**：模板 read 在 `vault.create` **前**·失败 `.catch(()=>null)` → fallback verbatim（**新笔记不会因模板缺失而空/坏**）；**create-before-edit + stale-guard 序不变**（新笔记先落盘·await 后重验 source span·变了保新笔记跳 splice）·source 改写仍走普通 CM 事务（非 applyExternalEdits）。**单趟展开**=提取内容含 `{{...}}` 字面不被二次展开（对抗输入）。默认 "" 逐字节同 R236 前（r44/r216 字节回归锁）。**桌面 probe**：extract 写 = R44 FS-vetted·R236 加 vault.read（既有只读 API）+ 纯字符串展开·浏览器 byte 等价。**测试**：`.calibration/r236-e2e.mjs`（默认空→verbatim·设模板路径→新笔记套模板[{{content}}/{{fromTitle}}/{{newTitle}}/{{date}} 各替换正确]·{{content}} 缺省追加·读取失败 fallback·持久化 reload·**对抗：提取内容含字面 {{newTitle}} 不被二次展开**）+ 回归 r44/r216/r234 字节不退。
...
> **设计（1 enum Store + ExtractMode 加 none + 注入 + 新设置 Tab）**：
> - **`core/noteComposer.ts`**：`ExtractMode` 加 `"none"` → `extractReplacement` 命中 `"none"` 返回 `""`（提取后原文删选区·不留引用·= Obsidian「None」）。embed/link 两分支字节不变。
> - **`core/appearance.ts`**：`ExtractReplaceMode = "link"|"embed"|"none"` + `extractReplaceMode` Store（默认 **"link"**·`geode.extractReplaceMode` localStorage·镜像 `defaultNewTabMode` enum idiom）+ `setExtractReplaceMode`。**命名避撞**：core 已有同名纯函数 `extractReplacement`(noteComposer.ts) → Store 命名 `extractReplaceMode`。默认 "link" = R234 前硬编码行为 = **零回归**。
> - **`features/editor/noteComposerCommands.ts`**（extractRange 内·原 :51 硬编码行）：`extractReplacement(finalName, "link")` → `extractReplacement(finalName, extractReplaceMode.get())`。`extractRange` 被 `editor:extract-selection` + `editor:move-heading` **共用**（两命令同尊重设置·= Obsidian 同一设置管两者）。"none" → insert `""` → 既有 `view.dispatch({changes:{from,to,insert:""}})` 自然当删除处理·`selection.anchor = from`（**create-before-edit + stale-guard 写路径逐字节不变**·底线①）。
> - **`main.tsx:1348`**：probe `__geodeComposer.replacement` 参数类型 `"link"|"embed"` → `ExtractMode`（import type·auto-sync "none"·防漂移）。
> - **`features/settings/SettingsModal.tsx`**：新核心插件设置 Tab「笔记重组」——4 处改动镜像 R231 QuickSwitcher：① `SectionId` +`"note-composer"`；② `NAV_GROUPS` corePlugins items +`{ id:"note-composer", labelKey:"settings.section.noteComposer", icon:"files" }`（**"files" 已注册**·避 fallback 到 file-text·testid 自动 `settings-nav-note-composer`）；③ 路由 switch +`{section==="note-composer" && <NoteComposerSection/>}`；④ `NoteComposerSection()` = 1 `<select className="settings-select" data-testid="settings-extract-replacement">` 三 option（link/embed/none·镜像 propertiesDisplay select 模板）。
> - **`core/i18n/dict.views.ts`**：`settings.section.noteComposer` + `settings.extractReplacement` + `Desc` + 三选项键 `settings.extractReplaceLink/Embed/None` × 中英。
> **范围（本轮只做下拉·拆轮）**：合并提示 toggle（需 confirm modal gate 在 `QuickSwitcher.tsx` merge 路径前）+ 模板文件位置（需 `expandTemplate` 接进 `extractedContent`·改 extract 字节）留 R235——两者是真功能·data-safety 邻接·单独排。
> **data-safety（逻辑档·碰 editor/vault 写路径）**：extract 走既有 `extractRange` create-before-edit + stale-guard（写新笔记失败源不动·await 后重验 span）·"none" 仅令 insert=""·**不新开写路径**·走普通 CM 事务（dirty→autosave·非 applyExternalEdits）。§A 竞态不碰（autosave/flush/rename/delete/watcher）·§C 未碰 markdown.ts（r18-diff N/A·extract 写编辑器 doc 非阅读管线）。默认 "link" = 现状字节同。**桌面 probe**：extract 写路径 = R44 vetted（已 FS-probe）·R234 纯 enum 注入·`__geodeComposer.replacement` 探针可断 link/embed/none 三输出·浏览器 byte 等价。**测试**：`.calibration/r234-e2e.mjs`（镜像 r231：进 Tab + select 三值 + 持久化 reload + 探针/真 extract 验源文留 `[[]]`/`![[]]`/空）。

## Round 233 additions — 选项2 bounded backlog 续：编辑器「始终聚焦新标签页」toggle（Obsidian native Editor「Always focus new tabs」）·gate openFile 新 tab 抢焦·零新依赖【契约冻结 v0.229】

> **状态：As-built（已交付·v0.229）。** 从「选项2 bounded backlog」取（碰 `core/workspace.ts` openFile = 核心导航·**逻辑档**·ROADMAP 已标「逻辑(碰 core openFile)」）。**Step 0 三 scout**：① reference `01-编辑器.md:12`（ground truth 表格行）= 「始终聚焦新标签页 | 开关 | **开** | 创建或打开文件时，立即切换至该标签页。」→ native 核心 Editor 设置·**默认 ON**·EN「Always focus new tabs」/ ZH「始终聚焦新标签页」；② WebSearch 实证（forum）= ON 新 tab 抢焦，OFF「从主区/书签面板打开到新 tab 保持当前 tab 焦点」（后台打开）；③ explorer 代码 scout 坐实抢焦唯一钥匙点 = `workspace.ts:445-446`（新 tab 分支 `activeTabId: tab.id` + `activePaneId: target.id`）。
> **设计（1 Store + openFile 加性 opt 门控 + 2 个显式命令豁免 + 设置 toggle）**：
> - **`core/appearance.ts`**：`focusNewTab` Store（默认 **true**·`geode.focusNewTab` localStorage）+ `setFocusNewTab`（镜像 `showViewModeToggle` R229 idiom）。默认 true = R233 前「恒抢焦」逐状态同 = **零回归**。
> - **`core/workspace.ts`**：openFile 签名**加性扩展** `opts.focus?: boolean`（冻结契约·~20 调用点默认值不变）。新 tab 分支 `shouldFocus = opts.focus ?? focusNewTab.get()`；`shouldFocus` 时维持原 `{ ...l, tabs:[...], activeTabId: tab.id }` + `activePaneId: target.id`（原行为）；否则**后台分支** = `{ ...l, tabs:[...l.tabs, tab] }`（不改 `activeTabId`）+ 返回 `{ ...s, root, modal: null }`（**不改** `activePaneId`/不改 leaf active）。`let openedInBackground` 旗标（reducer 内置·deterministic·幂等）→ 仅后台分支跳过 `emitActiveFile()`（active file 未变·避免对当前文件重发 active-file:changed）；**其余所有分支 `emitActiveFile()` firing 与 R233 前完全一致 = 零回归**。`modal: null` 两分支都保留（Ctrl+Enter 后台打开仍关 quick switcher）。
> - **两个显式命令强制 `focus: true`（correct·非 deviation）**：① **`app:new-tab` Cmd+T**（App.tsx）= 显式新建工作 tab（Geode 实现为 create Untitled.md·后台会遗留隐形空文件·Obsidian 空 new-tab 亦 focus）；② **`reopenClosedTab` workspace.ts:864**（R39）= 重开关闭 tab 后**立即读 `getActiveTab()`** 还原 mode/pin，依赖重开 tab 为 active（后台会静默丢失还原）。这是 `focus` opt 的两个真实调用点（满足 simplify ≥2 调用点·非投机参数）。
> - **所有 link/explorer「在新标签页打开」/ wikilink Mod-click / open-link-in-new-leaf / obsidian:// URI 等文件打开路径**：不传 `focus` → 尊重 `focusNewTab` 设置（OFF 后台打开·= native「Always focus new tabs」标准目标）。**compat 插件 getLeaf/openLinkText 亦继承设置**（faithful·插件开新 tab 尊重用户 pref）。
> - **`features/settings/SettingsModal.tsx`**：EditorSection +1 toggle 行（testid `settings-focus-new-tab-toggle`·镜像 view-mode/smart-lists 行）。
> - **`core/i18n/dict.views.ts`**：`settings.focusNewTab`/`settings.focusNewTabDesc` × 中英（官方串「Always focus new tabs」/「始终聚焦新标签页」+ 描述「创建或打开文件时，立即切换至该标签页。」）。
> **data-safety（逻辑档·非 .md 写）**：仅改「新 tab 是否设为 active」store 态——§A 竞态全不碰（不动 autosave/flush/rename/delete/watcher）·§B 无新 .md 写路径·§C 未碰 `markdown.ts`（r18-diff N/A）。reveal 消费 path-guarded（`EditorPane.tsx:891`）→ 后台打开 heading-link 不会 mis-scroll 当前 tab（reveal lingers·切到 bg tab 时 apply·eState-like）。**桌面 probe N/A**（纯 Store + 导航逻辑·跨端同码·WKWebView 无 CDP 难驱动焦点·browser E2E 全覆盖·同 R229/R232）。**测试**：`.calibration/r233-e2e.mjs`（默认 ON 抢焦/toggle OFF 后台打开[新 tab 存在但 active 未变]/Cmd+T 仍 focus/reopen 仍 focus+还原/持久化 reload）+ 回归 r81 tab 套件 + r229。

## Round 232 additions — 选项2 bounded backlog 续：编辑器「智能列表」toggle（Obsidian native Editor「Smart lists / 智能列表」）·gate markdownKeymap·零新依赖【契约冻结 v0.228】

> **状态：As-built（已交付·v0.228）。** 从「选项2 bounded backlog」取（碰 editor 逻辑档·镜像 R225 compartment）。**Step 0 双 scout**：① reference `01-编辑器.md:38`（编辑器-03.png ground truth）+ Obsidian 官方翻译仓 `setting.editor.option-smart-indent-lists` = native 核心 Editor 设置·**默认 ON**·EN「Smart lists」/ ZH「智能列表」·官方描述「Automatically set indentation and place list items correctly.」；② 代码 scout 坐实 HANDOFF「partial·无门控」——Geode 的列表续行来自 `@codemirror/lang-markdown` 的 `markdownKeymap`，**bundled 在 `markdown()` 内**（`addKeymap` 默认 true），`cmExtensions.ts markdownSansHeaderFold()` 只滤掉 foldService、keymap 一直在跑、**无任何 toggle/compartment 门控**。
> **关键洞察（gate 面比「列表」大·已记 deviation）**：`markdownKeymap` 绑定 Enter=`insertNewlineContinueMarkup`（列表续行/重编号/空项 outdent + **bundled 引用块续行**）+ Backspace=`deleteMarkupBackward`（markup 退格降级）。`@codemirror/lang-markdown` 把列表续行 + 引用块续行耦合进同一条 Enter 命令，**无免费的更细切分**（拆分须自写带上下文判定的命令包装·data-safety 红线高风险）。取舍：**gate 整条 markdownKeymap**（最干净的 R225 镜像·编辑器红线最低风险·默认 ON = 与 R232 前逐字节同），代价 = OFF 时引用块续行 + 退格降级一并停（仅用户主动关时显现·Obsidian 官方描述本身含糊「indentation + placement」·可辩护）。
> **契约（1 Store + smartListExtension factory + 新 compartment + 设置 toggle）**：
> - **`core/appearance.ts`**：`smartLists` Store（默认 **true**·`geode.smartLists` localStorage）+ `setSmartLists`（镜像 `autoPairMarkdown` R225 / `autoPairBrackets` R153 idiom）。
> - **`cmExtensions.ts`**：`markdown({…, addKeymap: false })` 摘掉 bundled keymap；新 `export smartListExtension(on) => on ? Prec.high(keymap.of(markdownKeymap)) : []`（与 lang-markdown 内部 `Prec.high(keymap.of(markdownKeymap))` 逐字节同·**保 Prec.high + 数组位置**=续行/退格 ON 时优先级与 R232 前不变）；`buildEditorExtensions` opts +`smartListCompartment: Compartment`（冻结契约·三处对齐：类型/解构/`.of()`）；`smartListCompartment.of(smartListExtension(smartLists.get()))` 插在 `markdownSansHeaderFold()` 后（keymap 原驻位）。
> - **`EditorPane.tsx`**：`smartListCompartmentRef` + `new Compartment()` 赋值 + 传 builder + cleanup 置 null + `useStore(smartLists)` + reconfigure effect（`compartment.reconfigure(smartListExtension(smartList))`·deps `[smartList]`·**逐位镜像 markdownWrapCompartment 7 处**）。
> - **`SettingsModal.tsx`**：EditorSection +1 toggle 行（testid `settings-smart-lists-toggle`·镜像 autoPairMarkdown 行）。
> - **`dict.views.ts`**：`settings.smartLists`/`settings.smartListsDesc` × 中英（官方串）。
> **分档＝逻辑档**（碰 editor 管线 keymap → 数据安全红线一票否决 + 新 store action + 新控制流）。**data-safety**：§A 竞态(1-6) 全不碰（autosave/flush/rename/delete/watcher 路径零改·toggle 只改 Enter/Backspace 插入物·不改保存机制·新 compartment ref 已在 cleanup 置 null）；§B 无新 .md 写路径（ON 时 keymap 逐字节同·OFF 时 defaultKeymap 纯换行）；§C `markdown.ts` 未碰 → **r18-diff N/A**；§D 桌面 probe **N/A**（keymap + localStorage·跨端同码·WKWebView 无 CDP 合不出 Enter·browser 全覆盖·同 R225）。compartment.reconfigure = 纯配置交换·永不动 doc/selection（R225/R153/R88 同 live-toggle 先例）。**简化门 clean**（faithful mirror·0 减项）。**多维对抗评审 7 维全证伪·0 缺陷**（评审对 `node_modules/@codemirror/lang-markdown/dist/index.js:422` 核实 `Prec.high(keymap.of(markdownKeymap))` 逐字节同·`addKeymap:false` 只摘 keymap[pasteURL/HTML 完成各自 flag 不动]·6 处 compartment 生命周期齐·e2e 非同义反复）。**r232-e2e 13... 10/10**（设置 toggle 默认 ON·`- foo`/`1. foo`/`> foo`+Enter 续行·OFF 不续·localStorage 持久化·reload honored）+回归 **r225 10/10·r153 8/8·r35 25/25** + typecheck 0 + cargo check + 生产构建。**deviation 记一句**：gate 整条 markdownKeymap·OFF 时引用块续行 + 退格降级一并停（耦合于 lang-markdown 单命令·拆分高风险不值·默认 ON 零感知）。

## Round 231 additions — 选项2 bounded backlog 续：快速切换器 3 档设置（Obsidian native Quick switcher「显示附件 / 仅显示已创建的笔记 / 显示所有受类型文件」）·纯只读导航·零新依赖【契约冻结 v0.227】

> **状态：As-built（已交付·v0.227）。** 从「选项2 bounded backlog」取（scout 评「★最干净」·实为中型：新设置 Tab + 文件列表逻辑）。**Step 0**：reference `05-核心插件.md:18+45`（核心插件-07.png ground truth）= native 3 设置：仅显示已创建的笔记(默认关) / 显示附件(默认开) / 显示所有受类型文件(默认关)。Geode QuickSwitcher（`QuickSwitcher.tsx:77 getMarkdownFiles()` 仅 .md·`:159 create 行恒显`）无此 3 设置（grep 零命中）·**无快切设置 Tab**（须新增）。底层就绪：`vault.getFiles()`（全量·vault.ts:230）+ `isAttachmentPath`（attachments.ts:266）+ openFile 已 R102 路由非-md 到只读 attachment 视图。**关键洞察**：显示附件默认 ON **既忠实又零回归**——只在 attachment 存在时才多列（既有 e2e[r38] fixture 全 .md·无 attachment → 行为字节同）。
> **契约（3 Store + QuickSwitcher 文件列表/create 门控 + 新设置 Tab）**：
> - **`core/appearance.ts`**：`switcherShowExistingOnly`(默认 false) / `switcherShowAttachments`(默认 **true**·Obsidian 默认) / `switcherShowAllTypes`(默认 false) 3 Store + setter（镜像既有 bool Store）。
> - **`QuickSwitcher.tsx`**：`files` memo 条件化——`showAllTypes ? getFiles() : showAttachments ? getFiles().filter(f => f.extension==="md" || isAttachmentPath(f.path)) : getMarkdownFiles()`（deps +3 flag）；create 行 `:159` 门控 `if (!exact && !showExistingOnly) out.unshift({kind:"create"})`；`useStore` 3 flag。
> - **`SettingsModal.tsx`**：NAV_GROUPS 核心插件组 +`quick-switcher` 入口 + `SectionId` +`"quick-switcher"` + render `{section==="quick-switcher" && <QuickSwitcherSection/>}` + 新 `QuickSwitcherSection`（3 toggle row·镜像既有 toggle）。**i18n `dict.views.ts`**：section heading + 3 toggle label/desc·中英。
> - **评审驱动修（重要·正确性）**：default attachments ON 后 `files` 含附件 → 原 `exact` 判定（决定是否隐 create 行）会被**附件 basename 精确匹配**误置 true（如 vault 有 `image.png` 无 `image.md`·键 "image" → 隐 create 行 → 无法经 create 行建 image.md）。修：**`exact` 仅 .md 文件命中才置 true**（`file.extension === "md"`·因 create 行建的是 .md NOTE·附件同名不使建笔记冗余）。r231-e2e 加 attachment-collision 用例验证。
> - **分档：逻辑档（新控制流[文件列表条件·create 门控·exact .md 守卫] + 3 新 Store——但非 data-safety：纯只读 switcher 导航·`getFiles/getMarkdownFiles` 只读·零 .md 写·未碰 editor 管线）** → 简化门 **clean**（3 inline toggle 匹配既有 idiom 不抽组件·3 Store 镜像既有 bool Store·simplifier 旁注 zh 措辞已校）→ 多维对抗 **deliverable·8 维全过·0 阻断**（文件列表三路条件正确[allTypes⊇attachments⊇md·isAttachmentPath 用对·md 单列必需]·**默认 attachments ON 零回归**[默认分支对全-.md vault === getMarkdownFiles 同集同序·r38 不破]·create 门控 + exact .md 守卫·useMemo deps 对·section 接线·持久化·分层/i18n/testid·纯只读非数据安全）。**As-built 验证**：typecheck 0（全 src）+ **r231-e2e 13/13**（设置 section 3 toggle·默认 attachments ON 显附件·all-types OFF 隐 .txt·create 行·**attachment 精确同名仍显 create 行[评审修]**·仅已创建 ON 无 create·attachments OFF 移附件·all-types ON 列 .txt·持久化 reload）+ 回归 **r38 19/19[switcher·.md fixture 零变·exact .md 守卫无影响]·r106 12/12** + 生产构建成功 + cargo check 通过（零 Rust）。**桌面 probe N/A**（switcher + localStorage·浏览器全覆盖）·**r18-diff N/A**（不碰 markdown.ts·零 .md 写）。
...
> **契约（命令 + 设置按钮·复用 vetted rebuildAll + showCommandNotice）**：
> - **`app/App.tsx`**：注册命令 `app:rebuild-cache`（`callback: () => void app.metadata.rebuildAll().then(() => showCommandNotice(t("cmd.rebuildCacheDone")))`·镜像既有 app-level 命令 + R183 toast）。
> - **`SettingsModal.tsx` FilesAndLinksSection（高级位·excluded files 后·`</section>` 前）**：action 按钮（`.settings-action-btn`·`onClick={() => void app.commands.execute("app:rebuild-cache")}`·镜像 reload-plugins 按钮 `:1442`）。**settings.css**：+`.settings-action-btn`（镜像 `.settings-accent-reset` 观感）。
> - **i18n `dict.*`**：`cmd.rebuildCache`/`cmd.rebuildCacheDone`（命令名 + toast·dict.app.ts）+ `settings.rebuildCache`/`...Desc`/`...Button`（设置·dict.views.ts）·中英。
> - **分档：逻辑档（rebuildAll 触 vault 文件 IO[读·重 index]·保守）——但 rebuildAll read-only：零 .md 写/删/链接改写·从磁盘重读重建 in-memory metadata index·不碰 DocumentHandle 未存内容** → 简化门 **clean**（纯接线·命令 thunk + 按钮 + i18n + CSS·无 intra-round 重复） → 多维对抗 + data-safety **deliverable·7 维全过·0 缺陷**（**data-safety 核心问坐实**：reviewer grep metadata.ts 全文零 `vault.write/create/modify/delete` + 零 `documents/DocumentHandle`·rebuildAll body 仅 clear 两 Map→indexFiles[唯一 IO=`vault.read` 纯读]→rebuildNameMap→bump·与 CM live state/未存内容完全隔离·零数据丢失·index 滞后未存=既有正常态非本轮引入；命令 id 唯一无 typo·toast `.then` rebuild 真完成才提示·空/大 vault 不抛·重复点击幂等无害）。**As-built 验证**：typecheck 0（全 src）+ **r228-e2e 9/9**（命令注册·执行 rebuildAll→revision bump + tag 'rbtag' 仍在 + tag→note 映射 rb.md 仍在[cache 重派生非丢]·设置按钮触发 revision bump·toast·无 page error）+ 回归 **r24 12/12·r65 12/12·r127 15/15[metadata 套件·rebuildAll 未改·零变]** + 生产构建成功 + cargo check 通过（零 Rust）。**桌面 probe N/A**（命令 + 内存重索引·浏览器全覆盖）·**r18-diff N/A**（不碰 markdown.ts·rebuildAll 零 .md 写）。**🆕 收尾据实纠正 R227「矿脉见底」为「partial-scout 误判」+ scout 余 11 项 bounded backlog 登记 ROADMAP「选项2 bounded backlog」节（持久执行队列·后续轮免重 scout）。新教训：① 别据【部分表面 scout】宣布矿脉枯竭——R226/R227 仅扫 editor/appearance 即判「交回大方向」·Ultracode 6 表面并行穷举 scout 实剩 13 bounded（文件与链接/核心插件设置 Tab/右键/Editor 顶部组等漏扫）→ 宣布 exhaustion 前须【全表面】穷举（含所有 reference 页 + 核心插件各设置 Tab + 右键 + 候选池）；② surface 既有 read-only 引擎（rebuildAll/setFontSize 等）= 最低风险 bounded 轮——引擎已 vetted·只补用户可达面（命令 + 设置/菜单）。**

## Round 227 additions — 选项2 补深六轮（bounded 矿脉收尾）：外观设置「快速调整字体大小」（Obsidian native Appearance「Quick font size adjustment」·字体组·默认关·Ctrl/Cmd+滚轮调字号）·零新依赖【契约冻结 v0.223】

> **状态：As-built（已交付·v0.223）。** 选项2 即取序 + Editor Display 组消化后**最后一个 trivial bounded 项**。**Step 0 verify-first**：reference `03-外观.md:34`「快速调整字体大小 | 开关 | 关 | 按住 Ctrl 滚轮快速调字号」（字体组·默认关）= native。Geode **已有字号基建**（`workspace.fontSize`[默认 16·workspace state] + `workspace.setFontSize(px)`[clamp [11,28] + 驱动 `--editor-font-size` CSS 变量]·R50·zoom 命令 app:zoom-in/out/reset 复用之）→ 缺口 = 一个 **toggle 门控的 Ctrl/Cmd+wheel 处理器**。
> **契约（新 Store + App 全局 wheel effect·复用 vetted setFontSize）**：
> - **`core/appearance.ts`**：`quickFontZoom = new Store<boolean>(readBool("geode.quickFontZoom", false))` + `setQuickFontZoom`（**默认 false = 当前无此行为·零回归**）。
> - **`app/App.tsx`（全局 wheel 监听·镜像既有 keydown effect line 1062）**：`useEffect` 加 `window.addEventListener("wheel", onWheel, { passive: false })`（**`passive:false` 必需·否则 preventDefault 无效**）；`onWheel` = 仅当 `quickFontZoom.get()`[读 live 值] 且 `(e.ctrlKey || e.metaKey)` 且 `e.deltaY !== 0` → `e.preventDefault()`（拦 webview 页缩放）+ `workspace.setFontSize(state.fontSize + (e.deltaY < 0 ? 1 : -1))`（复用 vetted clamp·scroll up=字号↑）。cleanup removeEventListener。
> - **`SettingsModal.tsx` 外观字体组**：toggle row 紧邻字体大小滑块（`settings-font-size`）·镜像既有 toggle。**i18n `dict.views.ts`**：+`settings.quickFontZoom`/`...Desc`（en+zh）。
> - **评审驱动改进（As-built）**：reviewer 建议 effect **gate on `useStore(quickFontZoom)`**（非 handler 内 `.get()`）→ **默认 OFF 时根本不装 non-passive wheel listener·保浏览器 passive-scroll 快路径**（默认零回归更纯·且缩小下条 nuance 的窗口到 ON-only）。已采纳：effect deps `[app, quickZoom]`·OFF early-return 不装监听。
> - **已知 minor（评审记·defer·gated 在 opt-in 后）**：toggle **ON** 时 Ctrl/Cmd+scroll **over graph canvas** → graph 自身 wheel handler（GraphView·preventDefault 缩放·不 stopPropagation）+ window handler **双触发**（graph 缩放 + 字号变）。default OFF 无此问题（不装监听）·niche 手势·修需耦合 graph 选择器或改 GraphView（diff 外）→ defer。
> - **分档：逻辑档（新控制流=wheel 处理器·新 Store——但非 data-safety：零 .md 写/vault IO·未碰 editor 管线[App-shell 全局监听·复用 vetted workspace.setFontSize·font size 是全局 UI state]）** → 简化门 **clean**（纯新 Store + 1 effect + 1 toggle·wheel/keydown effect 相似非相同不合并·守卫/passive:false load-bearing） → 多维对抗 **deliverable·8 维全证伪·0 阻断**（三段守卫各必要·`.get()`/gated 读 live·preventDefault 仅动作时[OFF/无修饰键不拦正常滚动]·默认 false early-return 零回归·cleanup 对称无泄漏·跨平台 ctrl||meta·分层/i18n/testid）。**As-built 验证**：typecheck 0（全 src）+ **r227-e2e 11/11**（默认 OFF Ctrl+wheel 无变·真 Settings UI toggle ON→Ctrl/Cmd+wheel up/down 改 fontSize·非修饰键滚轮无变·clamp 28/11 边界·持久化 reload·反向 OFF）+ 回归 **r50 15/15[font size/spellcheck] 不退（默认 false 零变）** + 生产构建成功 + cargo check 通过（零 Rust）。**桌面 probe N/A**（wheel + localStorage·浏览器全覆盖）·**data-safety 套件 N/A**（零 .md 写/vault IO·未碰 editor/markdown 管线）。**🛑 本轮后 bounded 零-dep native toggle 矿脉见底——HANDOFF/收尾报告就「交回大方向」（选项1 须批依赖 / Canvas·Bases·stacked-tabs 立项）向用户提示·建议暂停 loop 选向。**

## Round 226 additions — 选项2 补深五轮：G8 编辑器设置「从右到左 RTL」global toggle（Obsidian native Editor「Right-to-left」·显示组·默认关）·纯视觉·零新依赖【契约冻结 v0.222】

> **状态：As-built（已交付·v0.222）。** 选项2 即取序消化后续 G8 Display 组**最后缺口·完成后 Display 组全清**。**Step 0 verify-first**：① reference 截图 `_截图/01-编辑器/编辑器-02.png`（R224 ground-truth 实图·行号与 RTL 之间）确认「从右到左」是 native core 设置（显示组·默认关·`reference/01:28`「文档默认从右往左书写」）；② WebSearch 辨明：**Obsidian core = 全局默认方向 toggle**（本轮目标）·per-note frontmatter `direction` 是 community 插件 esm7/obsidian-rtl 特性（不做·非 core）。Geode 全仓零 RTL/direction 实现（explorer grep 证）= missing。
> **契约（镜像 R50 spellcheck contentDOM.setAttribute + R224 reactive effect·纯 DOM 属性）**：
> - **`core/appearance.ts`**：`rightToLeft = new Store<boolean>(readBool("geode.rightToLeft", false))` + `setRightToLeft`（镜像 spellcheckEnabled·**默认 false = LTR = 当前行为·零回归**）。
> - **`EditorPane.tsx`（编辑视图 + 阅读视图各一处·纯视觉）**：① 编辑视图——view 建时 seed `view.contentDOM.setAttribute("dir", rtl?"rtl":"ltr")`（镜像 R50 spellcheck seed）+ 反应式 effect `viewRef.current?.contentDOM.setAttribute("dir",...)` + `view.requestMeasure()`（CM6 原生 bidi 接管光标/选区/命中·**不碰任何编辑逻辑**·镜像 R50 spellcheck effect）；② 阅读视图——preview div（`markdown-preview-view`）加 `dir={rtl?"rtl":"ltr"}` prop（**不碰 `previewHtml` 串 → 绕开 markdown.ts/r18 字节管线**）。`useStore(rightToLeft)`。
> - **`SettingsModal.tsx` 显示组**：toggle row 紧邻 hideReferenceMarks（RTL 在 reference Display 组紧随其后）·镜像之。**i18n `dict.views.ts`**：+`settings.rightToLeft`/`...Desc`（en+zh）。
> - **分档：逻辑档（碰 editor 管线·底线①——但纯 contentDOM `dir` 属性 + 阅读 div prop·CM6 原生 bidi·零编辑逻辑改·零 .md 写·零字节）** → 简化门 **clean**（纯镜像 R50 spellcheck + 1 Store·seed+effect 对称必要实例非重复·requestMeasure 必需） → 多维对抗 + data-safety **deliverable·7 维全证伪·0 缺陷**（视觉-only[仅 contentDOM dir + preview div prop·零 dispatch/写路径·r226 末断言 doc bytes 不变]·CM6 bidi 接管光标/选区[livePreview decoration offset-based direction-agnostic 不动]·reactivity[effect 镜像 R50·requestMeasure 让 CM 重读方向·seed+effect 双覆盖]·阅读 div dir 不碰 previewHtml→r18 N/A·默认 false=LTR=prior·持久化·分层/i18n/testid·data-safety[无 dispatch/flush/activeElement·与 autosave/undo 零交互]）。**As-built 验证**：typecheck 0（全 src）+ **r226-e2e 9/9**（默认 OFF `.cm-content` dir=ltr·真 Settings UI toggle ON→编辑 dir=rtl + 阅读 preview div dir=rtl·持久化 reload 恢复·反向 OFF→ltr·**doc bytes 逐字节不变[RTL 视觉-only]**）+ 回归 **r33 37/37[字节锁]·r50 15/15[spellcheck]·r35 25/25·r24 12/12·r106 12/12 不退（默认 false 零变）** + 生产构建成功 + cargo check 通过（零 Rust）。**桌面 probe N/A**（纯 DOM 属性·浏览器全覆盖·R50/R88/R224 先例）·**r18-diff 字节套件 N/A**（不碰 markdown.ts·preview div dir prop 不碰 previewHtml 串）。**faithfulness 记（minor·非数据/契约）**：Obsidian RTL 在更高层挂 `dir`→整源码视图翻转（含行号 gutter 侧）；Geode 仅挂 contentDOM·**行号 gutter 可能仍居左**——纯像素级 nuance·在「纯视觉」冻结 scope 内·CM6 gutter 方向处理待视觉一瞥·不阻本轮。**完成后 Editor Display 组全清**（固定行宽/严格换行/属性/折叠标题·列表/行号/隐藏参考标记/从右到左 全 done）。

## Round 225 additions — 选项2 补深四轮：G8 编辑器设置「自动补全 Markdown 语法」toggle（Obsidian native Editor「Auto pair Markdown syntax」·行为组·默认开）·零新依赖【契约冻结 v0.221】

> **状态：As-built（已交付·v0.221）。** 选项2 即取序 G8 续项。**Step 0 verify-first + 据实纠误**：候选「自动补全英文标点」两设置——① **「自动补全英文标点符号」= Obsidian「Auto pair brackets」= R153 已 done**（`autoPairBrackets` toggle·配对 `( [ { " '`·非 R63 出队的 community 弯引号 typography）；② **「自动补全 Markdown 语法」= Obsidian「Auto pair Markdown syntax」= R35 实现功能但刻意未挂 toggle = partial**（`markdownWrapHandler` 选区包裹 `* _ ~ = $ \``·`cmExtensions.ts:677` 直进 base list·注释 `:673-676` 白纸黑字「deferred」）。web 实证（forum）：Auto pair brackets 管 `[] () {}` 结构符·Auto pair Markdown syntax 管 `* ** _ \`` 强调符·两设置正交。**R225 真缺口 = 第二项的独立 toggle**。
> **契约（镜像 R153 closeBracketsCompartment 模式·纯加性）**：
> - **`core/appearance.ts`**：`autoPairMarkdown = new Store<boolean>(readBool("geode.autoPairMarkdown", true))` + `setAutoPairMarkdown`（镜像 R153 autoPairBrackets·**默认 true = 当前恒开行为·零回归**）。
> - **`cmExtensions.ts`**：新 helper `markdownWrapExtension(on: boolean): Extension`（`on` → `markdownWrapHandler`·`off` → `[]`）；opts 加 `markdownWrapCompartment: Compartment`；base list `:677` 的 `markdownWrapHandler,` → `markdownWrapCompartment.of(markdownWrapExtension(autoPairMarkdown.get()))`；import autoPairMarkdown；改 `:673-676` 注释（deferred→toggled）。**`markdownWrapHandler`/`bracketWrap.ts` 行为零改**——仅把它移进 compartment 加门控，**维持 R35「仅非空选区包裹·空选区透传避让行首 bullet/fence/CJK」的刻意 scope·不扩行为**。
> - **`EditorPane.tsx`**：import autoPairMarkdown·`useStore`·新 `markdownWrapCompartmentRef`·create+pass+cleanup+reconfigure effect（**逐点镜像 closeBracketsCompartment**：ref:347 / create:496 / pass:515 / cleanup:585 / effect:629）。
> - **`SettingsModal.tsx` 行为组**：toggle row 紧邻 autoPairBrackets（:734）·镜像之。**i18n `dict.views.ts`**：+`settings.autoPairMarkdown`/`...Desc`（en+zh）。
> - **分档：逻辑档（碰 editor 管线·底线①——但新 toggle 纯 localStorage+CM reconfigure 零 dispatch·markdownWrapHandler 行为不改·写字节仅既有非空选区包裹 走 input.type[有 undo+autosave]·零新写路径）** → 简化门 **clean**（纯镜像 vetted R153 closeBrackets 模式 + 1 Store·compartment 7 接线点是既有模式对称实例非新重复·markdownWrapExtension 2 调用点镜像 closeBracketsExtension） → 多维对抗 + data-safety **deliverable·0 代码缺陷**（8 维全证伪：markdownWrapExtension(true)===原 base-list 直挂·markdownWrapHandler 移位无双挂[grep 证]·Prec.high 保留[Compartment.of 保 precedence·位置不变·r225 包裹仍工作=间接证]·reconfigure effect 逐点镜像 closeBrackets·正交[autoPairBrackets 独立 compartment 不干扰]·data-safety[仅既有非空选区包裹·input.type 有 undo/autosave·零新写路径·toggle reconfigure 不 dispatch changes]·默认 true=prior 恒开 no-op·据实纠误映射正确[英文标点=Auto pair brackets=R153 done·Markdown 语法=Auto pair Markdown syntax=markdownWrapHandler·bracketWrap.ts JSDoc 自证]）。**As-built 验证**：typecheck 0（全 src）+ **r225-e2e 10/10**（默认 ON `*/\`/_` 包裹选区·真 Settings UI toggle OFF→选区被 `*` 替换不包裹·**正交[autoPairBrackets 仍 `(`→`()`]**·持久化 reload 恢复·反向 toggle 回 ON 复包裹）+ 回归 **r35 25/25[bracket/wrap]·r153 8/8[auto-pair toggle]·r24 12/12[autosave] 不退** + 生产构建成功 + cargo check 通过（零 Rust）。评审揪 minor i18n（desc 漏 `$`·已补 en/zh）。**桌面 probe N/A**（CM 输入 + localStorage 设置·浏览器 e2e 全覆盖·R153/R88/R92 先例）·**r18-diff 字节套件 N/A**（不碰 markdown.ts 阅读视图·markdownWrapHandler 既有写路径未改）。**据实纠误**：候选「自动补全英文标点」第一项 = Auto pair brackets = R153 早 done（非 R63 community typography）·R225 仅补第二项 toggle。

## Round 224 additions — 选项2 补深三轮：G8 编辑器设置「隐藏参考标记」（Obsidian native Editor「Hide reference marks」·显示组·默认开）·纯渲染·零新依赖【契约冻结 v0.220】

> **状态：As-built（已交付·v0.220）。** 选项2 即取序 G8 首项。**Step 0 verify-first（R222/R223 纪律）**：① WebSearch native 设置名未确证（Obsidian online docs 稀疏）→ ② **读 reference 截图 `_截图/01-编辑器/编辑器-02.png` 确认 ground truth**：「隐藏参考标记」是真 native 设置（显示组·行号 与 从右到左 之间·默认 ON·toggle 实图可见）。语义：**ON（默认）= live preview 只在光标处显示 Markdown 语法分隔标记；OFF = 始终显示所有语法标记**（但 widget 渲染 + 样式标记保留·**不退化为 source mode**）。**不做 smart punctuation**（即取序 #4「自动补全英文标点」——data-safety 写字节 + R63 已出队「Smart typography」为 community 插件·native 的「自动补全英文标点」疑为 auto-pairing 非弯引号转换·须另轮 WebFetch 辨明）。
> **契约（单一门控 + 标准设置接线·全有先例）**：
> - **`core/appearance.ts`**：`hideReferenceMarks = new Store<boolean>(readBool("geode.hideReferenceMarks", true))` + `setHideReferenceMarks(on)`（镜像 R88 `showLineNumbers`·localStorage-backed·**默认 true = Obsidian 默认 ON = 当前 Geode 行为·零回归**）。
> - **`livePreview.ts`（核心·单一门控）**：`livePreview(app, getPath, hideMarks: boolean = true)` → `computeDecorations(view, app, getPath, hideMarks)`（line 685 签名加 param）→ **`hide()` helper（line 698）加首行 `if (!hideMarks) return;`**。这是整个 computeDecorations **唯一**的空-`Decoration.replace({})` 路径（13 个分隔符 hide 全经它：emphasis/strikethrough/inline-code/header/link/wikilink/highlight/callout/quote/block-id/comment/task），故单一门控即全量控制。**不动 `selectionTouches()`/widget replace/`.cm-live-*` 样式标记**（门控 hide() 本身·非揭示判定→OFF 时显示标记但保渲染）。
> - **`cmExtensions.ts`**：`editorModeExtensions(app, getPath, mode, hideMarks: boolean = true)` → `livePreview(app, getPath, hideMarks)`；buildExtensions（line 619）传 `hideReferenceMarks.get()`（import from @core/appearance）。
> - **`EditorPane.tsx`**：`useStore(hideReferenceMarks)`；2 个既有 `editorModeExtensions` 调用点（mode 切换 effect line 604·cbProcRev effect line 670）传 hideMarks；**新增 effect**（镜像 R88 showLineNumbers effect line 616-621 + cbProcRev modeCompartment.reconfigure line 664-671）：`hideRefMarks` 变更时 `modeCompartment.reconfigure(editorModeExtensions(app, ()=>handle.path, mode, hideRefMarks))`（livePreview ViewPlugin 仅在 doc/selection/viewport change 重建·Store 变更须 reconfigure 触发）。
> - **`SettingsModal.tsx` EditorSection（显示组）**：toggle row 镜像 `showLineNumbers`/`strictLineBreaks`（line 681-707）·`useStore(hideReferenceMarks)` + `onClick(setHideReferenceMarks)`。**i18n `settings.*`**：+`hideReferenceMarks`/`hideReferenceMarksDesc`（各 locale `dict.*.ts`）。
> - **门控全量取向（faithfulness 记·v1）**：`hide()` 门控**全部 13 个分隔符 hide**（emphasis/strikethrough/inline-code/header `#`/link/wikilink/highlight/callout/quote `>`/block-id/comment/task）→ OFF 全显。Obsidian「Hide reference marks OFF」精确子集（是否含 `#`/`>` 等块级标记）无法在仓库内确证（online docs 稀疏）；全量 no-op 是最简且语义合理（「显示所有语法标记·保 widget/样式」）的实现·若实测 Obsidian 仅揭示某子集再细分（minor·非缺陷）。
> - **分档：逻辑档（碰 editor/livePreview 管线·底线①——但纯 CM decoration·零 .md 写·非字节守恒回归）** → 简化门 **clean**（穿参线 reactive 必需·3 个 reconfigure effect 相似非相同[trigger 不同]·门控 load-bearing）→ 多维对抗 + data-safety **deliverable·0 代码缺陷**（门控只控 `hide()` 空-replace·widget[replaces.push widget]+样式[.cm-live-*]不经它故 OFF 保渲染非 source mode·reactivity 经新 effect reconfigure[ViewPlugin 不随 Store 重建]·appliedModeRef 守卫不阻新 effect·默认 true 是 prior behavior 的 no-op·纯 decoration 零 .md 写）。**As-built 验证**：typecheck 0（全 src）+ **r224-e2e 11/11**（默认 ON 隐 `**bold**`/hr widget 渲染·真 Settings UI toggle OFF→显 `**bold**`+hr widget 仍在[非 source mode]·持久化 reload 恢复·反向 toggle 回 ON 复隐）+ 回归 **r106/r107/r108/r109 12·r134 24·r135 18·r137 13·r128 19 全绿（默认 true 零变）** + 生产构建成功 + cargo check 通过（零 Rust）。**桌面 probe N/A**（纯 CM decoration 渲染·WKWebView 与浏览器同·零 FS/host 面·本特性无 file IO→MemoryVault↔FS adapter 差异无关·浏览器 E2E 等价）·**data-safety r18-diff 字节套件 N/A**（动 livePreview 编辑视图·非 markdown.ts 阅读视图管线·零 .md 写）。

## Round 223 additions — 选项2 补深次轮：FootnotesPanel 内联编辑脚注文本（对齐 Obsidian 1.9 native「Footnotes view」核心插件「Click a footnote to edit its text」）· data-safety 逻辑档·零新依赖【契约冻结 v0.219】

> **状态：As-built（已交付·v0.219）。** 选项2 即取序第 2 项 = Footnotes view。**Step 0 gate 实查 + WebFetch 双纠误**：① 独立 `FootnotesPanel` 早 R65 建、R213 增强（右侧栏·跟随 lastActiveFile·列 `[^id]: content`·click 跳定义·**纯读**）→ 面板早已存在；② 但 **WebSearch+WebFetch 实证 Obsidian 1.9.0（2025-05）新增 native「Footnotes view」核心插件**（`obsidian.md/help/Plugins/Footnotes+view`：「lists all footnotes in the active note. **Click a footnote to edit its text.** You can also navigate to the footnote's position」）——**explorer 初判「非 native」据 reference/05[1.9 前快照] + G3 矩阵[ID 列 `—`]，是 stale 误判**（R222 同族教训：reference 截图有时滞后 native 新增）。故 native 真缺口 = **面板内联编辑**（Geode 现只读·native 可改文本）。本轮补此缺口 = 把只读半成品做到 native 平价。**filter box/排序/分组等非 native 不加**（防造非 native 功能）。
> **契约（refactor core/metadata 抽共享解析器 + FootnotesPanel 改写为第二写者）**：
> - **`core/metadata.ts` 抽共享脚注定义解析器（单一真源·防漂移）**：新增导出 `extractFootnoteDefs(content: string, masked: string): FootnoteRef[]`（= 现 parseNote 内联 def 循环·**逐字节不变**搬出）+ `parseFootnoteDefinitions(content: string): FootnoteRef[]`（薄包装：`parseFrontmatter`→offset-preserving blank→`maskCodeRegions`→`extractFootnoteDefs`）。`parseNote` 改调 `extractFootnoteDefs(content, masked)`（复用其已算 `masked`·**零双 mask·零行为变**）。FOOTNOTE_DEF_RE 仍 module-level。**FootnoteRef 类型不变**（types.ts:68·`{id;content;from;to}`）。
> - **`FootnotesPanel.tsx` 改写为第二写者（镜像 R86 FilePropertiesPanel 范本）**：`app.documents.acquire(activePath)` 取 `DocumentHandle`（refcount·release-on-change·acquire-fail→empty）+ 订 `handle.revision` 重渲。**脚注从 `parseFootnoteDefinitions(handle.getText())`【LIVE 文本】解析**（非 metadata cache `getFootnotes`——cache 滞后 live 编辑→position 会 stale→写错位损坏；live parse 保 from/to 始终正确）。
> - **内联编辑 UX（native「click to edit」+「navigate」双行为）**：行内 `^id` marker = 跳转按钮（`geode:scroll-to-heading` 事件·复用 R213 jumpTo）；content = click 进编辑态（`<input>` 单行·Enter/blur 提交·Escape 取消）。提交 = `handle.applyExternalEdits([{from, to, insert: \`[^${id}]: ${newText}\`.trimEnd()}])`（替换整 def 行·复用 R86 vetted 写路径·路由 undo/dirty/save + 同步开着的 CM view）。
> - **data-safety 不变量（底线①·byte 正确性 6 条）**：① **写前 re-parse + 身份校验**（As-built·比契约初版的 startsWith-on-stale-range 更安全）：commit 时**重解析 LIVE 文本** `parseFootnoteDefinitions(handle.getText())`，按 document-order `index` 取 `fresh[ed.index]`，校验 `fresh[ed.index]?.id === ed.id`——不符则 abort（list 在编辑窗口内重排）·**永远写新鲜完整 def 行 `[from,to]`→`[^id]: text`，绝不写滞后偏移**（startsWith-on-stale 若并发改了同 def 长度会把尾残字节当 continuation 拼接·re-parse 杜绝）。残留次要语义边角（非字节缺陷）：重复 id + 编辑窗口内在被编辑 def 前并发插/删同 id def → 可能改错同 id 的另一条（极罕见·可 undo·不损坏字节）；② **单行约束**：`<input>` 单行·newText 无换行→def 永不被拆成 def+孤行；多行 continuation 行在 `to`（行尾·`\n` 前）之外→只编辑首行、continuation 字节不动（v1·与面板单行预览一致·defer 多行）；③ **no-op 跳过**：newText===oldContent 不写（零 churn）；④ **范围外字节不变**：替换仅 `[from,to]`（def 行）·其余 .md 字节逐字保留（byte 回归证）；⑤ **undo 经 handle**（applyExternalEdits 路由·非裸 vault.modify）；⑥ **竞态**：activePath 变→重置编辑态 + release 旧 handle；外部修改→handle 同步+revision 重渲+re-validate 兜底。
> - **i18n `dict.panels.ts` `footnotes.*`**：+`editAria`/`editPlaceholder`/`jumpAria`（中英）。**`footnotes.css`**：编辑 input 样式（复用面板观感）。
> - **分档：逻辑档（data-safety 红线·改 .md 字节·碰 documents 写路径）** → 简化门 **clean**（fresh code-simplifier·无 ≥8 行重复/死代码/脚手架·extractFootnoteDefs 两调用点·FootnoteEdit 是 hooks 约束下必需组件非可删抽象·re-validate 防御 load-bearing）→ **多维对抗评审 + data-safety 全回归 deliverable·0 阻断**（字节损坏族 R192/R197/R216 逐点证伪：EOF 无尾换行/多行 continuation 在 `to` 外不动/code-fence 经 maskCodeRegions 排除/frontmatter offset-preserving blank/含 `]:`·wikilink·inline-code/空内容 trimEnd→`[^id]:` 全字节正确；两写者经 R86 vetted applyExternalEdits·切文件 setEditing(null)+release 绝不写错文件·handle 生命周期镜像 FilePropertiesPanel 无泄漏；index+id 守卫字节绝对安全·残留重复-id 重排边角可 undo 非损坏）。**As-built 验证**：typecheck 0（全 src）+ **r223-e2e 13/13**（内联编辑 byte 精确+no-op revision 不 bump+Escape 取消+空内容 trim+marker navigate·笔记开在编辑器=两写者）+ 回归 **r213 13/13·r65 12/12[jump 断言改点 fn-marker·非真空]·r127 15/15·r24 12/12 不退** + 生产构建成功 + cargo check 通过（零 Rust）。**桌面 probe：write 路径 = R86/R213 vetted DocumentHandle→vault→Rust fs（已 FS-probe 证）·R223 新逻辑 = parseFootnoteDefinitions 范围计算 + insert 串纯 JS·浏览器 e2e byte 精确（MemoryVault 与 FS adapter 共享同一 DocumentHandle 写逻辑·仅 backend 异且 backend 已证）→ 等价桌面保证·不重建陈旧二进制**。

## Round 222 additions — 选项2 补深首轮：TagsPanel ↔ Obsidian native「Tags view」核心插件对齐（嵌套 树/扁平 切换 + 全部展开/折叠 + Cmd/Ctrl-click 切换标签到搜索词）·纯读·零新依赖【契约冻结 v0.218】

> **状态：As-built（已交付·v0.218）。** 2026-06-25 用户拍板「新取用顺序」选项2 即取序首项 = 独立 tag pane。**Step 0 gate 实查纠误**：独立 `TagsPanel`（右侧栏·树形·4 档排序·全库重命名·click→search）**早在 R41 建、R69/R150/R151 增强 → 已存在·矩阵 `tag-pane:open`=`app:show-tags` 早 done**（HANDOFF/ROADMAP 表「部分/未做」系据实纠误对象）。故本轮非「抽面板」（已做），而是**对照 Obsidian native Tags view 补三处真·缺口**（WebFetch `obsidian.md/help/Plugins/Tags+view` 权威 + forum 实证「filter box」属 Feature-archive 未实现 → **不加 filter box**[非 native]）：
> 1. **嵌套标签 树↔扁平 切换**（native 设置「nested tags as a tree or as a flat list」）→ 持久化 `localStorage["geode.tagsDisplay"]`（"tree"|"flat"·镜像 R151 `geode.tagsSort` raw-localStorage 先例）。flat = 仅真实标签（map keys·去 phantom 父）逐行展全路径名·按同 sortKey 排。
> 2. **全部展开 / 全部折叠**（native「Expand all / Collapse all」hierarchy 控件）→ 批量改既有 per-session `collapsed: Set<fullPath>`（R150·不持久化·与 native session 态一致）。collapse-all = `new Set(所有 children.length>0 的 fullPath)`；expand-all = `new Set()`。仅 tree 模式可见。
> 3. **Cmd/Ctrl-click 标签 → 切换到当前搜索词**（native「Holding Ctrl/Cmd toggles it within the search term rather than performing a direct search」）→ 普通 click 不变（`requestSearch('#'+path)`·Geode tag-browser 模式）；修饰键 click = 新 `app.workspace.toggleSearchTag(path)`。
>
> **契约（新增·跨模块冻结）**：
> - **`workspace.ts`**：`readonly searchTagToggle = new Store<string | null>(null)`（一次性·session-only·镜像 `searchRequest`）+ 方法 `toggleSearchTag(tag: string): void`（`setLeftPanel("search")` + `searchTagToggle.set(tag)`）。**不改 `requestSearch(query: string)` 签名**（5 caller：graph/tags/editor·uri·properties）→ 纯新增、零破坏。
> - **`SearchPanel.tsx`**：消费 `searchTagToggle` —— 用 **`.get()` 读现值并先置 null** 的 StrictMode-safe 模式（memory `geode-oneshot-store-strictmode`：toggle 是自反操作·dev 双触发会 on→off 抵消 → 必须 effect 内 `.get()`+先清零，闭包值会双写）；纯函数 `toggleTagInQuery(query, tag)`：按空白切 token，若已含 `tag:TAG` 或 `#TAG` 则删该 token，否则 push `tag:TAG`，`join(" ")`（多 `tag:` 运算符 AND·= native 累积过滤）。**已知边角（评审记·接受）**：含**内部多空格的引号短语**会被 `split(/\s+/)+join(" ")` 塌缩（`"a  b"`→`"a b"`）——短语匹配是字面子串、对内部空格敏感，故塌缩有语义；单空格短语完好，触发须「既有多空格短语 query + 又 Cmd-click 标签」=极低频。非必修。
> - **`TagsPanel.tsx`**：导出新纯函数 `buildFlatTags(map, sortKey): {fullPath; count}[]`（Map 去重防 malformed 重 testid·镜像 buildTagTree 的 `split("/").filter` 守卫）；`displayMode` state（持久化）；`collectCollapsibleパス`（树内 children>0 的 fullPath）；header 加 3 控件（display-toggle / expand-all / collapse-all 图标按钮）；row onClick 加修饰键分支。**纯读**——不动 `renameTagAcrossVault`/vault/documents 写路径（§风险：右键改名仍是唯一写路径·本轮不碰）。
> - **`icons.tsx`**：+3 Lucide path（`chevrons-up-down`=expand-all / `chevrons-down-up`=collapse-all / `list-tree`=tree 模式）。**机械追加·零逻辑**。
> - **i18n `dict.panels.ts` `tags.*`**：+`displayTree`/`displayFlat`/`expandAll`/`collapseAll`（中英双语·命名空间 `tags.*` 既有惯例）。
> - **`tags.css`**：header 控件按钮样式（复用 `.tag-chevron` 观感·新 `.tags-tool` 类）+ flat-row（复用 `.tag-row`·`.tag-chevron-spacer` 占位无 chevron）。
>
> **分档：逻辑档**（新增导出 `toggleSearchTag`/`searchTagToggle`/`buildFlatTags` 契约 + 新控制流[display 分支·修饰键分支·token 切换]）。**但非 data-safety 红线**：零 `.md` 写·零 vault/documents/markdown 管线触碰（标签面板纯读 `getTagMap()`·搜索面板纯设 query state）→ **data-safety byte 套件 N/A**；聚焦 reviewer 多维对抗（验：StrictMode 双触发不双写 toggle / flat 去重无重 React key / toggleTagInQuery 自反幂等不破 phrase / 修饰键分支不误触普通 click / expand-collapse-all 只在 tree 可见 / sort×display 正交持久化）。
> - **验收口径**（ROADMAP「强制验收口径」）：`app:show-tags` 早 done（命令进 280）·面板入口 right-tab-tags 早有·display/sort 值持久化·重启生效（r222-e2e 验 reload restore）·对照 native Tags view 行为字字核（树/扁平/展开折叠/修饰键累积过滤）。**「禁用插件后消失」N/A**——native 核心插件总表（reference/05）不含 Tags-view 可关项·tag-pane 非可 toggle 核心插件 → 该验收线不适用本项。
> - **验证（As-built）**：typecheck 0（全 src）+ **r222-e2e 24/24**（浏览器·镜像 r151 harness：树默认/扁平切换·全展开折叠·修饰键累积`tag:`·**StrictMode 单击只切一次[自反双触发防住]**·plain click 替换·display pref reload 持久化）+ 回归 **r150 20/20·r151 13/13 不退** + 生产构建成功（vite·4.46s）+ cargo check 通过（零 Rust 改）。**简化门：clean**（fresh code-simplifier·无 ≥8 行重复/无死代码/无脚手架·`renderTagButton` 是合法减法去重·两 comparator map 相似非相同[segment vs fullPath sort]→拒并）。**对抗评审 10 维 deliverable·0 confirmed**（StrictMode 自反双触发[最高危]手工推演两路径只切一次=memory `geode-oneshot-store-strictmode` 已落地·toggleTagInQuery 自反幂等+嵌套 token 不破·buildFlatTags Set 去重无重 React key·修饰键分支不误触 plain/右键·collapsiblePaths 深度递归全覆盖·sort×display 正交·纯读底线①[renameTagAcrossVault 字节未动]·分层合规[ReactMouseEvent 别名不遮蔽 DOM MouseEvent]·i18n/图标真注册·testid 无重无遗漏）。**桌面 probe N/A**（纯前端·只读 `getTagMap()`+UI state·零 vault/documents/FS adapter/Rust/host 触碰·浏览器 E2E 全覆盖即等价桌面行为·precedent R220/R221）·**data-safety byte 套件 N/A**（零 .md 写/vault IO/markdown 管线）。

## Round 221 additions — F2 类型对齐续片：`Command` editorCallback ctx 字字对齐 obsidian.d.ts（`MarkdownView`→`MarkdownView | MarkdownFileInfo`）+ 新增 hover 链 `MarkdownFileInfo`/`HoverParent`/`HoverPopover`/`Point`/`PopoverState`（compat 自包含·零新依赖）【As-built v0.217】

> **状态：As-built（已交付·v0.217）。** 商业主轴=插件迁移：让插件 `import type { X } from "obsidian"` 对 Geode compat shim 编译一致。续 R220 的 F2 类型对齐，本轮收 `Command`（plugin.ts）的 editorCallback/editorCheckCallback ctx 缺口 + 补 obsidian 顶层导出的 hover/MarkdownFileInfo 类型链。oracle = 仓库内 `.calibration/obsidian.d.ts`（≈v1.13）。**⚠️ 收尾发现 2026-06-25 用户拍板新顺序（ROADMAP「取用顺序总览」）= F2 降级、自 R222 转选项2「设置/核心插件补深」——R221 是 F2 末片（用户明示「不打断」）；下一项不再是 F2。**
> **契约（新文件 compat/obsidian/hover.ts + workspace.ts 加 MarkdownFileInfo + plugin.ts Command 修订 + index.ts barrel）**：
> - **`Command.editorCallback` ctx `MarkdownView`→`MarkdownView | MarkdownFileInfo`**（d.ts:1794）+ **`editorCheckCallback` ctx 同 widen**（d.ts:1821）。内部唯一 provider（fixture.ts:222）`view` 参数未用 → 不破；唯一 caller（plugin.ts addCommand:715/726）传 `MarkdownView` 实参 → 仍 assignable 进 union → 不破（全仓 grep + typecheck 0 证）。
> - **保留 `callback`/`editorCallback` 返回 `unknown`（不随 d.ts 改 `any`）**：文档化偏离——CLAUDE.md 硬律「不用 any」；返回型对插件**编写**用例零影响（`() => T` 恒 assignable 进 `() => unknown`），仅 differ 于「读回 callback 返回值」这一插件几乎不做的用例 → 取 `unknown`（strictly safer）。
> - **新增 `MarkdownFileInfo extends HoverParent`**（d.ts:3954·`app: App` / `get file(): TFile | null` / `editor?: Editor`）→ 落 workspace.ts（紧邻 MarkdownView·已 import App/TFile/Editor）+ barrel 导出。**compat `MarkdownView` 改 `implements MarkdownFileInfo`**——既有 `View.app!: App` + `FileView.file: TFile|null` + `MarkdownView.editor: Editor` 已 3/4 满足，唯缺 `hoverPopover` → 补 `hoverPopover: HoverPopover | null = null` 字段（**唯一运行时改·恒 null·Geode 无 hover 预览**）。**这是 ctx widen 真正生效的关键**：插件注解 `editorCallback (editor, ctx: MarkdownFileInfo)` 须 compat MarkdownView **assignable 进** MarkdownFileInfo（union 两成员皆 assignable）才与 obsidian d.ts（`MarkdownView implements MarkdownFileInfo`）编译一致——否则 widen 仅装饰、annotated 用例仍 differ。
> - **新增 hover 链（新文件 hover.ts·compat 自包含·只 import ./component）**：`Point { x; y }`（d.ts:5179）/ `enum PopoverState {}`（d.ts:5193·空枚举·faithful）/ `HoverParent { hoverPopover: HoverPopover | null }`（d.ts:3464）/ `class HoverPopover extends Component { hoverEl; state; constructor(parent, targetEl, waitTime?, staticPos?) }`（d.ts:3476）。**HoverPopover/PopoverState 为值导出（class/enum）→ 支持插件 `new`/`instanceof`/值 import**；但 **Geode 无 hover-preview 运行时**（已记 gap plugin.ts `registerHoverLinkSource`）→ HoverPopover 是**惰性 shim**（构造 inert·`hoverEl=createElement("div")`·不渲染预览）·文档化。
> - **v1 defer（非本轮）**：`Workspace.activeEditor` 返回型 `MarkdownView`→`MarkdownFileInfo`（d.ts·语义层 widen·插件读 activeEditor 拿窄面·属 Workspace 类型片·单独轮）；`WorkspaceLeaf` 等其它 HoverParent（本轮只补 MarkdownView 的 hoverPopover·Command ctx 只需它）。
> - **R128/原 Command 契约修订留痕**：`Command` 接口此前于 plugin.ts 既定。本轮 editorCallback/editorCheckCallback ctx widen 为**字字对齐 d.ts 的契约修订**（向后兼容 widen·全仓 grep 证零内部 provider/caller 破坏 + typecheck 0）。
> - **分档：逻辑档**（修订导出 `Command` 契约 + 新增导出类型/惰性 class·虽近零运行时）→ 简化门 + 多维对抗评审（验：ctx widen 不破内部 provider/caller·hover 链字字对齐 d.ts·HoverPopover shim inert 不渲染·MarkdownFileInfo 字段对齐·返回型 unknown 偏离正当）；**data-safety byte 套件 N/A**（零 .md 写·零 vault IO·零 editor 管线改动）。
> - **验证（As-built）**：r221-types.ts + tsconfig.r221.json **编译期 exit 0**（`import type/value { MarkdownFileInfo, HoverParent, HoverPopover, Point, PopoverState }` 可解析 + editorCallback 接受 `ctx: MarkdownFileInfo` 注解 + **4 个 `@ts-expect-error` 全 bite**：HoverParent.hoverPopover 必填 / Point.y 必填 / ctx≠`any`[`_ctx: number` 错] / ctx 是 union 非 MarkdownView[`_ctx: MarkdownView` narrow 错·同 obsidian d.ts strictFunctionTypes 行为]）+ **r221-e2e 14/14**（运行期 require('obsidian')：HoverPopover class+PopoverState enum 值导出存在·`new HoverPopover(...)` inert 不抛[hoverEl=DIV·instanceof Component]·compat addCommand editorCallback 注册+执行·ctx instanceof MarkdownView 且带 hoverPopover/editor/app[MarkdownFileInfo 实证]·replaceSelection 写穿）+ **typecheck 0（全 src）** + 回归 **r220 7/7·r166 18/18·r128 19/19**。**简化门：删 1 处自相矛盾 stale 注释（净 −2·MarkdownFileInfo jsdoc 误留「defer implements」与实际 implements 矛盾）。对抗评审 7 维 deliverable·0 confirmed**（d.ts 字字核·MarkdownView 真 assignable 进 MarkdownFileInfo[View.app!/FileView.file/editor/hoverPopover 四面齐]·ctx widen 零内部 provider/caller 破坏·HoverPopover shim inert 无泄漏·unknown 偏离变性证安全·测非 green-by-omission）。**cargo N/A（零 Rust）·桌面 probe N/A（纯类型面+惰性 shim·typecheck/浏览器可验证）·data-safety byte 套件 N/A（零 .md 写/vault IO/markdown 管线）**。
...
> - **`core/workspace.ts`**：`isFilelessSingletonView` 类型守卫（4 视图）。**重构既有触点改用它**（行为逐字保持·r211/r81/r36 套件证）：duplicate 守卫[686]、**sanitizeTab 拒收门[1371]**（`markdown||attachment||isFilelessSingletonView`）、**sanitizeTab viewType 重派生[1384]**（`isFilelessSingletonView(vt) ? vt : filePath? fileViewType : "markdown"`·类型守卫窄化免 cast）。`openOutgoingLinks()`/`openOutline()`（各镜像 openBacklinks singleton）。**`openSingletonView(viewType,title)` 私有 helper**收 openGraph/openBacklinks/openOutgoingLinks/openOutline 四个 find-or-focus-or-create 为一行委托（4 真实调用点·减重复非新抽象）。**reopenClosedTab[839]：评审驱动改为 helper 路由**——原 4 硬 per-view 分支收成 `if (isFilelessSingletonView(entry.viewType)) { openSingletonView(entry.viewType, entry.viewType); return true }`（**reopen 是 R211 漏判类的第 5 个潜在触点**·未来加视图只更新 helper 即覆盖·net −10 行·title 对这些视图纯装饰[TabBar 由 viewType 经 i18n 取名 App.tsx:1789-93·从不读 tab.title]）。
> - **`core/types.ts`**：`TabState.viewType` += `"outgoinglinks" | "outline"`。
> - **`features/outgoinglinks/OutgoingLinksPanel.tsx` + `features/outline/OutlinePanel.tsx`**：各 `activePath` 改 `activeTab?.viewType === "markdown" ? filePath : useStore(lastActiveFile)`（非 markdown active tab 回退·同 R211 BacklinksPanel·主区面板有内容·更忠实 Obsidian）。
> - **`app/App.tsx`**：主区 render 分发 +2 per-view 分支（`.main-outgoinglinks-view`/`.main-outline-view` 包 Panel）；tab title +2（i18n）；icon +2（outgoing `external-link`·outline `list`）；split-disabled[1861/1869] 改用 helper；命令 `outgoing-links:open-outgoing-links` + `outline:open-outline`（**无默认键**）→ openOutgoingLinks/openOutline。
> - **`core/i18n/dict.app.ts`** +键 `app.outgoingLinksTab`/`app.outlineTab`（en/zh）+ `cmd.openOutgoingLinks`/`cmd.openOutline`（en/zh）。**`app.css`** +`.main-outgoinglinks-view, .main-outline-view{flex:1;min-height:0;padding:0 8px}`（同 R211 main-backlinks-view·避 flex 链断裁剪）。
> - **分档：逻辑档（workspace 序列化·主区视图 host·view-only 无 .md 写零 vault IO）**——helper 重构既有序列化触点须行为保持（r211/r81/r36 不退证）；序列化 round-trip 走真 persist→reload→restore（R211 教训）。
> - **对抗评审（Ultracode 3-lens[serialization-completeness/refactor-fidelity/regression-blast-radius]+skeptic 综合 → verdict=deliverable·0 crit/major·2 findings 均 confirmed-but-low）**：① **nit→主动修**=reopenClosedTab 未走 helper（见上·补 r212-e2e reopen 断言锁定）；② **minor→defer**=FootnotesPanel/FilePropertiesPanel 未加 lastActiveFile 回退·与 backlinks/outgoing/outline 三兄弟面板不对称（非回归[非 markdown active 时本就空]·出 R212 范围·记候选池[尤 FilePropertiesPanel：Obsidian Properties 跟随活动文件→应加]）。serialization lens 穷举全 viewType switch 触点·确认 sanitizeTab 两站 + duplicate + split 已全经 helper。
> - **As-built 实测**：typecheck 0·**r212-e2e 17/17**（注册无键 + 2 命令开主区[跟随 lastActiveFile] + singleton + **真 persist→reload→restore 全 4 singleton viewType** + 重构未破 graph + **reopen 经 helper 恢复为本型**）·回归 r62 14/14[出链空态据新回退校准]·r211 12/12·r81 14/14·r82 27/27·r36 47/47·cargo·**生产构建**·**桌面 probe r212-probe 8/8**（直调 workspace.open* 绕命令注册时序·验真 WKWebView：两 viewType 开+singleton+markdown 仍开）。**probe 教训**：plugin onload 早于 React 命令注册（cmd_count=5）→ 桌面验命令注册不可靠·改直调 workspace 公有方法测序列化逻辑（App-Nap 安全·读 Store 非 DOM）。
> - **桌面 probe r212-probe 8/8（见上）**。**v1 偏差/defer**：pinned 模式；主区 aux 视图不可 dup/split（singleton·同 graph）；侧栏出链/大纲在非 markdown active tab 改显 lastActiveFile（更忠实·r62 据此校准）；footnotes/properties 面板回退对齐 defer。

## Round 211 additions — G3 §8「在新标签页打开反向链接」首片（1 命令 backlink:open-backlinks · 主区视图 host：viewType "backlinks" + openBacklinks · 复用 graph 主区先例 + lastActiveFile 跟随 · data-safety 逻辑档[workspace 序列化·无 .md 写] + 零新依赖）【As-built v0.207】

> **状态：As-built（已交付）。** 表面复刻 G 系列——按 R182 矩阵收割 §8 `partial`「在新标签页打开反向链接」（Obsidian「Backlinks: Open backlinks for the current file」·把反链面板开成**主工作区 tab**·非仅侧栏）。**主区视图 host 立项首片 = 仅 backlinks 一个面板·仅「跟随活动文件」一种行为**（不含 outgoing/outline/pinned·那些 follow-on）。**关键：复用 graph 主区视图先例**（`viewType:"graph"` 已是 filePath:null·主区 singleton·可序列化）——backlinks 照搬此模式·**唯一额外功**=反链 INHERENTLY 需「当前文件」（graph 是全库视图无此依赖）→ 复用既有 `lastActiveFile`（主区 backlinks tab active 时 getActiveFile 返 null·但 lastActiveFile 经 emitActiveFile 仅在非空时更新→**保留最后活动 markdown 文件**）。**契约（扩 workspace viewType 冻结契约 + 主区 render 分发 + 1 命令，无破坏性签名变更）**：
> - **`core/workspace.ts`**：`TabState.viewType` += `"backlinks"`（**扩冻结序列化契约**·sanitizeTab[1342] 加 "backlinks" 放行→持久化 tab 可恢复·reopenClosedTab[816] 加 backlinks 分支→Mod+Shift+T 复开）。`openBacklinks()`（镜像 openGraph：主区 singleton·find 既有或在活动 pane 新建·filePath:null·mode:"preview"·title 经 i18n 渲染时覆盖）。duplicate 守卫[661] + split-disabled 同 graph（singleton 不可 dup/split）。
> - **`features/backlinks/BacklinksPanel.tsx`**：`activePath` 计算改 `activeTab?.viewType === "markdown" ? activeTab.filePath : useStore(lastActiveFile)`（**非 markdown active tab[graph/backlinks] 时回退 lastActiveFile**·使主区 backlinks 显最后活动文件反链·亦更忠实 Obsidian[侧栏在 graph tab 时也显最后文件非空]·markdown active 时两者同值不变）。
> - **`app/App.tsx`**：主区 render 分发[1620] 加 `viewType === "backlinks" ? <BacklinksPanel /> :`；tab title[1764]/icon[1792·"link"]/split-disabled[1851/1859] 加 backlinks（同 graph）；命令 `backlink:open-backlinks`（name「Open backlinks for the current file」·**无默认键**）→ `workspace.openBacklinks()`。
> - **`core/i18n/dict.app.ts`** +键 `app.backlinksTab`（en `Backlinks` / zh `反向链接`）+ `cmd.openBacklinks`（en/zh）。
> - **分档：逻辑档（workspace 序列化·主区视图 host·view-only 无 .md 写零 vault IO）**——扩 viewType 冻结契约·序列化 round-trip 须全（漏一 touch-point=backlinks tab 恢复失败/丢布局·非 vault 数据丢失但降级体验）。BacklinksPanel 改纯前端视图（不写文档）。
> - **桌面 probe N/A 浏览器可验证**（openBacklinks + viewType 序列化 + 主区 render + lastActiveFile 跟随 browser-E2E 兜）。**v1 偏差/defer**：outgoing-links/outline 主区变体（同 host·follow-on）；pinned 模式（仅 follow-active）；主区 backlinks 不可 dup/split（singleton·同 graph）；侧栏反链在非 markdown active tab 时改显 lastActiveFile（原显空·更忠实 Obsidian·r82 27/27 不退）。
> - **⚠️ 对抗评审结论（Ultracode 2-lens[serialization-workspace/correctness-fidelity] + skeptic synthesis · 初判 needs-fixes → 修后 deliverable · 1 critical + 2 major 全修 · 4 refuted）**：serialization lens **穷举全部 `viewType === "graph"` touch-point 查 backlinks 对等**·揪出 1 个漏。**1 critical 修（序列化完整性·正是本 lens 存在的意义）**：`sanitizeTab` 有**两处** viewType 处理——拒收门[1371·我已加 backlinks] + **viewType 重派生[1384·漏]**（原仅 `=== "graph" ? "graph" : ...`·持久化 backlinks tab[filePath null] 非 graph 且 filePath null → 重派生为 "markdown" → 重启后变幻影空 editor·graph 活 backlinks 死）→ 修=重派生分支保 graph **AND backlinks**。**1 major 修（测试假阳性）**：r211「round-trips」断言原 `JSON.parse(JSON.stringify(state))` 只验字段存活·**从不调 sanitizeTab**→把坏 restore 路径认证为绿 → 改=**真 persist→reload→restore round-trip**（写 localStorage geode.workspace.v1→reload→sanitizeTab→断言 backlinks tab 恢复为 backlinks view 非幻影 markdown·此测在 critical 修前会红）。**1 major 修（CSS flex 链断）**：`.main-backlinks-view` 无 CSS·wrapper div 缺 flex:1/height → `.backlinks-panel{height:100%}` 对无定高父解析为内容高·`.bl-scroll` 无界 viewport 不滚·长反链列表被 `.main-content overflow:hidden` 裁剪无滚动条（graph/editor 是直接 flex child 填高·额外 wrapper 断链）→ 修=`.main-backlinks-view{flex:1;min-height:0;padding:0 8px}`。**4 refuted**：侧栏 lastActiveFile 回退[有意·更忠实·r82 不退·delete 清 null→空]/lastActiveFile 指已关 tab 文件[数据仍对·同 local-graph 先例]/singleton+follow diverge[v1·同 graph 先例]/e2e 覆盖缺口[两要害已成 confirmed]。**根因教训（新通则·已记忆）**：**扩展序列化枚举（viewType）有多处处理点·须穷举**（拒收门 sanitizeTab[1371] AND viewType 重派生[1384]·漏一即静默降级 tab 布局）；测序列化 round-trip 必走真 persist→restore 路径·非 JSON clone（JSON clone 假阳性认证坏路径）。
> - **r211-e2e 12/12**（注册无键 + 命令开主区 backlinks tab[viewType backlinks·跟随 lastActiveFile 显源笔记反链] + singleton + **backlinks tab active 时 lastActiveFile 回退仍显反链** + **D 真 persist→reload→restore round-trip[localStorage→sanitizeTab→恢复为 backlinks view 非幻影 markdown]**）+回归 **r81 14/14[tabs]·r82 27/27[backlinks 侧栏]·r36 47/47[tab 键盘/reopenClosedTab]**·typecheck 0/cargo/生产构建。

> **状态：As-built（已交付）。** 表面复刻 G 系列——按 R182 矩阵收割 §10 末项 `missing`「折叠/展开当前笔记属性」（Obsidian 内置「toggle properties folding」命令·web 实证真命令[Fold-Properties-By-Default 插件「calls the built-in command」]），**§10 整组补完**。**纯视图折叠·绝不改文档**（无 .md 写·底线①不在「丢数据」意义受威胁；唯一 wrinkle = live 模式属性面板是 CM block widget·折叠改高度需 CM 重测）。同轮**矩阵据实纠误**：§5「更换主题 theme:switch」web 实证 **Obsidian 核心无独立切换器命令**（仅设置内主题下拉·Geode 已匹配[R178]）→ partial=忠实对齐非缺口·改 done-as-Obsidian；§14「搜索默认键 Mod+Shift+F」已 R185 绑定·矩阵 stale→done。**契约（新 per-path fold Store + PropertiesPanel/EditorPane 加性 + 1 命令，无跨模块签名破坏）**：
> - **`features/editor/foldProperties.ts`（新·纯前端）**：`foldedPropertiesPaths = new Store<ReadonlySet<string>>(new Set())`（**in-memory·per-path**·resets on restart=v1·Obsidian workspace 持久化 defer）+ `togglePropertiesFold(path)`（immutable Set copy→set·Store 通知）。
> - **`features/editor/PropertiesPanel.tsx`**：`useStore(foldedPropertiesPaths)` → `collapsed = folded.has(props.path)`。根 `.properties-panel` 加 `is-collapsed` class + 一个 `.properties-fold-toggle` chevron 按钮（`data-testid="properties-fold-toggle"`·`aria-expanded`·点击 `togglePropertiesFold(path)`）。**entries 始终渲染**（CSS `is-collapsed` 时 `display:none`·保留既有 testid·折叠时高度塌缩）。新增可选 prop `onLayoutChange?: () => void` + `useLayoutEffect(() => onLayoutChange?.(), [collapsed])`（折叠后同步触发 CM 重测）。
> - **`features/editor/EditorPane.tsx`**：live 模式 PropertiesPanel 传 `onLayoutChange={() => viewRef.current?.requestMeasure()}`（CM block widget 折叠改高度·requestMeasure 让 heightmap 重测·避布局空隙）；阅读视图 PropertiesPanel 不传（regular DOM·自然回流·无需重测）。
> - **`app/App.tsx`**：命令 `editor:toggle-fold-properties`（`available: getActiveFileEditorView()!==null`·**无默认键**）→ `togglePropertiesFold(activePath)`（命令只翻 Store·面板自身 useLayoutEffect 处理重测）。
> - **`core/i18n/dict.app.ts`** +2 键 `cmd.toggleFoldProperties`（en `Toggle fold properties` / zh `折叠/展开当前笔记属性`）+ `editor.propertiesFoldLabel`（en `Properties` / zh `属性`·折叠时标签）。**`editor.css`** +`.properties-fold-toggle` chevron 样式 + `.is-collapsed .property-row{display:none}` + chevron 旋转。
> - **分档：逻辑档（editor 管线·CM heightmap）**——但**纯视图态·零 .md 写·零 vault IO**（折叠不 mutate 文档·data-safety 不在丢数据意义受威胁·保守跑 editor 回归）。
> - **桌面 probe N/A 浏览器可验证**（折叠纯前端·Store + DOM class + requestMeasure browser-E2E 兜）。**v1 偏差/defer**：fold 态 in-memory（restart resets·Obsidian workspace 持久化 follow-on）；折叠 affordance = 小 chevron（Obsidian fold gutter hover·近似）；**纯阅读视图命令灰显**（getActiveFileEditorView 在阅读视图返 null=CM view 已销毁·同 R23 全编辑器命令惯例·chevron 仍可折叠=阅读视图 affordance）。
> - **⚠️ 对抗评审结论（Ultracode 2-lens[datasafety-cm/correctness-fidelity] + skeptic synthesis · verdict=deliverable · 0 crit/major · 2 minor[1 修 1 记]+1 nit 修 · 7 refuted）**：data-safety lens **证折叠零文档写**（togglePropertiesFold 仅 copy in-memory Set + Store.set·两触发[chevron onClick/命令]皆只调它·collapsed 渲染分支仅改 className+label·rows 仍 read-only parseProperties·无 applyEdit/dispatch 路径·byte-identical e2e 有码佐证）；CM lens **证 heightmap 重测时机正确**（useLayoutEffect 在 React commit `.is-collapsed`[display:none 瞬时·仅 chevron rotate 过渡·layout-neutral]后、paint 前跑·requestMeasure 排下帧 layout flush 后 measure 读最终塌缩高度·null viewRef optional-chain no-op·StrictMode 双触发仅 measure 两次幂等不双 toggle）。**1 minor 修（#1·真 UX bug）**：命令 available() 原仅判 active editor·非「笔记有属性块」→ 在无 frontmatter 笔记 toggle 会存幻影 fold 态·后续加属性时面板 mount 即 `.is-collapsed` 隐藏新行 + 延后 .focus() 落 display:none input → 修=available()/callback 加 `parseProperties(doc)!==null` 门控（对齐面板自身渲染门）·e2e +「无属性笔记命令不可用」断言。**1 nit 修（#3）**：onLayoutChange inline arrow 每渲染重建 → 布局 effect deps[collapsed,onLayoutChange] 每次 portaled 重渲染都重测（无害·CM 合帧·但多余）→ 修=`useCallback(()=>viewRef.current?.requestMeasure(),[])` 稳定化·只 fold 翻转时重测。**1 minor 记不修（#2）**：纯阅读视图命令灰显[同 R23 惯例·chevron 仍折叠]=有意一致 v1。**7 refuted**：折叠改文档[证伪·零写路径]/heightmap 时机错[证伪·时机正确]/Set 不清理[v1 in-memory·零数据影响]/per-note 跨笔记泄漏[证伪·path-keyed 稳健]/Set 无界增长[v1 session-bounded]/CSS·aria·always-chevron 错[证伪·正确·chevron 有意 discoverability]/e2e 覆盖缺口[非缺陷·17→19 断言已覆盖核心]。
> - **r210-e2e 19/19**（注册无键 + 命令折叠/展开[is-collapsed class + rows display:none] + **底线① 文档字节 fold 前后逐字一致[零 .md 写]** + chevron 点击+aria-expanded + **per-note 态[折 A 不折 B·返 A 仍折]** + **命令门控[有属性可用/无属性不可用]**）+回归 **属性套件 r30 25/25·r126 11/11·r194 12/12·r196 15/15[chevron 上置未破属性编辑]·r33 37/37[EditorPane onLayoutChange 加性未破格式引擎]**·typecheck 0/cargo/生产构建。**§10 整组补完**（折叠/展开 R188 + 折叠属性 R210）。

> **状态：As-built（已交付）。** 表面复刻 G 系列——按 R182 矩阵收割 §2 末项 `missing`「插入附件」（Obsidian `editor:attach-file`「Insert attachment」），**§2 整组补完**。**关键决策：用 `<input type=file>` 而非 Tauri 原生 `open()` 对话框**——原生对话框只回 vault 外**绝对路径**，而无命令能读它（`vault_read_binary` 受 `safe_join` 限定 vault 根内·`plugin-fs` 未装=硬边界#5）→ 走原生要加 Rust 命令 + 新依赖；`<input>` 在 WKWebView + 浏览器都弹原生选择器、都回 `File`（`.arrayBuffer()` 取字节），**和 paste/drop 吃同一种 `File`**→100% 复用 R17 vetted `ingestFiles`/`importAttachment`（含数据安全护栏：doc-identity stale 检测、isConnected、uniquePath 串行 + 大小写不敏感 no-overwrite）·单代码路径·零分支·**双端可验证**（浏览器 `<input>` + MemoryVault 二进制写）。**契约（新 editor 命令文件 + 窄改 export + 复用 R17，无跨模块签名破坏）**：
> - **`features/editor/attachCommand.ts`（新）**：`registerAttachCommand(app, getActive: ()=>ActiveView|null)` 注册 `editor:attach-file`（`available: getActive()!==null`·**无默认键**）。callback：建隐藏 `<input type=file>`（`data-testid="attach-file-input"`）→ click 弹选择器 → `change`：取 `input.files`→**重取 `getActive()` 取新鲜 view+path**（picker 长异步·闭包旧值会指向陈旧 doc/offset=[[geode-oneshot-store-strictmode]] 同源纪律）→ `pending=files.map(f=>({file,name:f.name}))`→ `ingestFiles(app, ()=>active.path, active.view, pending, sel.from, sel.to)`。`cancel` 监听移除孤儿 input（best-effort·现代引擎）。
> - **`features/editor/attachments.ts`**：`ingestFiles` 由模块私有改 `export`（窄改·零行为变·r67 9/9 不退证 paste/drop 未破）。**任意文件类型**（attach 不像 paste/drop 滤图片·pending 含全部 input.files·ingestFiles/importAttachment 不看 MIME·byte-generic）。
> - **`app/App.tsx`**：mount `...registerAttachCommand(app, () => getActiveFileEditorView(app))`（命令集群·同 registerLinkCommands getActive 范式）。
> - **`core/i18n/dict.app.ts`** +1 键 `cmd.attachFile`（en `Insert attachment` / zh `插入附件`）。
> - **分档：逻辑档（data-safety·vault 二进制写 + editor 插入·异步 picker 新时序）**——经 ingestFiles→importAttachment→`vault.createBinary`（create_new 拒覆盖）写二进制。**字节安全**：复用 R17 vetted 全护栏（uniquePath + 大小写不敏感 no-overwrite·doc-identity stale 退化为光标插入·isConnected）·picker 后重取新鲜 view+path（同一 getActive() 快照·view/path 一致）·零新写机制。
> - **桌面 probe N/A 浏览器可验证**（Playwright filechooser 拦 `<input>` 驱真命令端到端·MemoryVault 二进制写回；原生对话框无法自动化是所有桌面 picker 的共性·桌面手测）。**v1 偏差/defer**：单文件 picker（无 `multiple`·下游引擎已多文件就绪[map+join]·增强 follow-on）；切文件中途插入落**当前活动笔记**（重取新鲜·避陈旧 offset·同 R17 paste/drop 退化语义）；cancel 清理 best-effort（现代引擎）。
> - **⚠️ 对抗评审结论（Ultracode 2-lens[datasafety-timing/correctness-wiring] + skeptic synthesis · verdict=deliverable · 1 minor 修 · 6 refuted）**：**1 minor 修（fidelity·本轮 any-file picker 新暴露 R17 fallback）**：无扩展名文件（Makefile/LICENSE）命中 `doImport` 的「无扩展名→硬默认 .png」分支（paste/drop 名总带扩展名·从未触此）→ `Makefile`→`Makefile.png`+`![[Makefile.png]]`（背离 Obsidian 逐字保留·无数据丢失但保真偏差）→ 修=`doImport` 无扩展名分支**保留原名**（`ext=""`·`uniquePath` 已 empty-ext-safe[`suffix=ext?'.'+ext:''`]·大小写碰撞 loop 同改 empty-ext-aware）；扩展名分支不变→零 risk 图片命名·r67 9/9 不退证。**6 refuted（全 v1 设计选择/测试覆盖建议非缺陷）**：cancel-leak[best-effort 已加监听]·切文件中途丢 embed[isConnected+docAtStart 守卫·同 R17 退化·孤儿附件可恢复无字节丢]·e2e 未测非图片[byte-generic 路径同·已被 extensionless 修捕获]·单文件[v1]·重取新鲜插当前活动笔记[有意 data-safety 设计·避陈旧 offset]·e2e 未覆盖 cancel/no-active 分支[构造守卫·非缺陷]。**no-overwrite 经 uniquePath + 大小写不敏感 + createBinary(create_new) 三层证**（e2e pic.png 二次→pic 1.png·原 [1,2,3,4] 不变）。
> - **r209-e2e 13/13**（A 注册无默认键[name=Insert attachment] + C 全流程[filechooser pick pic.png→`![[pic.png]]` 插入 + `vault.readBinary` 字节匹配 + 瞬态 input 移除] + D **no-overwrite**[同名二次→`![[pic 1.png]]`·原 pic.png 字节不变·新 pic 1.png 新字节] + E **extensionless 保留**[Makefile→`![[Makefile]]` 非 .png + vault 无扩展名文件]）+回归 **r67 9/9[drop/paste 摄取·importAttachment 改未破]**·typecheck 0/cargo/生产构建。**§2 整组补完**（插入内部链接 R189 + 设/移除小标题 R186 + 数学/分隔线/脚注 R198/R199 + 重命名小标题 R204 + 插入表格 R205 + 嵌入内容 R208 + 插入附件 R209 …）。

## Round 208 additions — G3 §2「嵌入内容」（1 命令 editor:insert-embed · 插入 `![[]]` · 扩 R33 format 引擎 insertEmbed[镜像 R189 insertWikilink + `!` 前缀] · data-safety 逻辑档[byte] + 零新依赖）【As-built v0.204】

> **状态：As-built（已交付）。** 表面复刻 G 系列——按 R182 矩阵收割 §2 `missing`「嵌入内容」（Obsidian `editor:insert-embed`「Insert embed」·从热键截图转写·真命令）。**bounded·镜像 R189 `insertWikilink` 仅多 `!` 前缀**（`![[]]` vs `[[]]`）。`插入附件`（§2 余 1·需原生文件选择器 + vault 拷贝·桌面中心·浏览器不对称）defer。**契约（扩 R33 纯引擎 + 1 命令，无跨模块签名破坏·纯加性）**：
> - **`core/format.ts`**：`FormatOp` += `"embed"`。`insertEmbed(text, from, to): FormatEdit` 纯函数（**镜像 insertWikilink**：空选区→`![[]]` 光标在括号内[`from+3`]；有选区→`![[selected]]` 光标在 `]]` 后；只替换 [from,to]·绝不 unwrap·永不 no-op）。`applyFormatOp` switch +`case "embed"`（`never` 穷尽门）。**< ~8 行不与 insertWikilink 合并**（低于 dedup 阈值·`!` 前缀 + offset 差异·两薄函数比加 prefix 参数清晰）。
> - **`features/editor/formatCommands.ts`**：`FORMAT_COMMANDS` +1 spec `{ id:"editor:insert-embed", nameKey:"cmd.insertEmbed", op:"embed" }`（**无默认键**·同 insert-wikilink·Obsidian 未设置）。复用既有 registerFormatCommands→applyFormat 派发，零改。
> - **`core/i18n/dict.app.ts`** +1 键 `cmd.insertEmbed`（en `Insert embed` / zh `嵌入内容`·矩阵官方名）。probe `__geodeFormat.apply` 通用覆盖，无需改。
> - **分档：逻辑档（data-safety·碰 core/format.ts editor 写路径）**——insertEmbed 产 .md 字节。**字节安全**：纯字符串插入·**不读 syntaxTree/overlay**（非 Lezer 损坏族）·镜像 R189 vetted insertWikilink（仅多 `!`）·只替换 [from,to]·零新 vault 写路径（走正常 CM transaction → autosave）。
> - **桌面 probe N/A 浏览器可验证**（纯函数 `__geodeFormat.apply` 探针 + live CM dispatch·同 R189/R198 format 引擎既定）。**v1 偏差/defer**：`插入附件`（需原生 picker + vault 拷贝·桌面中心·单独轮）；embed 仅插结构（用户再键入/补全目标·同 insert-wikilink·Geode 不弹建议器）。
> - **⚠️ 渲染调查（评审 key 点·证实非死语法）**：本轮 render PROOF 初见 `__geodeRenderMarkdown("![[Note]]")` → `<p>!<a class="internal-link">Note</a></p>`（字面 `!`+link·**非** embed）。调查根因：**裸 `__geodeRenderMarkdown` 探针不传 `noteEmbeds` 选项 + null resolver** → 走 markdown.ts:536-540 fallback（`!`+link）。**真实阅读视图非如此**：`features/editor/preview.ts`/EditorPane/HoverPreview/Slides/Export 均调 `renderMarkdownToHtml(src, resolver, {noteEmbeds:true,…})` → markdown.ts:489-535 `bang && resolveEmbed/noteEmbeds` 分支渲染 **geode-embed**[图片]/**geode-embed-file**[音视频/PDF]/**geode-embed-note**[笔记 span]。旁证 `noteComposer.ts:77` 既有用 `![[${name}]]` 作 embed 形态。**∴ insert-embed 产出有意义的 embed 语法·非死语法**；空 `![[]]`（inner 空）不匹配 `WIKILINK_RE`（`[^[\]]+?` 要 ≥1 字符）→ 与 vetted 空 `[[]]`(R189) 同保持字面、不触坏 widget。**修正：r208 render PROOF 移除**（测的是探针缺省非命令·embed 渲染属 R12/R26/R61 既有套件域）。
> - **⚠️ 对抗评审结论（focused reviewer · 放行 · 0 confirmed 缺陷）**：① insertEmbed 字节逐字镜像 insertWikilink（node 实测 5 边界：空/选区/CJK/doc-start/元字符 `*|`·空选区光标 `from+3`=wikilink `from+2`+1·只动 [from,to]·永不 no-op）；② 非死语法（见上渲染调查）；③ `!` 前缀无副作用（纯字符串插入·走标准 CM 事务→autosave·不读 syntaxTree/overlay·空 embed 不触坏 widget）；④ wiring 齐（union/switch/never 门/spec 唯一/i18n en+zh·typecheck 0 证无 phantom 键）；⑤ SIMPLIFY-NO（~6 行低于 dedup 阈值·镜像兄弟 insertInlineMath 先例·不合并）。data-safety 清单复核：无新写路径·继承 B 类写护栏·避开 Lezer syntaxTree 根因。
> - **r208-e2e 12/12**（byte[空选区 `![[]]` 光标+3·mid-text·选区 `![[Note]]`·中段选区 span·startsWith `![[`·**CONTRAST wikilink `[[x]]` vs embed `![[x]]`**] + 注册无默认键[name=Insert embed] + live[选区 `Target`→`![[Target]]`·周边字节保留]）+回归 **r33 37/37·r189 17/17[insert-wikilink 镜像源不退]**·typecheck 0/cargo/生产构建。

## Round 207 additions — G3 §17 表格编辑器 片 B（9 命令 复制/移动行列 + 列对齐 · 扩 R206 已 vetted core/tableEditor.ts 纯模型[只加 9 thin op·解析/序列化/检测/守卫层不动] · data-safety 逻辑档[byte·整表重写] + 零新依赖）【As-built v0.203】

> **状态：As-built（已交付）。** 表面复刻 G 系列——收割 §17「表格 Table」**片 B = 剩 9 命令**（复制列/复制行·向上/下移动行·向左/右移动列·左/右/居中对齐），补完 §17 整组（R206 片 A 6 + 片 B 9 = 15）。**关键：R206 的解析/序列化/检测/`hasOverlayPipe` 守卫/indent 保留全部不动**（已 vetted）——片 B 只往 `applyOp` switch 加 9 个**纯 model→model** 分支（重排/复制/改 align），不新增任何字节/检测面，故风险面 ≪ R206。**契约（扩 R206 纯模块 + 命令文件，无跨模块签名破坏·纯加性）**：
> - **`core/tableEditor.ts`**：`TableOp` += `"copy-row"|"copy-column"|"move-row-up"|"move-row-down"|"move-column-left"|"move-column-right"|"align-left"|"align-right"|"align-center"`（9）。`applyOp` +9 case（`never` 穷尽门强制补）：**copy-row**=在 body i+1 插 `rows[i]` 副本（表头区→null）；**copy-column**=在 col c+1 插 header[c]/aligns[c]/各 row[c] 副本；**move-row-up/down**=swap body i↔i∓1（边界 i≤0 / i≥末 / 表头→null）；**move-column-left/right**=`swapCols(model,c,c±1)`[swap header/aligns/各 row·边界 c≤0/c≥末→null]；**align-left/right/center**=`aligns[c]=op.slice("align-".length)`（设列 align·focus 不动）。focus：copy/move 跟随新/移动后的行列；align 留原位。**解析/序列化/locateCell/hasOverlayPipe/applyTableOp 签名与体不变**。
> - **`features/editor/tableCommands.ts`**：`TABLE_COMMANDS` +9 spec（`id: editor:table-copy-row/-copy-column/-move-row-up/-move-row-down/-move-column-left/-move-column-right/-align-left/-align-right/-align-center`·`nameKey`·op·**无默认键**）。`runTableOp`（含 R206 indent 保留）不动——新命令复用同一派发。
> - **`core/i18n/dict.app.ts`** +9 键 en+zh（zh 用矩阵官方名 复制行/复制列/向上移动行/向下移动行/向左移动列/向右移动列/左对齐/右对齐/居中对齐；en Duplicate row/Duplicate column/Move row up/Move row down/Move column left/Move column right/Align left/Align right/Align center）。**`复制`=duplicate（插入副本·自包含完整 op）**——web 实证 Obsidian 1.5「copy and paste」指剪贴板，但剪贴板复制无配套 paste-row 命令=半功能、出片 B 范围；duplicate 是完整有用语义（收尾报告记此偏差·剪贴板 copy/paste row = follow-on）。probe `__geodeTableEdit` 无需改（通用覆盖任意 TableOp）。
> - **分档：逻辑档（data-safety·editor 写路径·整表重写）**——applyTableOp 产 .md 字节。**字节安全继承 R206**：9 op 纯重排/复制/改 align 模型·不碰解析/序列化/检测/守卫（hasOverlayPipe 仍 gate·renderCell 仍转义·indent 仍保留）·整表替换走 view.dispatch 单事务·零新写路径·内容字节保留。
> - **桌面 probe N/A 浏览器可验证**（同 R206）。**v1 偏差/defer**：剪贴板 copy/paste row/column（Obsidian「copy and paste」真义·需 paste-row 解析·follow-on）；align 为 SET 非 toggle（无「清除对齐」命令·Obsidian 同）；含 overlay-pipe/blockquote/缩进越界表仍 safe decline（R206 守卫）。
> - **⚠️ 对抗评审结论（Ultracode 2-lens[op-correctness/data-safety] + skeptic synthesis · verdict=deliverable · 0 confirmed[0 crit/major/minor/nit]）**：op-correctness lens **0 finding**（逐 op 手算边界守卫[copy-row 表头→null·move-row-up bodyRow≤0→null·move-row-down ≥末→null·move-column-left col≤0→null·move-column-right ≥末→null]·focus 落点·swapCols 三处一致 swap·align SET 非 toggle·分隔字节 `:--`/`--:`/`:-:`·never 穷尽门）；data-safety lens 6 raw 全 **refuted=安全断言成立非缺陷**（**片 B 不产新 cell 串**——9 op 只 relocate 既有 model cell[`.slice()`/splice/swap]或空串·均经 R206 unchanged renderCell round-trip；escaped-pipe 复制双侧重转义·align 不改 `aligns.length`；**hasOverlayPipe 守卫统一 gate 全 9 op**[applyTableOp→parseTable→null·无旁路]；ragged 经 parseTable 先 rectangularize·op 保矩形；**indent 保留共享 runTableOp·op-agnostic**·列表内嵌表不塌陷；非光标行字节经 sliced 副本保留）。**关键验证：R206 字节风险层[parse/serialize/renderCell/hasOverlayPipe/runTableOp indent]全未改 → r206-e2e 52/52 回归绿**，证片 B 纯加性零回归。**根因/策略印证**：R206 把模型做对（critical 在彼处揪净）→ 片 B thin ops 评审零缺陷净过——**片 A/片 B 拆分策略奏效**（高风险 greenfield 单独重审·后续 thin ops 复用 vetted 基建）。
> - **r207-e2e 32/32**（byte 9 op[copy-row/column·move-row-up/down·move-column-left/right·align-left/right/center] + no-op 守卫[表头/首末行列界] + **align render PROOF[`:--`/`:-:`/`--:`→ HTML text-align:left/center/right]** + **G overlay-pipe 守卫统一 gate 片 B[move/align/copy on `[[a|b]]`→null]** + 注册 9 命令无键 + live[move-row-down 实测换行·表头保留]）+回归 **r206-e2e 52/52[字节风险层未改]·r33 37/37**·typecheck 0/cargo/生产构建。**§17 表格组补完**（片 A 6 + 片 B 9 = 15 全 done）。

## Round 206 additions — G3 §17 表格编辑器 片 A（6 命令 表格增删行列 · 新 core/tableEditor.ts 纯模型[markdown-it escapedSplit 对齐阅读视图] + features/editor/tableCommands.ts[复用 R55 findTableRanges 检测] · data-safety 逻辑档[byte·整表重写] + 零新依赖）【As-built v0.202】

> **状态：As-built（已交付）。** 表面复刻 G 系列——按 R182 矩阵收割 §17「表格 Table」（Obsidian 1.5+ 内置表格编辑器·命令组 ~15 条·**无默认键**）。**片 A = 增删行列 6 命令** + 建**可复用表格模型**（解析/序列化/光标定位）；**move/align/copy 9 命令 = 片 B（R207·模型已 vetted 后追加 thin ops）defer**。**关键根因纪律（R201/R204/R205 同族）**：表格单元格切分**必须对齐阅读视图**——Geode reading view 走 `markdown.ts:550` markdown-it 默认 GFM table 规则，其 `escapedSplit` 按未转义 `|` 切分（`\|` 转义·**无代码 span 例外**）；编辑器模型若用别的切分会与渲染不一致→编辑可损坏。**契约（新 core 纯模块 + 新 editor 命令文件，无跨模块签名破坏）**：
> - **`core/tableEditor.ts`（新·纯·greenfield）**：
>   - `type TableOp = "insert-row-above"|"insert-row-below"|"insert-column-left"|"insert-column-right"|"delete-row"|"delete-column"`；`type ColumnAlign = "none"|"left"|"right"|"center"`；`interface TableModel { header: string[]; aligns: ColumnAlign[]; rows: string[][] }`（aligns.length===header.length·每 body 行规整到 header.length 单元格）；`interface CellPos { col: number; bodyRow: number }`（bodyRow=-1=表头/分隔行区）。
>   - **`applyTableOp(op, tableText, cursorOffsetInTable): { text: string; cursorOffsetInTable: number } | null`**（命令层单一入口·镜像 applyFormatOp）：parse→locate 光标 cell→应用 op→serialize→返回新表文本 + 新光标相对偏移（落在受影响 cell 内容首）。cursor 不在合法表格 / op no-op（删到 0 列 / 表头删行）→ null。
>   - 内部：`parseTable(text): TableModel | null`（行 0=表头·行 1=分隔[`:?-{1,}:?` 解析 align]·行 2+=body；**cell 切分镜像 markdown-it escapedSplit**[未转义 `|` 切·`\|`→字面 `|`]+ 去外缘空 cell + body 规整到列数；非合法表格→null）；`serializeTable(model, focus?): { text, cursorOffset }`（单空格填充 `| a | b |` + 分隔行按 align 出 `---`/`:--`/`--:`/`:-:`·focus cell 内容首偏移）；`locateCell(text, offset): CellPos | null`（行→row[行 1 分隔归 -1]·列=该行 offset 前未转义 `|` 数−含首管道）。
>   - 6 op 纯 model→model（col/bodyRow 出自 locateCell）：insert-row-above/below=在 body i / i+1 插空行；insert-column-left/right=在 col c / c+1 插空列（header+全 body+align "none"）；delete-row=删 body 行（表头区 no-op·删到 0 body 行仍合法[表头+分隔渲染为空表]）；delete-column=删列（仅 1 列→no-op·防 0 列非法）。
> - **`features/editor/tableCommands.ts`（新·同 features/editor/ 可 import liveTables）**：`registerTableCommands(app, getView)` 注册 6 命令（`id: editor:table-*`·`name: ()=>t(cmd.table*)`·`available: getView()!==null`·**无默认键**）。callback：取 view → cursor pos → **`findTableRanges(view.state)`（R55·Lezer Table 节点·fence 内是 FencedCode 非 Table→天然 code-skip）找含 cursor 的 range** → 无则 no-op → `tableText=doc.slice(from,to)`·`applyTableOp(op, tableText, cursor-from)` → null no-op → `view.dispatch({changes:{from,to,insert:result.text}, selection:{anchor: from+result.cursorOffsetInTable}, userEvent:"input.table"})` + focus。
> - **`app/App.tsx`**：mount `...registerTableCommands(app, () => getActiveFileEditorView(app)?.view ?? null)`（R33 命令组旁·active-file 门控）。
> - **`core/i18n/dict.app.ts`** +6 键 `cmd.tableInsertRowAbove/Below`/`cmd.tableInsertColumnLeft/Right`/`cmd.tableDeleteRow/Column`（en + zh·zh 用矩阵官方名「在上方新增行/在下方新增行/在左侧新增列/在右侧新增列/删除行/删除列」）。`main.tsx` +探针 `__geodeTableEdit.apply(op,text,offset)`（避与既有 `__geodeTable` 检测探针冲突）。
> - **分档：逻辑档（data-safety·editor 写路径·整表重写）**——applyTableOp 产 .md 字节（重写整表）。**字节安全**：纯字符串模型·**不读 syntaxTree**（检测在命令层用 vetted findTableRanges·只读 Table 节点名非动 mark·R201 规则允许）·cell 切分镜像 markdown-it（编辑器与渲染一致）·整表替换走 view.dispatch 单事务[undo 原子]·零新 vault 写路径·内容字节保留（仅 inter-cell 空白规整）。
> - **桌面 probe N/A 浏览器可验证**：applyTableOp 纯函数探针 + live CM dispatch + findTableRanges（R55 既有 probe 覆盖）browser-E2E 兜。
> - **As-built 加固（实现 + 评审揪出的字节安全）**：① **renderCell 重转义** `|`→`\|`（model 存裸内容[escapedSplit 折 `\|`→`|`]·写回不转义则 `| a|b |` 重解析为 2 cell=损坏）；② **parseTable 规整到最宽行**（ragged 行多出的 cell 绝不截断丢失·补 header/aligns 列·底线①）；③ **hasOverlayPipe 守卫**：任何 `|` 落在 Geode overlay（wikilink/embed `[[…]]`/`![[…]]`·inline code·inline math）内 → parseTable 返 null（命令 no-op）——阅读视图视 overlay 为原子（`[[Page|Alias]]` 渲染为单列·实证），escapedSplit 会切断它=损坏；math arm 对齐 markdown.ts:899（hug+非数字闭·`\\.` 吞转义·内容类排 `\` 防 `\$` 被拆=货币 `$5\|$10` 不误拦、`$a\$\|b$` 正确拦）；④ **缩进/列表内嵌表格保 indent**（findTableRanges 把 range.from 放在表头首管道[indent 在其前]·body/delim 行 indent 在 range 内·纯模型 flush-left 序列化→命令层把 indent 重补到每行+光标偏移 `indent.length*(L+1)`·否则 delim/body 失 indent→表格塌陷[1 table→0]=底线①改非光标行)。
> - **⚠️ 对抗评审结论（Ultracode 3-lens[byte-correctness/data-safety/fidelity-wiring] + skeptic synthesis · 初判 needs-fixes → 焦点复评 deliverable · 1 critical + 3 minor + 1 nit 全修 · 6 refuted）**：**1 critical 修**=缩进/列表内嵌表格编辑剥离 delim/body 行 indent→表格塌陷（live 复现 1 table→0·改非光标行·底线①）→命令层 indent 重补（见上④·复评实证光标落新 cell·table count 1→1）。**3 minor 修**=（a 实现自查）renderCell 管道重转义[否则 in-cell `|` 重解析损坏]·（b 实现自查）ragged 行截断丢 cell→改规整到最宽行·（c 评审）hasOverlayPipe 货币 `$5\|$10` 误拦[coarse math regex]+（复评）escaped-`$` 下 under-strip 撕裂 math cell→escape-aware regex 双修。**1 nit 修**=TableModel.rows 过期注释。**6 refuted**：escapedSplit 字节奇偶/光标 offset/boundary ops 全对·两处自查 fix 完备·整表重序列化改未编辑字节[markdown-it 渲染等价·非损坏]·blockquote `> ` 表格（findTableRanges 检出但 delim 行 `>` cell 令 parseAlign 失→null·safe decline）·tab 缩进表格（Lezer 不识别→no-op）·available 非表格内仍 enabled[同 R33 先例]。**根因教训（记忆扩展）**：(i) **「编辑器对齐阅读视图」扩到表格**——cell 切分须避开阅读视图视为原子的 overlay（wikilink/code/math 内 `|`），否则撕裂[R192/R201/R204/R205 同族]；(ii) **整块替换 [from,to] 时，块外/逐行前缀（indent/`> `）必须保留**——findTableRanges 的 range.from 在表头首管道之后，flush-left 序列化会丢后续行的 indent。
> - **r206-e2e 52/52**（byte 6 ops + 光标 offset + no-op 守卫[表头删行/单列删列/非表格] + **overlay-pipe 守卫[body `[[a|b]]` 撕裂防护/header/code/math/escaped-`$`/货币可编辑/pipe-less `[[Page]]` 可编辑保留]** + **ragged 不丢 cell** + escape round-trip + render PROOF[含 escaped cell 不损坏] + 注册无键 + live[insert-row + **缩进/列表内嵌表保 indent·table count 1→1**] + blockquote safe decline）+回归 **r33 37/37·r55 15/15·r198 34/34**·typecheck 0/cargo/生产构建。**v1 偏差/defer**：片 B move/align/copy 9 命令；序列化单空格填充[非列宽 pretty-format·渲染等价·CJK 显示宽]；含 overlay-pipe 的表格 no-op（overlay-aware 编辑=follow-on）；blockquote/tab 缩进表格 no-op（safe）。

> **状态：As-built（已交付）。** 表面复刻 G 系列——按 R182 矩阵收割 §2 `missing`「插入表格」（Obsidian `editor:insert-table`「Insert table」），亦即 §17 表格组的**入口片**（骨架插入；列/行编辑 ~15 命令仍 `missing`·整片立项另计）。web 实证（obsidian help/forum）：Obsidian「Insert table」插**2×2 表格**（表头 + 1 数据行·2 列）·**空单元格**·**光标落首表头格**·**无默认键**。**契约（扩 R33 纯引擎 + 1 命令，无跨模块签名破坏）**：
> - **`core/format.ts`**：`FormatOp` += `"table"`（**按内容命名**·动词只留在命令 id·镜像兄弟 `inline-math`/`math-block`/`horizontal-rule`）。`insertTable(text, _from, to): FormatEdit` 纯函数——**不消选区**（塌缩到 `to`=底线①·镜像 `insertHorizontalRule`）·`lead = (to===0||text[to-1]==="\n") ? "" : "\n"`（非行首才补前导）·**`trail = (to<text.length && text[to]!=="\n") ? "\n" : ""`（尾部 fence）**·骨架 = `|     |     |\n| --- | --- |\n|     |     |\n`（5 宽格对齐 `---`·markdown-it 裁剪空白）·光标 = `to + lead.length + 2`（首表头格内·`| ` 之后）。`applyFormatOp` switch +`case "table"`（`never` 穷尽守卫强制补 case）。
> - **`features/editor/formatCommands.ts`**：`FORMAT_COMMANDS` +1 条 `{ id:"editor:insert-table", nameKey:"cmd.insertTable", op:"table" }`（无默认键·Obsidian「未设置」）。注册逻辑（`registerFormatCommands`·`available:getView()!==null`·`callback` 调 `applyFormat` 设 `selFrom/selTo` 选区 + `view.focus()`）原样复用，零改。
> - **`core/i18n/dict.app.ts`** +1 键 `cmd.insertTable`（en `Insert table` / zh `插入表格`·官方本地化）。**probe 无需改**——`__geodeFormat.apply(op,text,from,to)` 通用覆盖任意 `FormatOp`（footnote 单挂钩仅因其是双址 `FootnoteAction` 非单段 `FormatEdit`；table 走 `applyFormatOp` 单段插入，自动覆盖）。
> - **分档：逻辑档（data-safety·碰 `core/format.ts` editor 写路径=底线①一票否决）**——insertTable 产 .md 字节。**字节安全**：纯字符串插入·**不读 syntaxTree/overlay**（非 R192/R197/R201/R204 Lezer 损坏族·data-safety lens grep 实证）·不消选区（塌缩到 `to`）·零新 vault 写路径（走正常 CM transaction → documents dirty → autosave，继承 B 类写保护）。
> - **桌面 probe N/A 浏览器可验证**：纯函数 `__geodeFormat.apply` 探针 + live CM `view.dispatch` browser-E2E 兜（同 R198 既定「format 引擎 browser-E2E」）。**v1 偏差/defer**：§17 列/行编辑命令（复制/删除/移动/对齐/增删行列 ~15 条·依赖表格编辑器·整片立项）；骨架固定 2×2 空表（同 Obsidian·无「指定行列数」入口=Obsidian 亦无）；游标在既有表格内插表 = 嵌套骨架（同 Obsidian·纯插入不检测既有表·非损坏）。
> - **⚠️ 对抗评审结论（Ultracode 3 lens[byte-offset / data-safety / fidelity-contract] + skeptic synthesis · 初判 needs-fixes → 修后 deliverable · 1 major + 1 minor + 1 nit 全修 · 1 refuted）**：byte-offset/data-safety lens **0 finding**（独立复算全 offset：`lead`/`trail`/`cell=to+lead+2` 对 `lead=""`与`"\n"` 皆落首格·选区不消·exhaustiveness 守卫在位·零 syntaxTree 读·零新 vault 写）；fidelity lens 出 4 raw。
>   - **1 major 已修（fidelity·阅读视图保真）**：insertTable 原仅镜像 `insertHorizontalRule` 的**单尾 `\n`**，但 **GFM 表格非自终止**（不同于自终止的 `<hr>`：孤立 `***\n` 行即 `<hr>`、下一行另起段落）——markdown-it **贪婪吸收下一非空行为表格体行**。在已有文本行**上方**插表（光标行首·`text[to-1]==="\n"` 故无 lead·且 `text[to]` 非空）→ 下方段落被并入表格成第 4 个 `<td>`（reviewer 经仓库**真实 markdown-it** 复现：`'para'+SKEL+'para two'` → `<td>para two</td>`）。**字节/选区不丢（底线①未破·无数据丢失）**·但阅读视图保真破·「在已有段落上方插表」高频可达。修 = **对称尾部 fence**（`trail`·见上契约）·光标 offset 不受影响（trail 在插入内容之后）。
>   - **1 minor 已修（测试盲区·正是漏掉 major 的根因）**：r205-e2e 的 render PROOF 原仅渲染**孤立 SKEL**（平凡 2 th/2 td）·从未把**并接文档**回喂 `__geodeRenderMarkdown` → 漏检吸收。修 = +**D2 render PROOF**（把「上方插表」的真实产出字节回喂渲染·断言恰 2 td + `below` 独立 `<p>` · **CONTRAST** 反证无 fence 的 `SKEL+below` 会被吸收=fence load-bearing）。
>   - **1 nit 已修（一致性）**：op `insert-table` 重复了兄弟 op 丢弃的动词前缀 → 改 op 名为 `table`（命令 id `editor:insert-table` 不变·纯内部·穷尽 switch + e2e 全覆盖·零风险）。
>   - **1 refuted**：「有意 Obsidian 偏差是缺陷」= 无动作确认（2×2 空格·首格光标·无默认键·en+zh 齐·单 `\n` lead 渲染正确·5 宽格被裁剪——皆正确；真正缺陷是尾 `\n` 非这些）。
> - **r205-e2e 32/32**（byte[空/行尾/空行/行中/**选区不消** + **尾 fence: 上方插表/行下有文/已空行不重复/文末不补**] + skeleton shape[2 列·分隔行·空格格] + **render PROOF[孤立 SKEL=`<table>`+2 th+td · D2 并接=尾 fence 阻吸收恰 2 td+独立 `<p>` · CONTRAST 反证]** + 注册无默认键 + live[exec 插表·前文逐字保留·首格光标 head=13]）+回归 **r33 37/37·r198 34/34**·typecheck 0/cargo/生产构建。

## Round 204 additions — G3 §2「重命名当前小标题」片 A（1 命令 editor:rename-heading · 改 heading 文字 + 同文件自锚 `[[#OldH]]`→`[[#NewH]]` 更新 · core/renameHeading.ts 纯函数 + window.prompt · data-safety 逻辑档[byte 风险·链接改写] + 零新依赖）【As-built v0.200】

> **状态：As-built（已交付）。** 表面复刻 G 系列——按 R182 矩阵收割 §2 `missing`「重命名当前小标题」（Obsidian `editor:rename-heading`「Rename this heading…」）。**忠实基线 + 局部增强**：web 实证（Obsidian dev Licat 明确表态）**Obsidian 核心 rename-heading 不更新任何 heading 链接引用**（`[[#Section]]` 静默失效·不会自动更新·正式不实现）→ 忠实基线 = 改 heading 文字；**Geode 片 A 加同文件自锚更新**（同一文档内 `[[#OldH]]` 自引用跟随·明显合理的局部完整性增强）。**跨档 `[[File#OldH]]` 改写 = 片 B（R16-重·高 data-safety·字节套件重建·下一轮）defer**（与 Obsidian 核心同样不做·非回退）。**契约（新 core 纯函数 + 命令，无跨模块签名破坏）**：
> - **`core/renameHeading.ts`（新·纯）**：`renameHeadingAt(text, cursorPos, newText): { changes: Array<{from,to,insert}>, selFrom, selTo } | null`——① 光标所在行测 `/^(#{1,6})\s+(.+)$/`（非 heading 行→null）；② oldText=组 2 trim·newTrim=newText.trim()·`!newTrim||newTrim===oldText`→null（no-op）；③ changes=[替 heading 文字段（`### ` marker 后）为 newTrim]；④ **自锚更新仅当 newTrim link-safe（无 `|[]#`）**：扫全文 `(!?)\[\[#([^[\]|#]+?)(\|[^[\]]*)?\]\]` 自锚 wikilink·每个 `stripHeadingText(子路径)===stripHeadingText(oldText)`（复用 metadata.ts 导出·镜像 resolveSubpath 匹配语义·大小写/markdown/空白不敏感）→ 替组 2 子路径为 newTrim（保 `!` 前缀 + `|alias`）·**跳过落在 heading 文字 change 区内的自锚（防 overlap）**；⑤ selFrom/selTo=改名后 heading 文字末。changes 按 from 排序、非重叠。
> - **`features/editor/formatCommands.ts`**：注册 `editor:rename-heading`（无默认键·`available: getView()!==null`）。callback：取光标 → **先测是否在 heading 行**（不在→no-op·不弹 prompt）→ `window.prompt(t("editor.renameHeadingPrompt"), currentText)`（复用 TagsPanel:158/BookmarksPanel:155 vetted 先例·零新模态）→ 结果非空 → `renameHeadingAt` → `view.dispatch({changes, selection})` 单事务（undo 原子）→ `view.focus()`。
> - **`core/i18n/dict.app.ts`** +2 键 `cmd.renameHeading`（Rename heading… / 重命名当前小标题…）+ `editor.renameHeadingPrompt`（New heading text: / 新的小标题文字：）。`main.tsx` +探针 `__geodeRenameHeading(text,cursor,newText)`。
> - **分档：逻辑档（data-safety·byte 风险·链接改写）**——renameHeadingAt 产 .md 字节（改 heading + 自锚链接）。**字节安全**：纯单文件 string ops、走 view.dispatch 单事务 undo 原子、不碰 vault/documents/跨档写路径；自锚改写只换组 2 子路径（保 `!`/alias/区外字节）·link-unsafe newTrim 跳过自锚（不产破损链接·heading 仍改）·overlap 守卫（自锚在 heading 区内跳过）。
> - **As-built 校准（契约简化）**：`renameHeadingAt` 返**裸 `HeadingChange[] | null`**（非 `{changes,selFrom,selTo}`）；命令 `view.dispatch({changes, scrollIntoView, userEvent})`（**无显式 selection·CM 自动映射光标**穿过 changes·更鲁棒省手算 offset）。
> - **桌面 probe N/A 浏览器可验证**：renameHeadingAt 纯函数探针 + live CM dispatch browser-E2E 兜；`window.prompt` 在 WKWebView 偶返 null（三处 vetted 先例既定·desktop 跑一次确认）。**v1 偏差/defer**：跨档 `[[File#OldH]]` 改写=片 B（下一轮·与 Obsidian 核心同样不做）；markdown 自锚 `[x](#OldH)` defer（仅 wikilink 自锚）；link-unsafe newText 跳过自锚更新（heading 仍改·自锚破损同 Obsidian）；重复同名 heading 时自锚全改向 newTrim（固有歧义·同 resolveSubpath 首匹配）。
> - **⚠️ 对抗评审结论（Ultracode 3 lens[byte-correctness/data-safety-corruption/fidelity-wiring] + skeptic synthesis · verdict=deliverable · 0 critical/0 major）**：byte-correctness lens **建独立 harness 复算全 offset**（textFrom=lineStart+(m[0].length−m[2].length)·subFrom=from+a[1].length+3 含 embed `!` 偏移·alias/区外字节保·overlap 守卫半开区间·changes 排序非重叠 CM 不抛·CRLF 经 vault.normalizeContent →LF）；data-safety lens **grep 实证 zero syntaxTree 读**（非 R192/R197 损坏族）·重复同名 heading=resolveSubpath 首匹配歧义非损坏·link-unsafe 守卫不写破链。
>   - **1 minor 已修**：**renameHeadingAt 原扫 raw text 无 maskCodeRegions** → 码内字面 `[[#Old]]` 被改写、fence 内 `## x` 误判为 heading（背离阅读视图 + 全仓 scanner 的 R68/R125/R127「detector 须对齐 reading-view」纪律 + R201 code-context 教训）→ 修=heading 检测 + 自锚扫描均跑 `maskCodeRegions(text)`（同长 blank·offset 对齐·复用 metadata 既有，与 resolveSubpath/unlinkedMentions 同源），改动应用于 raw·r204-e2e +inline-code/fence 自锚不改 + fenced `## x` no-op 断言。
>   - **2 nit 已修**：① newText 含换行无守卫（window.prompt 单行不可达·但探针可达）→ `replace(/[\r\n]+/g," ")` 折叠（heading/子路径本单行）+ e2e 断言；② 块引用注释不精确·`[^[\]|#]` 不排 `^` → 加 `a[2].startsWith("^")` 显式跳块引用 `[[#^id]]`（即便 heading 字面名 `^abc` 亦不误改块引用）+ e2e 断言。**1 nit（契约漂移）已上「As-built 校准」节调和。**
>   - **5 refuted**：重复同名过匹配（固有歧义·valid link）/跨档 defer（片 B·Obsidian 同）/window.prompt vs inline popover（UI 打磨非 bug）/overlap 抛（守卫已挡）/缺 zh 键（en+zh 齐）。
> - **r204-e2e 23/23**（byte renameHeadingAt[plain/自锚/大小写/alias/embed/markdown-heading/link-unsafe 守卫/跨档不改/非匹配/**inline-code 不改**/**fence 不改**/**fenced `## x` no-op**/**块引用 `^` 跳**/**换行折叠**/非 heading/空/不变] + 注册无键 + live[prompt→改 heading+自锚单事务·undoable·非 heading 不弹]）+回归 r33 37/37·r192 28/28·typecheck 0/cargo/生产构建。

> **状态：As-built（已交付）。** 表面复刻 G 系列——审计点名 `管理仓库`/`切换仓库`（C1）首片。**bounded + 浏览器可验证**（explorer 实证：唯一桌面-only 是 `pickVaultFolder()` 原生对话框；切到**已知最近路径**不需对话框——`openVaultFlow` 函数体除首行 pickVaultFolder 外就是接受路径字符串的切库全序）。**片边界**：最近仓库列表的纯前端逻辑（store CRUD + 切换器 UI + 命令）做掉，「弹原生对话框选新文件夹」留给既有 `openVaultFlow`（桌面）。**契约（新 core store + App.tsx 重构/加性，无跨模块签名破坏）**：
> - **`core/recentVaults.ts`（新·纯·镜像 commandMru.ts）**：键 `geode.recentVaults`、全局 `string[]`（绝对路径·非 per-vault）·上限 10。`loadRecentVaults(): string[]`（防御式解析：坏 JSON/非数组/非 string/去重 → `[]`）/ `pushRecentVault(path)`（dedupe→unshift→cap·best-effort try/catch）/ `removeRecentVault(path)`。**零 fs·纯 localStorage**（移除仅删列表项·绝不碰 vault 数据=硬边界#3）。
> - **`core/types.ts`**：`ModalKind` += `"vaultswitcher"`。
> - **`app/App.tsx`**：① **重构** `openVaultFlow` body（1873 flushAll 起至插件重载）抽成 `switchToVault(app, picked: string)`——**逐字保留切库全序**：`flushAll`（防旧库未存丢/late-save 覆盖新库）→ `lastActiveFile.set(null)`（防同名文件跨库串）→ `setVaultPath` → `localStorage LAST_VAULT_KEY` → `await vault.load()` → **`pushRecentVault(picked)`（仅 load 成功后）** → `closeMissingFileTabs` → loadExternal + loadObsidianPlugins；`openVaultFlow` = `pick → if(!picked)return → switchToVault`。② **`VaultSwitcherModal`（inline·镜像 WorkspacesModal chrome）**：`useState(loadRecentVaults())` 列表（basename 显示名）·点项 → `try{switchToVault;close()}catch{removeRecentVault+刷新}`（坏路径剔除）·移除按钮 → `removeRecentVault`+setVaults·「打开其他仓库」→ openVaultFlow。modal-overlay 点击关 + Escape 全局关。③ 渲染 `{ws.modal==="vaultswitcher" && <VaultSwitcherModal/>}`。④ 命令 `app:switch-vault`（**不门控 isTauri**·浏览器要测）→ openModal("vaultswitcher")。
> - **`main.tsx`**：boot `last` 分支 `await vault.load()` 成功后 `pushRecentVault(last)`（首库进列表）。
> - **`core/i18n/dict.app.ts`** +键 en+zh：`cmd.switchVault`/`vaultSwitcher.title`/`vaultSwitcher.empty`/`vaultSwitcher.openOther`/`vaultSwitcher.remove`。
> - **分档：逻辑档（data-safety·vault 切换红线）**——switchToVault 触 vault.load/flushAll/setVaultPath。**字节安全**：switchToVault 逐字保留 openVaultFlow 既有切库纪律（flushAll→…全序·少一步即踩历轮根因[late-save 覆盖新库/同名跨库串]）·仅加性 pushRecentVault（load 成功后·纯 localStorage）；移除最近仅删列表项零 fs；坏路径 catch 剔除（匹配 boot main.tsx:1433 既有 removeItem 先例）。
> - **桌面 probe N/A 主体浏览器可验证**：store CRUD + 模态渲染 + 命令注册 + switchToVault 不抛（memory adapter setVaultPath no-op·load 重载同内存库）browser-E2E 兜；真实 fs 切库内容变 桌面 probe（既有 openVaultFlow 序·未改）。**v1 偏差/defer**：浏览器 MemoryVault 无多库语义（切换不换内容·测的是 store/UI/命令）；坏 recent 路径切换后中断态恢复同 openVaultFlow 既有（用户重选·v1 不回滚 adapter）；VaultPicker 空态加最近列表 defer（本片只做模态）；`app:open-vault`（弹窗选新库）不动。
> - **⚠️ 对抗评审结论（Ultracode 3 lens[switchToVault-datasafety-sequence/recentVaults-store-boundary3/modal-wiring-fidelity] + skeptic synthesis · verdict=deliverable · 0 critical/0 major/**0 confirmed**）**：lens **git diff 实证 switchToVault body 对旧 openVaultFlow 逐字节一致·唯一插入 `pushRecentVault(picked)` 于 load 成功后**（flushAll 仍 FIRST·先于 setVaultPath=A.2 防晚存覆盖新库；lastActiveFile.set(null) 防同名跨库串；坏 load 不入 recents）；**recentVaults.ts 零 import·removeRecentVault 只写 localStorage=硬边界#3 satisfied**（remove 绝不触 fs/vault）；防御式解析逐字镜像 commandMru.loadIds（坏 JSON/非数组/非 string/去重）；wiring 全（命令 ungated·ModalKind·render·i18n en+zh·boot push·probe）；模态生命周期净（close 先于异步 switch·无 setState-on-unmount）。
>   - **3 raw findings 全 refuted**：① 坏 recent 路径切换后不回滚 adapter（2 lens 同根因）= **pre-existing openVaultFlow 既有行为·非新回归·flushAll 先行无数据丢失·桌面 only·重启恢复**（记 v1 限制·非阻塞）；② r203-e2e test-D click 路径欠验 push-to-front = 测试质量 nit（产品码 pushRecentVault 在位且 load 后·section A 已验 push/dedupe/cap）→ **已收紧**（seed [Delta,Gamma]·点旧的 Gamma·断言 → [Gamma,Delta] 移前+保留 Delta）。
>   - **v1 已知限制（OBSIDIAN-COMPAT 记）**：坏 recent 路径切换中断态不回滚（同 openVaultFlow 既有·无数据丢失·重启恢复）；浏览器无多库语义；basename 仅 split "/"（Windows `\` 路径全显·pre-existing 共享 util·非本轮）。

> **状态：As-built（已交付）。** 表面复刻 G 系列——续 R201 收割 §4 末可达项 `editor:open-link-in-new-split`（⌘⌥Enter「在新标签组中打开光标处链接」/ Obsidian「Open link under cursor to the right」missing→done）。`open-link-in-new-window`（多窗口）仍 defer。**bounded·复用 R201 全部基建**（linkAtCursor + openWikilink/resolveMarkdownLink/window.open）+ vetted `splitActivePane("row")`（app:split-right 同源）。**契约（重构 R201 followLinkAtCursor + 1 命令，无跨模块签名破坏）**：
> - **`features/editor/linkCommands.ts`** `followLinkAtCursor` 第 4 参 `newTab: boolean` → `mode: "current" | "tab" | "split"`（`newTab = mode==="tab"`；R201 两命令改传 mode "current"/"tab"，命令行为不变）。**split 编排**：从**原 view**（split 前）取 linkAtCursor + code-context 守卫 → 按 kind 开：**wikilink** → `if(mode==="split") splitActivePane("row")`（新窗格 duplicate 当前 tab 并置为 active）→ `openWikilink(…, {newTab})`（开进 active=新 split 窗格·不消选区）；**markdown external** → `window.open`（无 split/tab）；**markdown internal** → `resolveMarkdownLink` 先解析，**unresolved→return false（不 split·避免残留空窗格）**，resolved→`if(mode==="split") splitActivePane("row")`→`openFile(resolved,{newTab})`+#reveal。`splitActivePane` 返 null（如 graph tab）则不 split、退回当前窗格开（优雅降级）。+1 命令 spec（`editor:open-link-in-new-split`·hotkey `Mod+Alt+Enter`·mode "split"）。
> - **`core/i18n/dict.app.ts`** +1 键 `cmd.openLinkNewSplit`（Open link under cursor to the right / 在新标签组中打开光标处链接·与 Obsidian 官方本地化一致）。
> - **hotkey**：`Mod+Alt+Enter`（⌘⌥Enter）经 Prec.highest handleKeydown 精确模位比对·不撞 R201 的 Alt+Enter/Mod+Enter（多 Mod 或 Alt 之一不同即不同键）·不破裸 Enter。
> - **分档：逻辑档（data-safety）**——续 R201（openWikilink 可 createNewNote·vault 写 vetted uniquePath）。**字节安全**：零新写机制（复用 R201 全部 vetted 路径 + splitActivePane 纯 workspace 树操作无 vault 写）；split 不改 .md；markdown unresolved 先判后 split 不留空窗格。
> - **桌面 probe N/A**：导航 + split（同 R183/R201 既定 browser-E2E）。**v1 偏差/defer**：§4 余 `open-link-in-new-window`（多窗口未支持）；split 方向固定 "row"（向右·Obsidian 默认）；external 链接 split 模式仍 window.open（无 Geode 窗格意义）。
> - **⚠️ 对抗评审结论（Ultracode 3 lens[split-composition/refactor-regression-datasafety/fidelity-wiring] + skeptic synthesis · verdict=deliverable · 0 critical/0 major）**：lens 追进 workspace.ts 证 **splitActivePane dup tab → openFile `replace-in-place`(ws.ts:409) RETARGET 单 tab·无残留 dup/双开/错窗格**、newTab=（mode==="tab"）current/tab 与 R201 逐字节等价（r201 29/29）、code-context 守卫读 pre-split 原 view、零新写路径（splitActivePane 纯 workspace 树操作）、热键 Mod+Alt+Enter 无撞、en/zh 标签与 Obsidian 官方一致。
>   - **1 minor 已修**：split 模式下 degenerate `[[#]]`/`[[#|x]]`（空 target+空 subpath）wikilink 分支**无条件 split** 后 openWikilink no-op → 残留 dup 窗格（markdown/external 免疫——先 resolve/判后 split）→ 修=wikilink 分支 split **前**加 `if(target===""&&!subpath) return false`（镜像 openWikilink 自身 no-op·current/tab 字节等价[仅未用 bool 返回翻转]），r202-e2e +`[[#]]` split 无残留窗格断言。
>   - **2 nit（不改·R201 fidelity 已反驳）**：① 「热键非 Obsidian stock」——**R201 fidelity lens 经 keycheck.dev/obsidian-translations 已实证** Alt+Enter/Mod+Enter/Cmd+Opt+Enter 即 Obsidian 默认键、split=Cmd+Opt+Enter，故保留；② 「zh 标签缺『向右』」——zh `在新标签组中打开光标处链接` **是 Obsidian 官方 zh-CN 字符串**（R201 实证），加『右侧』反偏离官方，故保留。
>   - **6 refuted**：new-window defer（v1）/ self-link `[[#h]]` split 开源笔记副本（Obsidian 忠实非残留）/ 重构无回归 / 无双开错窗格 / 热键无撞 / split 无新写路径。
> - **r202-e2e 12/12**（注册 Mod+Alt+Enter + live[split wikilink→新窗格+B202·split markdown→新窗格+C202·split subpath·external→浏览器无 split·inline-code 守卫 no-op 无 split 无幻影·**degenerate `[[#]]` 无残留窗格**]）+回归 r201 29/29（重构内部·命令行为不变）·r71 17/17·typecheck 0/cargo/生产构建。

> **状态：As-built（已交付）。** 表面复刻 G 系列——按 R182 矩阵收割 §4 链接导航：`editor:follow-link`（⌥Enter「打开光标处链接」partial→done）+ `editor:open-link-in-new-leaf`（⌘Enter「在新标签页打开」missing→done）。**bounded 高确定性**（真 Obsidian 核心命令 · 纯导航**零新 vault 写路径**=复用 vetted openWikilink[含 unresolved→createNewNote vetted 分支]/resolveMarkdownLink/window.open · openFile 已支持 newTab）。**契约（新 core 纯函数 + 扩 wikilinks.ts + 新 editor 命令文件）**：
> - **`core/linkAtCursor.ts`（新·纯）**：`linkAtCursor(line, lineStart, cursor): LinkAtCursor | null`——扫光标所在行的 wikilink `(!?)\[\[([^\[\]]+?)\]\]` 与 markdown `\[[^\]]*\]\(([^)]+)\)`，返**含光标偏移**（`from<=cursor<=to`，取首个）的那条：`{ kind:"wikilink"|"markdown", target, subpath, from, to, external }`。wikilink：body `split("|")[0]`（去 alias）→ 首 `#` 前=target、后=subpath、external=false。markdown：href 首 `#` 前=target、后=subpath、external=`/^(?:[a-z][a-z0-9+.-]*:\/\/|mailto:|tel:)/i` 测 href。
> - **`features/editor/wikilinks.ts`**：`openWikilink(app, target, fromPath, subpath?, opts?: { newTab?: boolean })`——加可选 5 参，把 `opts?.newTab` 透传到 3 处 `openFile`（resolved/self-link/created）。**向后兼容**（既有 8 调用点不传 opts→newTab undefined→原行为）。
> - **`features/editor/linkCommands.ts`（新）**：`followLinkAtCursor(app, view, fromPath, newTab): boolean`——取光标 → `linkAtCursor`（无则 false）→ 分支：**wikilink** → `openWikilink(…, {newTab})`；**markdown external** → `window.open(target+(subpath?"#"+subpath:""), "_blank", "noopener")`（Tauri opener 路由 OS 浏览器·复用 livePreview:1343 vetted·new-leaf 对外链 N/A 仍开浏览器）；**markdown internal** → `metadata.resolveMarkdownLink(target, fromPath)` → `openFile(resolved, {newTab})` + `resolveSubpath` 命中则 `requestReveal`。`registerLinkCommands(app, getActive)` 注册 2 命令（hotkey `Alt+Enter` / `Mod+Enter`·`available: getActive()!==null`）。
> - **`app/App.tsx`**：`...registerLinkCommands(app, () => getActiveFileEditorView(app))`（同 registerFormatCommands 旁·getActive 返 `{view,path}` 活动文件视图[R23 DS-1 active-file]）。
> - **`core/i18n/dict.app.ts`** +2 键 `cmd.followLink`（Follow link under cursor / 打开光标处链接）+ `cmd.openLinkNewLeaf`（Open link under cursor in new tab / 在新标签页打开光标处链接）。`main.tsx` +探针 `__geodeLinkAtCursor`。
> - **hotkey 安全**：命令热键经 `app.commands.handleKeydown` 在 `Prec.highest`（cmExtensions.ts:613）先于 CM keymap 拦截；`Alt+Enter`/`Mod+Enter` ≠ 裸 `Enter`，**不破换行**（`commands.ts:229` 解析 `Enter`；全仓无既有 Mod/Alt+Enter 绑定，无冲突）。
> - **分档：逻辑档（data-safety）**——触 editor + openWikilink 可 createNewNote（vault 写）。**字节安全**：**零新写机制**（全复用 vetted openWikilink/createNewNote[既有 Ctrl+Click 路径]/openFile/window.open）；导航主体只读；createNewNote 仅 unresolved wikilink（Obsidian 忠实·R23 active-file 门控）。
> - **桌面 probe N/A**：导航 + window.open（同 R183 既定「编辑器导航 browser-E2E」）；`linkAtCursor` 纯函数浏览器侧探针直驱。**v1 偏差/defer**：§4 余 `open-link-in-new-split`（⌘⌥Enter「在新标签组中打开」·需 splitLeaf+openFile 组合）/`open-link-in-new-window`（⌘⌥⇧Enter·多窗口未支持）defer；markdown unresolved internal 不 create（仅 wikilink create·更安全）；光标恰在两相邻链接边界取首条。
> - **⚠️ 对抗评审结论（Ultracode 4 lens[linkAtCursor-bytes/data-safety-navigation/fidelity/wiring-hotkey] + skeptic synthesis · verdict=deliverable · 0 critical/0 major）**：lens 实证——scan 全边界正确（含界判定/alias-drop/embed `!`/external 检测）、**追进 metadata.ts 证 `javascript:`/`data:` 无 `://` → 非 external → resolveMarkdownLink 经 normalizeMdHref scheme 守卫返 null → no-op（无 window.open 无执行）**、openWikilink 加 opts 向后兼容（openFile `opts.newTab??false` · 8 调用点 newTab=undefined 不变·r71 17/17）、热键 `Alt+Enter`/`Mod+Enter` 经 matchParsedHotkey 精确模位比对**不破裸 Enter/IME/补全接受**、en+zh 标签**与 Obsidian 官方本地化逐字一致**。
>   - **2 minor 已修**：① **`linkAtCursor` 无 inline-code/fence 感知** → 跟随 `` `[[X]]` ``/fence 内字面链接会导航、unresolved wikilink 还会 **createNewNote 写幻影笔记**（背离 Geode 阅读视图 markdown.ts:440/465-468 跳码 + Obsidian）→ 修=`followLinkAtCursor` 加 **Lezer code-context 守卫**（`syntaxTree.resolveInner(pos,-1)` 祖先名含 `Code`[InlineCode/FencedCode/CodeBlock/CodeText] 即 false·**只读节点名安全·非动 emphasis mark·R192/R197 规则允许**），r201-e2e +inline/fence live no-op + 无幻影笔记断言；② **`MDLINK_RE` href `[^)]+` 截断含平衡括号 URL**（`…/Foo_(bar)` → 错页·背离 markdown-it）→ 修=`(?:[^()]|\([^()]*\))+` 容一层平衡括号 + byte 断言。
>   - **1 nit 记录（不修）**：`Mod+Enter` 经 Prec.highest 命令拦截**遮蔽 CM defaultKeymap 的 insertBlankLine**——有意为之（同 editorMotionCommands 遮蔽 Alt-Arrow 先例·`editor:insert-blank-line` 仍在仅无默认键·Obsidian 不绑 Cmd+Enter 到 insertBlankLine），记此防误判回归。
>   - **3 refuted**：`javascript:` scheme（安全·非 external 路由 no-op）/ openWikilink 向后兼容回归（newTab=undefined 不变）/ 跟随真 unresolved `[[X]]` 创建 X（Obsidian 忠实非缺陷）。
> - **r201-e2e 29/29**（byte linkAtCursor[wikilink/subpath/alias/embed/self-link/markdown/external/external+frag/**平衡括号 URL**/无链接/双链接/边界] + 注册 Alt+Enter & Mod+Enter + live[no-op/follow 同 tab/new-leaf 2 tab/subpath/markdown internal/external window.open + **inline-code/fence 守卫 no-op + 无幻影笔记**]）+ 回归 r71 17/17·typecheck 0/cargo/生产构建。
...
> - **`features/editor/formatCommands.ts`** 新增 `clearFormatting(view)`（CM 视图级、非纯 op——需 syntaxTree）：iterate 选区内 syntax tree，对 **`Emphasis`/`StrongEmphasis`/`Strikethrough`/`InlineCode`** 包裹节点，**仅当节点完整包含于选区（`from>=sel.from && to<=sel.to`）**才删其直接子 mark（`EmphasisMark`/`StrikethroughMark`/`CodeMark`）——**部分选中→跳过、绝不留半个标记（防 unbalance 损坏）**；removals 排序后单事务 `changes` 删除（CM 自动映射选区到清理后文本）；removals 空→返 false 不派发。经既有 registerFormatCommands 注册（available=active-file view、callback→view.dispatch=既有 dirty→autosave，**无新写路径**、R23 只写 active-file）。i18n dict.app +1 键 en+zh。
> - **⚠️⚠️ 本轮根因教训（CRITICAL ×2 · 字节安全核心）**：**裸 syntaxTree（Lezer GFM 树）≠ Geode 的实际 literal 结构**。Lezer 只认 GFM，**不认识 Geode 在其上加的正则 overlay 扩展**（wikilink/embed/alias、inline+block math、frontmatter、`%%comment%%`）；在这些结构内，Lezer 把**字面** `**`/`~~` 误判为真 Emphasis/Strikethrough → 裸剥除会**损坏链接目标 / 数学公式 / frontmatter 元数据 / 注释体**（底线①违反）。如 `[[**x**]]`→`[[x]]`=指向不同笔记、`$a*b*c$`→`$abc$`=数学乘法损坏。**修复 = `clearFormatting` 删 mark 前过 overlay 排除**：① **wikilink/embed/alias** 经 `hasLinkAncestor(node)`（祖先链含 Lezer `Link`/`Image` 即跳过——这些 Geode 把目标/别名当 literal）；② **math/frontmatter/comment** 经 `protectedSpans(state,from,to)`：frontmatter `---…---` 块 + 复用 vetted `findMathBlockRanges`（块 math）+ `commentSpans`（`%%…%%` 单行/跨行块/EOF，**fence 感知**：fenced code 内孤立 `%%` 不开注释，否则 over-protect 全文）+ inline math `INLINE_MATH_RE`；每个候选 mark 经 `inProtected` 相交过滤丢弃。**方法论**：overlay 权威清单 = reading-view pre-pass 处理的全部 literal 结构（wikilink + comment + math + frontmatter；fenced/indented code + link-ref 由 Lezer 天然不解析 emphasis 而安全）——补 overlay 排除须**对照该清单逐项确认**，勿临时枚举（本轮第一次修漏了 comment，第二轮才补全）。
> - **字节安全不变量（守住）**：① 只删真 mark token（非字面字符）；② 过保护是安全方向（**under-removal=漏删、绝不 corruption**）；③ 部分选中绝不残半标记。**对抗评审实证**：reviewer 真驱动命令把副字面置各 overlay + 真键执行，逐字节核对 28 e2e 场景（含 7 wikilink/math/frontmatter/embed/alias + 3 comment + fence-内孤立 `%%` + 代码跨距字面 `*` 保 + 部分选中 + 嵌套 + no-op）。
> - **分档：逻辑档（data-safety）**——formatCommands.ts=编辑器写路径，跑 data-safety skill + byte 回归 r33 37/37 + r192 28/28 字节级 live-view e2e。
> - **v1 已知 honest gap（均 under-removal=安全方向、非 corruption，记录在案）**：① **highlight `==` 不剥除**——`==` 非 `*`/`~`/`` ` ``，Lezer 不误判为 emphasis（**非 corruption 源**，仅未实现剥除 `==` 自身 = feature gap）；highlight 内的真 `**bold**` 仍被正确清除。② `INLINE_MATH_RE` 过匹配 currency/散文 `$`（`$5…$10` 间会 over-protect 真 emphasis = 漏删，罕见、安全方向；`scanInlineMath` 未导出故未复用）。其余 missing 纯编辑（删除段落/添加别名·标签/清除笔记属性）；`partial` 余（panel「新标签页」变体[需主区视图]/theme:switch）；`管理仓库` C1。
...
> - **`SettingsModal.tsx` `SectionId`** 由 5 段扩为 **12 段**：`about | editor | files-and-links | appearance | hotkeys | keychain | plugins | command-palette | templates | daily-notes | unique-notes | page-preview`。
> - **导航 `SECTIONS` → `NAV_GROUPS`**（数组 of `{titleKey, testid, items[]}`），渲染时每组先出一个 `.settings-nav-title.settings-nav-subtitle`（带 `data-testid={group.testid}`，复用 R163 既有 class，**零新 CSS**）：① **选项**（`settings-navgroup-options`）= about/editor/files-and-links/appearance/hotkeys/keychain/plugins；② **核心插件**（`settings-navgroup-core-plugins`）= command-palette/templates/daily-notes/unique-notes/page-preview；③ **第三方插件** = R163 per-plugin tabs（`settings.pluginSettingsGroup` 标题 + `settings-nav-plugin-${id}`，原样保留排末）。nav item testid 仍 `settings-nav-${id}`；内容容器 `settings-section-${section}`（动态）。
> - **臃肿 `AppearanceSection`（旧 255-1212，store reads 一次性声明在顶部）拆为 12 个 section 组件**。迁移＝把控件 `<div className="setting-item">` JSX 块 + 它用到的 `useStore(x)` read 行整段搬到目标组件，setter（模块级函数）照常调用，**testid 逐字不变**。归页：行号/可读行宽/拼写/严格换行/自动补全括号/文档内反链/折叠标题/默认新标签模式/制表符/**笔记属性**(`propertiesDisplay`，需 `useApp()`)→**EditorSection**；检测扩展名/自动更新链接/Wiki 链接/链接路径/新建位置(+folder)/附件路径/忽略文件→**FilesAndLinksSection**；模板/日记/唯一笔记/页面预览→**核心插件子页**（TemplatesSection/DailyNotesSection/UniqueNotesSection/PagePreviewSection）；**语言**→**AboutSection**（加 `useStore(locale)`）；主题/强调色/三字体/字号/内联标题/ribbon/标签栏/状态栏/obsidianCss·theme·snippets→留 **AppearanceSection**。新增空页 **`KeychainSection`**（`settings-keychain-empty`，"尚未添加密钥…"）。**默认段仍 `appearance`**（保 r163 不变量）；plugin-tab 失效回退 `plugins` 不动。
> - **i18n**（`dict.views.ts`，en+zh 双补）：新 `settings.navGroup.options`/`.corePlugins`/`settings.section.keychain`/`settings.keychainEmpty`；section `<h2>` 与 nav label 复用既有 heading key（editorHeading/filesAndLinks/templates/dailyNotes/uniqueNotes/pagePreviewHeading）；`settings.section.plugins` 改值更忠实（en "Community plugins"/zh "第三方插件"）。
> - **`app/icons.tsx`** 补 lucide `key` path（keychain nav 图标——原 `icon:"key"` 未注册→静默回退 file-text；同组 sun/command/puzzle/pin 实际都已注册渲染真图标，仅 key 漏注册）。
>
> **⚠️ 根因教训（本轮最大坑，G2/G3 必读）**：**把设置控件迁到新段，会让所有「`openModal('settings')` 后不导航、直接 `page.click([data-testid=该控件])`」的旧 e2e 套件 timeout**——默认段仍 appearance、控件已搬走不在那里。本轮波及 **15 个套件**（r25/r48/r50/r53/r87/r88/r89/r92/r96/r153/r154/r155/r156/r157/r159），修复＝在**每处** openModal 后插一句导航 `await page.click('[data-testid="settings-nav-<目标段>"]')`（每文件只用单一迁移段的控件，故一次导航即可；r153/r155/r156 的读 aria-checked 在浏览器侧 `app(async…)` 块内、`page` 不可用→改用 `document.querySelector(...).click()`）。**只加导航、零行为断言改动**。**G2/G3 续迁/改设置面控件时，必须同步扫这批 UI-驱动套件**（grep `openModal\(.settings` ∩ 被迁 testid）。
>
> **data-safety「迁控件不丢值」**：本轮不写 .md/vault（setter 是 localStorage-backed appearance/core/hover store），风险仅「搬 JSX 时漏带 `useStore` read 行 → 控件不回显但 setter 仍写 = UI 与 store 脱节」。`tsconfig noUnusedLocals:false` **不抓**孤儿 read，故靠 ① reviewer 人工 grep（30 个迁移 read 各 1 次、均在同组件被消费）② r177-e2e 显式「editor 段 toggle line-numbers → aria-checked 翻转 + 写穿 `localStorage["geode.showLineNumbers"]` + 关窗重开仍回显持久值」双证读绑定未脱节。
>
> **对抗评审 7 维 → 1 minor confirmed（已修：key 图标）+ 余全证伪**（控件守恒 grep 各 testid==1、分层无 compat import、i18n 双补无悬空 key、导航三组顺序/默认段/回退正确、aria/select 映射逐控件核对）。**简化门 clean（零编辑）**——7 个 section 的 `<section><h2 className="settings-heading">` 样板抽共享 wrapper 属 SIMPLIFY-NO（加抽象＝Step 1 契约决策，非 Step 3.5 减法），STOP。验收：**r177-e2e 52/52** + **15 套件修复后全绿** + 回归 r163 20/20·r145 15/15·r142 18/18·r166 18/18·r79/r85/r94/r100（appearance 留存控件未动）·typecheck 0·cargo check·**生产构建成功**。**桌面 probe N/A**（纯 React 设置弹窗、平台无关、setter 是未改的 localStorage store、零 fs/Rust/分支、同 R164 先例）。
>
> **v1 defer/nuance**：① **视觉像素级 = G2**（本轮只做 IA 结构，未碰弹窗尺寸/控件样式/浅深主题/focus-ring bug）；② **热键 280 命令 = G3**；③ **核心插件「总览/开关页」**（Obsidian 选项组第 7 项，列全部核心插件可开关）**defer**——需 core-plugin enable 模型，本轮只做 `核心插件` **组标题 + 各核心插件设置子页**；④ 导航图标多沿用既有名（unknown 回退 file-text；系统性图标对齐留 G2）；⑤ language 仍是简单 en/zh select（非 Obsidian 完整语言列表）。
...
>
> **Gate（R160 教训：explorer 逐子项实查）**：三子项**全缺、非误判**（getMostRecentLeaf/setActiveLeaf 零命中、openLinkText 实现存在但忽略 `_openViewState` + `.split("#")[0]` 丢弃 subpath）。**全部纯 compat 复用既有**（core R14 `requestReveal`/`resolveSubpath`/`revealTarget` + R171 `parseLinktext`），**无 core 新能力、无 defer 项**——疑第八梯队最后一个纯前端中型项。**精确范本 = `features/editor/wikilinks.ts:29-38`**（openFile→resolveSubpath→requestReveal）。
>
> **商业主轴价值**：Templater/QuickAdd 程序化导航（开到特定 pane + 滚动到 #heading/^block）。

**契约（加性 · 单文件 workspace.ts · 复用既有 reveal 基建）**：
- **`openLinkText(linktext, sourcePath, newLeaf?, openViewState?)`**（workspace.ts:478 改）：① `parseLinktext(linktext)`[R171 util.ts:99]拆 `{path, subpath}` 替手写 `.split("#")[0]`；② 现有 openFile 逻辑（含未解析→vault.create 分支**不动**）；③ **新增**：openFile 后，若有 subpath → `resolveSubpath(resolvedPath, subpath)`[core metadata.ts:546，返 `{kind,from,to}|null`]→ 非 null 则 `requestReveal(resolvedPath, from, to)`[core workspace.ts:348]。**# 形处理逐字镜像 wikilinks.ts:29-38**（resolveSubpath 的 subpath 入参 # 形式照范本——实现者读 wikilinks.ts + parseLinktext 返回的 subpath 形式核对、勿臆断）。**时序**：requestReveal 必在 openFile **之后**（一次性 Store、EditorPane 后消费）；新建空文件 resolveSubpath 返 null→不 reveal（范本 `if(span)` 守卫）。
- **`getMostRecentLeaf(root?): WorkspaceLeaf | null`**（新增，~3 行）：返回 `this.activeLeaf`（active-pane facade，workspace.ts:341/356/370）。
- **`setActiveLeaf(leaf, params?): void`**（新增，~5 行）：SidebarViewLeaf → `revealLeaf` 思路（workspace.ts:443）；否则 active-pane facade 下记录为已解析的 no-op（无 leaf id 寻址=单 active-pane 模型的 recorded deviation）。

**文件所有权（2 implementer 并行 · fixture MAIN_JS 运行时字符串零编译耦合）**：
- **Owner A**：`src/compat/obsidian/workspace.ts`（3 方法）。**绝不碰** openLinkText 的 vault.create 未解析分支。
- **Owner B**：`src/compat/obsidian/fixture.ts` + `.calibration/r175-e2e.mjs`（openLinkText 开正确文件 + subpath reveal[**读 r64/r107-e2e 或 wikilinks 怎么断言 reveal**——revealTarget Store 订阅捕获 set / flash class，按 probe 时序纪律断 Store 优先]、getMostRecentLeaf 返 leaf、setActiveLeaf 不抛）。

**data-safety**：纯导航/滚动（openFile + requestReveal Store），不写 vault/.md/editor 文档内容；**openLinkText 的 `vault.create`（未解析链接）是既有行为、本轮不动**（改 subpath 解析勿误改创建分支）→ 非 data-safety 轮（但对抗评审核创建分支未被破坏）。

**v1 nuance / defer**：setActiveLeaf 对非 SidebarViewLeaf 落为 reveal/no-op（Geode 单 active-pane facade、无独立 leaf id 寻址=recorded deviation）；getMostRecentLeaf 的 `root?` 参在单窗口下忽略（无 popout）；openViewState 只消费 subpath reveal（`active`/`group` 等其余字段未用）。
...
>
> **Gate（R160 教训：explorer 实查现状）**：① loader 注入区（loader.ts:163/171/182）只注入 moment/global/app，`sleep`/`nextFrame` 全仓零命中=**完全缺**（插件调全局 `sleep()` 直接 `ReferenceError` 崩，QuickAdd/Templater 用户脚本常用）；② `Document.on/off` **完全缺**（`dom.ts` 只 patch 了 `HTMLElement.prototype.on/off` 委托监听[:373-408 用 `_EVENTS` 簿记 + `wrapped` 闭包 `target.closest(selector)` ∧ `host.contains`] + `DocumentFragment.prototype.find`[:335]）；③ **关键复用**：`Document.on/off` 与 `HTMLElement.on/off` 的委托逻辑 **token 级相同**（`host.contains`/`addEventListener`/`_EVENTS` 对 Document 同样成立）→ **抽 `delegatedOn`/`delegatedOff` 模块函数、两 prototype 共用**（净减法、2 真实调用点，非加间接层）。
>
> **商业主轴价值**：全局 `sleep()` 缺席是「插件加载即崩」的高频根因之一（crash-safety）；`Document.on/off` 是设置页/全局委托监听常用。

**契约（加性 · compat dom 表面 · 全部复用既有机制 / 标准 API）**：

- **`dom.ts`**：① 抽 `HTMLElement.on/off` 的内联实现为模块级 `delegatedOn`/`delegatedOff`（`this: HTMLElement | Document`、`DelegatedHost = (HTMLElement | Document) & {_EVENTS?}`、逻辑逐字节不变），`define(HTMLElement.prototype, {…})` 改 `on: delegatedOn, off: delegatedOff`（行为不变）+ 新 `define(Document.prototype, { on: delegatedOn, off: delegatedOff })`。② `installDomAugmentation()` 的 `g` block（:461 fish/fishAll 处）加 `g.sleep = (ms) => new Promise((r) => setTimeout(() => r(), ms))`、`g.nextFrame = () => new Promise((r) => requestAnimationFrame(() => r()))`。
- **`global.d.ts`**：① 新 `interface Document { on(this:Document, type:string, selector:string, listener:(this:Document, ev:Event, delegateTarget:HTMLElement)=>unknown, options?):void; off(同) }`（镜像既有 HTMLElement.on/off 的 global.d.ts 风格、不声明内部 `_EVENTS`）；② `declare function sleep(ms: number): Promise<void>;` + `declare function nextFrame(): Promise<void>;`（与 fish/fishAll 并列）。

**文件所有权（owner A 实现 + owner B 测试 · fixture MAIN_JS 运行时字符串、与 dom.ts/global.d.ts 零编译耦合 → 全并行）**：
- **Owner A**：`src/compat/obsidian/dom.ts`（抽取 + Document.prototype + g.sleep/g.nextFrame）+ `src/compat/obsidian/global.d.ts`（Document 接口 + 两 declare function）。
- **Owner B**：`src/compat/obsidian/fixture.ts`（onload 里 `await sleep(20)` / `await nextFrame()` / `document.on("click", selector, handler)` → 程序化 click 验 fire + delegateTarget + `document.off` 后不 fire，结果写 `<div data-testid="fixture-d7-results">` JSON）+ `.calibration/r169-e2e.mjs`（读 JSON 断言 + **HTMLElement.on/off 回归**[证抽取零回归]）。

**data-safety**：纯 compat global/dom 表面（Promise 定时器 / 委托事件监听）；零 vault/.md/editor/markdown.ts 写 → **非 data-safety 轮**。

**v1 nuance / defer**：`nextFrame` 用 `requestAnimationFrame`（桌面 App Nap 后台不保证 fire 是运行时/§D 时序问题、非 shim 缺陷；浏览器前台 e2e 正常）；`Document.on/off` 复用 HTMLElement 同款委托（`off` 按 selector+listener 匹配移除、不比 options，与既有一致）；D9 renderMath/sanitizeHTMLToDom、D3 prepareFuzzySearch 留后续轮。

## Round 168 additions — Tier 8 D 系列零依赖速赢打包（D1 `apiVersion` 升 1.8.0 + D2 `parseLinktext` + D10 base64 三件套 + D9 `loadMermaid` 再导出 · compat util 表面填补 · 复用既有内部实现 · 零新依赖）【As-built v0.165】

> **状态：As-built（已交付）。** 对抗评审 8 维 → **0 confirmed defect（clean）**。**最关键 = D1 版本假承诺风险已严格排除**（reviewer 枚举 compat 全部 39 处 `reportGap`/throw stub × 引入版本：**(1.5.0, 1.8.0] 区间零 stub**——removeCommand 1.7.2/getAllFolders 1.6.6/onUserEnable 1.7.2/getFileByPath 1.5.7/Menu.forEvent 1.6.0/footnoteRefs+copy 1.8.7 全实证已实现；所有 stub 引入版本均 >1.8.0[appendBinary 1.12.3/声明式 Settings 1.13/SecretStorage 1.11/addComponent 1.11] 或 pre-1.0 era[registerObsidianProtocolHandler/registerExtensions/addColorPicker——这些在旧 1.5.0 下就已被假承诺、非本轮新增] → **1.8.0 = 已实现最高面与最低 stub 之间的诚实最大值**）。D2 parseLinktext 与 Obsidian 真实源 `substring(0,idx)`/`substring(idx)` 逐字节等价（不 trim、subpath 含 `#`、split 首个 `#`，10 边角 node 实跑），与 trim 的 `getLinkpath` 语义不同故**未合并、getLinkpath 字节未动**。D10 base64：`.buffer` **无 slack**（core/net `base64ToBytes` = exact-size `new Uint8Array(bin.length)`）、round-trip live 验证 `[72,105,33]`→`"SGkh"`→还原。D9 loadMermaid 类型更精确的子类型、live e2e 实测 resolve 出的实例 `.render` 是 function（非 mock）。分层 compat→core 无循环、无 features 反向 import。简化门 **clean/skip**（生产 diff ~20 行 / ≤2 文件 / 全薄包装，命中 skip 条件、不制造工作）。验收：r168-e2e **16/16**（apiVersion + 3 门控含 req("1.6.0")===true 对旧 1.5.0 回归 + 4 parseLinktext 形状 + base64+blob round-trip + loadMermaid .render）、回归 r167 25/25·r165 9/9·r113 10/10·typecheck 0·cargo check·**生产构建成功**。**桌面 probe N/A**（纯 JS：版本常量/字符串拆分/base64/动态 import mermaid，零 fs/Rust/平台分支，WKWebView≡Chromium 标准 web API，同 R165/R167）。**informational（非本轮）**：core/net.ts:31 `String.fromCharCode(...subarray)` spread 理论爆栈，0x8000 分块已缓解、属既有代码。
>
> **Gate（R160 教训：explorer 实查现状，别信缺口表字面）**：explorer 逐项核实 4 项真实状态 + 可复用内部实现位置。**本轮只打包「纯再导出 / 极薄桥接、复用既有内部实现、零新依赖」的 4 项**；**defer**：D7 sleep/nextFrame/Document.on/off（触 dom.ts + global.d.ts 不同子系统）、D9 `renderMath`/`finishRenderMath`（需 KaTeX 适配逻辑）、D9 `sanitizeHTMLToDom`（安全敏感、无现成安全 DOM 构建器）。
>
> **商业主轴价值**：D1 是最高杠杆——`apiVersion="1.5.0"`（util.ts:51，R4 旧值）落后已实现面（removeCommand 1.7.2 / getAllFolders 1.6.6 / footnoteRefs+copy 1.8.7 全 explorer 实证已实现，唯 appendBinary 1.12.3 仍 stub）→ 任何 `requireApiVersion(">=1.6/1.7/1.8")` 当前假阴性、走 legacy/禁用分支 + minAppVersion 误 warn。D2/D10 是 Dataview/Templater/Excalidraw 普遍调用的工具函数。

**契约（加性 · compat util 表面 · 全部复用既有内部实现）**：

- **D1 `apiVersion`**（`util.ts:51`）：`"1.5.0"` → **`"1.8.0"`**（匹配已实现的 1.7.x 全 + 早期 1.8.x footnoteRefs/copy；**不虚高到 1.12.3** 因 appendBinary 仍 stub）。一行常量改。`semverCompare`/`requireApiVersion` 逻辑本身正确不动；唯一读者 `loader.ts:261-262`（minAppVersion 比较）现对 minAppVersion≤1.8.0 的插件不再 warn = 预期收益。
- **D2 `parseLinktext`**（`util.ts` 新增、紧跟 getLinkpath）：`parseLinktext(linktext): {path, subpath}` —— `idx=indexOf("#")`；`idx<0` → `{path:linktext, subpath:""}`；否则 `{path:slice(0,idx), subpath:slice(idx)}`。**subpath 保留前导 `#`/`^`**（对齐 Obsidian 真实源 `substr(n)` 语义，**不 trim**——与 Geode 既有 `getLinkpath`[trim] 语义不同 → **getLinkpath 不动、不合并**）。零 core 依赖。
- **D10 base64 三件套**（`util.ts` 新增；`import { bytesToBase64, base64ToBytes } from "@core/net"`）：`arrayBufferToBase64(buf)=bytesToBase64(new Uint8Array(buf))`；`base64ToArrayBuffer(b64)=base64ToBytes(b64).buffer`（core 返 `Uint8Array<ArrayBuffer>`、`.buffer` 类型即 `ArrayBuffer` 无需 cast、exact-size alloc 无 slack）；`getBlobArrayBuffer(blob)=blob.arrayBuffer()`（标准 API）。桥接既有 core/net，零新依赖。
- **D9 `loadMermaid`**（`index.ts` barrel）：`export { loadMermaid } from "@core/mermaid"`（实现已在 core/mermaid.ts:15，`Promise<Mermaid>` 形状与 Obsidian 一致）。一行再导出。
- **barrel `index.ts`** util block（95-116）加 `parseLinktext`/`arrayBufferToBase64`/`base64ToArrayBuffer`/`getBlobArrayBuffer`；新增 `loadMermaid` 从 `@core/mermaid` 再导出。

**文件所有权（owner A 串行改 barrel 热点 + owner B 独立测试 · fixture MAIN_JS 是运行时字符串、与实现零编译耦合 → 全并行）**：
- **Owner A**：`src/compat/obsidian/util.ts`（D1+D2+D10）+ `src/compat/obsidian/index.ts`（barrel：4 个 util 导出 + loadMermaid 再导出）。**绝不碰 `getLinkpath`**（trim 语义保留）。
- **Owner B**：`src/compat/obsidian/fixture.ts`（onload 里 `require("obsidian")` 调这些 + `await loadMermaid()`[try/catch]，把全部结果 `JSON.stringify` 写进 `<div data-testid="fixture-d-results">`）+ `.calibration/r168-e2e.mjs`（`?obsfixture=1` 读 JSON blob 断言每项）。

**data-safety**：纯 compat util 函数（版本常量 / 字符串拆分 / base64 编码 / mermaid 再导出）；零 vault/.md/editor 写、零 markdown.ts 触碰 → **非 data-safety 轮**。

**v1 nuance / defer**：D1 取 "1.8.0" 保守值（不虚高、appendBinary 1.12.3 等更晚成员仍诚实未声明）；D2 不 trim（faithful Obsidian）、getLinkpath 保留 trim 不合并；D9 只再导出 loadMermaid，`renderMath`/`finishRenderMath`（KaTeX 适配）/`sanitizeHTMLToDom`（安全 DOM）+ D7（sleep/nextFrame/Document.on/off）留后续轮。

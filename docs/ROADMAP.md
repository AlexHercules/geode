...
> **🆕 表面复刻续取序（✅ 2026-06-28 用户 AskUserQuestion 拍板「Surface-replica last-mile」· 自 R269 起 · 优先级最高）**：插件 compat（R221-268·真插件迁移 4 大件 Dataview/Templater/Calendar/Tasks）已强证商业主轴 → 收益递减 → 用户拍板回归**像素级 UI 保真 vs `reference/` 截图 + 高频操作**。取用序：~~**右键菜单补图标（R269 done · 表面复刻最后一公里首项 · 编辑器右键菜单每项补 lucide-ish 图标 · 截图 diff scout 取「高频操作」高值项 · `icons.ts` BUILTIN +8 手绘图标 + `editorMenu.ts` setIcon 装饰 · 机械档·compat 自包含·clipboard/delete 逻辑字节级未动 · r269-e2e 18/18·回归 r252/r200/r131/r267 绿）**~~ → **下一项 = settings-card 圆角卡 → 扁平行（R269 scout ② 次高值偏差 · Appearance/Files/CorePlugins/Hotkeys 把行装圆角卡[`settings.css:189` `.settings-card`·JSX 6 处]·而 Obsidian 与 Geode 自己 Editor 页是扁平整宽行+细分隔线无卡=内部不一致 · 修：去卡视觉+行与 subheader 同 inset·2 特殊卡[core-plugin-list/hotkeys-search-card]保专有类只去 settings-card · 验收 computed-style e2e[无卡背景]+截图对照）** → 表面 backlog：窗口骨架类名对齐 `.workspace-tab-header*`/`.view-header`（逻辑档·App.tsx·风险高·低像素值）· 控件原生化（toggle→`.checkbox-container`/segmented→dropdown）· 热键页结构。**Step 0 仍铁律 fresh scout**（截图 diff·R256「枯竭」不算数·R228 纪律）·**优先机械档（CSS/IA/控件样式）**·绝不硬编码颜色（走 CSS 变量）·UI 串走 i18n。**仍搁置**：差异化 E 系列 / Canvas·Bases 等大件须再拍板 / 插件 compat niche 项（Dataview inline/ctx.frontmatter/第五插件/Templater settings）。

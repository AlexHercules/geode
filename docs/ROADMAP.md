...
### R283 — v0.276（2026-07-05）Markdown 阅读视图链接样式 + 拼写检查默认 ON + 状态栏信息密度

从 2026-07-04 截图对比的 shell 差距中取 2 个 bounded 子项：① Markdown 阅读视图链接样式成熟度（internal 无下划线 / external 下划线 / unresolved dashed 下划线 / footnote-backref muted 无下划线）；② 拼写检查默认 ON（`geode.spellcheck` 默认值由 `false` 翻为 `true`，匹配 Obsidian Editor 默认，红毛线默认出场）；③ 状态栏工作态信息密度（`.status-bar` gap 14→8px、padding 0 12px→0 10px，`.status-item` 加 0 4px 呼吸间距）。

机械档为主、零新依赖、未碰 .md/editor/vault/文件 IO 写路径。简化门 clean。评审 3 处 stale 注释已修、0 逻辑缺陷。验证：`r283-e2e 7/7`、回归 `r50 16/16` / `r185 19/19` / `r66 8/8` / `r100 15/15`；typecheck 0、`npm run build`✓、`PATH=$HOME/.cargo/bin:$PATH cargo check --manifest-path src-tauri/Cargo.toml` 通过。桌面按 by-equivalence（纯 CSS/默认属性，无 Rust/FS 新路径）。
...
### R276 — v0.269（2026-07-04）图谱 Obsidian-style 圆形力导向布局（用户指令单开 loop）

按用户 2026-07-03 指令解决全局关系图布局不像 Obsidian（摊成长条/偏团块）的问题。`GraphForces` 新增持久化 `circle` 字段（默认 0.06，范围 0–0.25）；`GraphView.tsx` 引入已有 `d3-force` 的 `forceRadial`，仅 global 模式启用，local 模式保持 R103/R110 锚点语义；`circleTargetRadius` helper 按 `nodeCount` 与节点度数计算目标半径，hub 内聚、叶子外散；新节点初始位置改为稳定 hash + golden-angle 的 annulus 分布（替换 `Math.random()`）。UI 在 Forces 组 Center 后追加 `graph-force-circle` slider，i18n `graph.forceCircle` 中英。逻辑档、零新依赖、未碰 vault/documents/editor/markdown IO。简化门删 `circleTargetRadius` 无收益泛型与单用局部 clamp、合并 probe 重复 hypot；多维对抗评审 0 confirmed。验证：`r276-e2e 14/14`（旧 blob 补 circle / clamp / slider 持久化 / reset / aspect-ratio 0.75–1.33 / hub median radius < leaf median radius / local graph 未破坏）；回归 `r78 16/16` / `r84 17/17` / `r90 17/17` / `r99 16/16` / `r101 23/23` / `r103 23/23` / `r110 10/10` / `r240 13/13`；`r276-bench` bench=10000 sampled 3k：`graphSettleMs=4587` / `graphDrawMs=1.7`，在 R7/R15 基线范围内无退化；typecheck 0、`npm run build`✓、`cargo check` 通过。桌面按 by-equivalence（纯 React+d3-force+canvas，无 Rust/FS 新路径）。

### R277 — v0.270（2026-07-04）设置弹窗控件像素级视觉对齐 Obsidian（用户指令：像素级别复制 obsidian 界面）

用户 2026-07-04 指令「像素级别复制 obsidian 界面」，本轮聚焦最高频视觉单元——Settings 弹窗控件向 Obsidian 截图 `_截图/03-外观/` 对齐。修改 `src/features/settings/settings.css` + `SettingsModal.tsx` 一个 className：① 开关尺寸改为 Obsidian 42×24 pill、thumb 18px、translate 18px（原 36×20/14px/16px 明显偏小）；② 主题「管理」按钮增加 `.is-primary` 填充强调色变体（原描边按钮不像 Obsidian）；③ 强调色取色器改为 28×28 圆形 swatch（原 36×26 圆角矩形）；④ 弹窗宽度从 in-tree 误恢复的 1000px 调回 900px（匹配 R178 锁定比例与 Obsidian 实际宽度），同步更新 `r178-e2e` 左导航宽度断言到 232px；⑤ 新增 `r277-e2e` computed-style 视觉回归锁（toggle 尺寸/primary 按钮/accent swatch/modal 宽度）。机械档、零新依赖、未碰 vault/editor/markdown/文件 IO。简化门：机械档跳过。scoped review 发现 2 处测试断言脆弱性并已修复（toggle 显式置 ON、primary 背景直接比对 `--accent` 计算色）。验证：`r277-e2e 16/16`、回归 `r178 16/16` / `r270 14/14` / `r275 11/11`、typecheck 0、`npm run build`✓、`PATH=$HOME/.cargo/bin:$PATH cargo check --manifest-path src-tauri/Cargo.toml` 通过。桌面按 by-equivalence（纯 CSS/JSX settings 层，无 Rust/FS 新路径）。
...
> **🆕 表面复刻续取序（✅ 2026-06-28 用户 AskUserQuestion 拍板「Surface-replica last-mile」· 自 R269 起 · 优先级最高）**：插件 compat（R221-268·真插件迁移 4 大件 Dataview/Templater/Calendar/Tasks）已强证商业主轴 → 收益递减 → 用户拍板回归**像素级 UI 保真 vs `reference/` 截图 + 高频操作**。取用序：~~**右键菜单补图标（R269 done · 表面复刻最后一公里首项 · 编辑器右键菜单每项补 lucide-ish 图标 · 截图 diff scout 取「高频操作」高值项 · `icons.ts` BUILTIN +8 手绘图标 + `editorMenu.ts` setIcon 装饰 · 机械档·compat 自包含·clipboard/delete 逻辑字节级未动 · r269-e2e 18/18·回归 r252/r200/r131/r267 绿）**~~ → ~~**settings-card 圆角卡 → 扁平行（R270 done · R269 scout ② 次高值偏差 · Appearance(4)/CorePlugins(1)/Hotkeys(2) 把行装圆角 `--bg-input`/`--bg-modal` 卡+20px 内缩·而 Obsidian 与 Geode 自己 Editor/Files 页是扁平整宽行+细分隔线无卡=内部不一致 · 修[纯 CSS·settings.css 5 处]：`.settings-card`/`.core-plugin-list`/`.hotkeys-search-card`/`.hotkey-list` 去卡视觉+`.settings-subheader` 横向 margin 20→0[顺带修 Editor/Files subheader-vs-row 既有错位] · 机械档·评审揪 1 major[`.hotkey-list` 未 classed settings-card→grep 漏·半扁平错位]已修 · r270-e2e 14/14 computed-style 锁视觉·回归 r245/r231/r242/r229/r145 绿）**~~ → ~~**设置控件原生化 segmented → Obsidian 真实控件型（R271 done · 表面复刻续第 3 项 · Step 0 verify-first 逐控件纠误：3 个 segmented 不统一 →dropdown · tabIndentSize→slider[reference 01-编辑器-02·非下拉] · newNoteLocation→dropdown[02-文件与链接-01] · defaultNewTabMode→2 下拉[拆分=逻辑档·R272] · 修[纯控件 form swap·复用既有控件/setter·零 editor 改]：tabIndentSize→`.settings-slider` 1-8 · newNoteLocation→`.settings-select` · 机械档·控件型变同步扫 5 tracked 套件[R177 纪律] · r271-e2e 10/10·回归 r92/r89/r88/r177/r247/r248 绿）**~~ → ~~**defaultNewTabMode 3-way segmented → 2 个正交下拉（R272 done · 完成控件原生化弧 · Obsidian 拆「视图模式」editing/reading × 「编辑模式」live/source[reference 01-编辑器-01] · 设计：combined defaultNewTabMode 仍是 workspace.openFile 消费的单一真源[消费者零改]+新增持久化 defaultEditMode 记编辑模式[read 视图下 combined 塌 preview 会丢 live/source] · 逻辑档非数据安全 · 多维对抗 7 维揪 1[r271 stale 断言]已修 · r272-e2e 14/14·回归 r271 11/r88/r177/r247/r195 cycle 绿）**~~ → ~~**附件默认位置 text input → Obsidian 4-mode dropdown（R273 done · 表面复刻收尾项 · 逻辑档非数据安全 · 零新依赖 · `attachmentFolder` 单串 view transform：root `/` / specified `path` / current `./` / subfolder `./path` · 保留 Geode 既有默认 `assets`→specified · Explorer raw path 写入不退 · 同轮 disabled honest affordances[theme Manage + 未插件化 core-plugin toggles] · r273-e2e 24/24 + desktop probe 6/6 + r230/raw-path 回归）**~~ → ~~**R274 编辑器右键菜单补「在新标签页打开 / 在右侧打开」（v0.267·机械档·r274-e2e 16/16）**~~ → ~~**R275 Files & Links「启用 URI 链接」toggle（v0.268·逻辑档·r275-e2e 11/11）**~~ → ~~**R276 图谱 Obsidian-style 圆形力导向布局（v0.269·逻辑档·r276-e2e 14/14）**~~ → ~~**R277 Settings 弹窗控件像素级视觉对齐（v0.270·机械档·r277-e2e 16/16）**~~ → ~~**R278 热键页 chip + 圆形加号/删除控件（v0.271·机械档·r278-e2e 15/15）**~~ → **下一项 = fresh scout / surface tail depletion check**：R269-278 已清菜单图标、settings-card 扁平、控件原生化弧、附件下拉、编辑器右键两项、URI 开关、图谱布局、settings 控件尺寸、热键页控件结构；若 fresh scout 仍找到 autonomous-safe 的设置/核心插件窄表面（CSS/IA/disabled honesty/testid/i18n 级）则取最清爽一项；若只剩 Canvas/Bases/多窗口/删除去向/新依赖/系统能力等硬边界或须拍板大件，则登记为候选池清空/阻塞并停。**仍搁置**：差异化 E 系列 / Canvas·Bases 等大件须再拍板 / 插件 compat niche 项（Dataview inline/ctx.frontmatter/第五插件/Templater settings）。
...
**选项2 即取序（autonomous-safe · 自 R222 起 · R221 F2 照跑不打断）**：~~独立 tag pane~~（**R222 done·对齐 native Tags view 三缺口**：树↔扁平切换/全展开折叠/Cmd-click 累积过滤·Step 0 实查纠误面板 R41 早建）→ ~~Footnotes view~~（**R223 done·内联编辑脚注文本**——Step 0 双纠误：面板 R65/R213 早建只读 + **WebSearch 纠误 Obsidian 1.9.0 新增 native Footnotes view[可 edit]**→补 native「click to edit」缺口·data-safety 第二写者写 def 行·byte 精确·r223-e2e 13/13）→ ~~G8 隐藏参考标记~~（**R224 done·Obsidian native Editor「Hide reference marks」**·显示组·默认 ON·单一门控 livePreview hide() helper·纯渲染零 .md 写·reference 截图 ground truth 确认 native·r224-e2e 11/11·回归 8 livePreview 套件零变）→ ~~G8「自动补全英文标点」~~（**R225 done + 据实纠误**：拆成两 native 设置——①「自动补全英文标点符号」= Auto pair brackets = **R153 早 done**[非 R63 community typography]；②「自动补全 Markdown 语法」= Auto pair Markdown syntax = R35 markdownWrapHandler 功能在但缺 toggle → **R225 补 toggle**[镜像 R153 compartment·markdownWrapCompartment+autoPairMarkdown Store·markdownWrapHandler 行为零改]·r225-e2e 10/10·回归 r35/r153/r24 不退）→ ~~G8 RTL 从右到左~~（**R226 done·Obsidian native Editor「Right-to-left」**·显示组·默认关·纯 DOM `dir` 属性[contentDOM + preview div]·CM6 原生 bidi 接管·零字节·镜像 R50 spellcheck·reference 截图 ground-truth 确认 native[per-note frontmatter 是 community 不做]·r226-e2e 9/9·回归 r33 字节锁 37/37 不退·**Editor Display 组全清**）→ ~~G8 快速调整字体大小~~（**R227 done·Obsidian native Appearance「Quick font size adjustment」**·字体组·默认关·Ctrl/Cmd+滚轮调字号·`quickFontZoom` Store + App 全局 wheel effect[gate on toggle·passive 默认快路径] + 复用 vetted setFontSize[clamp 11-28]·r227-e2e 11/11·回归 r50 不退·**外观字体组补完**）→ ~~重建仓库缓存~~（**R228 done·Obsidian native Files&Links 高级组「Rebuild vault cache」**·命令 `app:rebuild-cache` + 设置按钮·复用既有 vetted `metadata.rebuildAll()`[read-only 重 index·零 .md 写]·r228-e2e 9/9·回归 metadata 套件不退）→ ~~视图模式切换 toggle~~（**R229 done·Obsidian native Editor「Show view mode toggle」**·默认开·纯渲染门门控 `.editor-mode-group`[live/source/preview 按钮]·OFF 隐按钮但 Ctrl+E 命令仍切·镜像 showRibbon R94/R100·r229-e2e 9/9·回归 r106/r50/r131 不退）→ ~~设为附件文件夹~~（**R230 done·Obsidian native folder context「Set as attachment folder」**·机械档·Explorer folder 右键 +1 项调既有 vetted setAttachmentFolder[localStorage·非 vault IO]·r230-e2e 8/8·回归 r130/r93 不退）→ ~~快切 3 档 toggle~~（**R231 done·Obsidian native Quick switcher「仅显示已创建/显示附件/显示所有受类型」**·新设置 Tab·QuickSwitcher files 三路条件化[md→+attachment→all]+create 行门控·default attachments ON 零回归·评审修 exact .md 守卫·r231-e2e 13/13·回归 r38 19/19 不退）→ ~~智能列表 toggle~~（**R232 done·Obsidian native Editor「Smart lists / 智能列表」**·默认 ON·gate `@codemirror/lang-markdown` 的 `markdownKeymap`[Enter 续行/重编号/outdent + bundled 引用块续行·Backspace 退格降级]·`addKeymap:false` + `smartListExtension(on)=Prec.high(keymap.of(markdownKeymap))` 进新 compartment·镜像 R225·ON 逐字节同·OFF defaultKeymap 纯换行·r232-e2e 10/10·回归 r225/r153/r35 不退·deviation: OFF 时引用块续行+退格降级一并停[lang-markdown 单命令耦合]）→ ~~始终聚焦新标签页 toggle~~（**R233 done·Obsidian native Editor「Always focus new tabs / 始终聚焦新标签页」**·默认 ON·gate `workspace.openFile` 新 tab 抢焦：`focusNewTab` Store + openFile 加性 `focus?` opt[`shouldFocus = opts.focus ?? focusNewTab.get()`·后台分支 append tab 不改 active/activePaneId·`openedInBackground` 旗标仅后台跳 emitActiveFile]·两显式命令 focus:true 旁路[Cmd+T `app:new-tab` + reopenClosedTab 依赖 getActiveTab 还原 mode/pin]·link/explorer/URI 文件打开尊重设置·reference 01:12 ground truth 默认开·多维对抗 8 维 deliverable·评审揪 1 minor 已修[probe `__geodeNewTabMode` 加 focus:true 自卫]·default ON 逐状态零回归·r233-e2e 17/17·回归 r81 14/14·r39 17/17·r229/r232/r88 不退）→ ~~笔记重组「替代原文的方式」下拉~~（**R234 done·Obsidian native 核心插件「Note composer / 笔记重组」Replace selection with: Link[默认]/Embed/None**·新设置 Tab「Note composer」+ enum 下拉·`ExtractMode` 加 "none"[→ extractReplacement 返回 ""]·`extractReplaceMode` Store[默认 link·镜像 defaultNewTabMode]·`noteComposerCommands` 注入 `.get()` 替换硬编码 "link"[extract-selection + move-heading 共用 extractRange]·reference 05:41 + WebFetch 实证·**逻辑档 data-safety 坐实零丢失**[none 走 create-before-edit + stale-guard·= move 非 delete·新笔记仍建]·默认 link 逐字节零回归·简化门 clean·多维对抗 9 维 deliverable·0 缺陷·r234-e2e 14/14[真 extract 三模式]·回归 r44 25/25·r216 17/17·r47 11/11·r231 13/13·**合并提示+模板位置留 R235**）→ ~~笔记重组「合并提示」toggle~~（**R235 done·Obsidian native「Note composer」Ask to confirm before merging·默认 ON**·Tab 1/3→2/3·`confirmDelete`→`confirmAction` 泛化重命名[4 调用点·delete 套件全绿]·`mergeConfirm` Store·QuickSwitcher merge 包 async confirm gate[**不改 vetted mergeNotes 写**·cancel→源不动零丢失]·confirm 消息 stripExtension·probe 不 gate·r47 加 dialog accept·逻辑档 data-safety 坐实 cancel 不丢·简化门 clean·多维对抗 8 维 deliverable·1 minor 已修·r235-e2e 14/14[ON+accept→merge/ON+dismiss→源存活/OFF→无 dialog]·回归 r47/r140/r161/r38 不退）→ ~~笔记重组「模板文件位置」~~（**R236 done·Note composer Tab 收官 3/3**·Obsidian native Template file location·新 `expandExtractTemplate`[单趟镜像 expandTemplate·{{content}}/{{fromTitle}}/{{newTitle}}/{{date}}]·`extractTemplatePath` Store[默认空 verbatim 零回归]·extractRange 读模板→展开→{{content}} 缺省追加·**data-safety 坐实缺模板不丢内容[catch→verbatim fallback]+单趟不重扫**·评审修 1 minor[大小写检测]·r236-e2e 16/16·回归 r44/r216/r23[修 pre-existing nav]/r234/r235 不退）→ ~~换库常驻入口~~（**R237 done·二次拍板表面复刻序第 1 项**·状态栏 vault 名 `<span>`→`<button>` 可点触发既有 vetted `app:switch-vault` 切换器·机械档·只补 UI 入口[切库逻辑 R203 已建]·跳简化门·scoped review 8 项全过·0 缺陷·r237-e2e 9/9·回归 r203 21/21·r100 15/15）→ ~~`editor:focus` 将焦点切换至编辑区~~（**R238 done·Obsidian native「Focus on the editor」高频命令**·G3 矩阵 line 139 missing→done·**Step 0 纠误**：HANDOFF 候选随机笔记/复制路径都早已 done→改取真 missing·镜像既有 5 个 getActiveView fold 命令 `getActiveView()?.view.focus()`[CM6 DOM 焦点·null-guard no-op·非数据安全]·逻辑档·简化门 clean·聚焦评审 0 缺陷·1 minor 测试质量修[openGraph 真覆盖 null 分支]·r238-e2e 9/9·回归 r188/r113/r217 不退）→ ~~`bookmarks:bookmark-search` 收藏当前搜索~~（**R239 done·完成搜索书签**·G3 line 265 missing→done·§11 整组补完·**Step 0 半成品发现**：search 书签 type/render 早建[R158]但缺创建命令+点击恢复 stale stub→双补·reflection store[SearchPanel 镜像 query→命令读 add+notice]·点击修复 requestSearch 注入·空 no-op·逻辑档非数据安全[bookmarks.json]·简化门 clean·多维对抗 8 维 0 缺陷·1 minor 硬化[切库清 query]·r239-e2e 8/8·回归 r158/r193/r222/r27 不退）→ ~~`graph:open-local` 打开局部关系图~~（**R240 done·复用 R103/R110 local 图基建**·G3 line 238 missing→done·**Step 0 半成品发现**：GraphView 早支持 local 模式[anchor=lastActiveFile]→缺直接打开命令·one-shot openLocalGraphRequest store + openGraph→GraphView 消费切 mode local·逻辑档非数据安全[图谱只读 viz·prefs localStorage]·简化门 clean·多维对抗 8 维 0 缺陷·1 minor 测试覆盖补[path A 未开打开]·r240-e2e 13/13·回归 r78/r103/r110 不退）→ ~~`file-explorer:copy-absolute-path` 复制绝对路径~~（**R241 done·桌面 gated**·G3 line 180 missing→done·纯 toAbsolutePath OS-aware join[sep 检测/尾 sep strip/windows rel 转换·零 IO] + Vault getVaultPath 代理·available getVaultPath!==null[浏览器不可用·镜像 R218]·clipboard R183 vetted·逻辑档[碰 vault.ts 红线但纯助手零写]·简化门删 1 转发·多维对抗 8 维 0 缺陷·r241-e2e 12/12·回归 r179/r183/r218 不退）→ ~~Files&Links「删除文件时确认」toggle~~（**R242 done·回收站组 slice 1·data-safety careful 轮**·Step 0 全表面 scout：热键冲突筛选 WebSearch 纠误非 native 不加·剩唯一 native 缺口=回收站组三设置·取最安全 slice 删除确认 toggle·gate 3 删除点[App+Explorer 单删/批删]不碰 vetted .trash 写·**默认 ON 安全 deviation**[Obsidian 默认 OFF·按用户 flag「放松防护」取安全默认·toggle 关即 Obsidian 忠实]·逻辑档·多维对抗 8 维 0 缺陷[cancel 绝不 trash·OFF 不旁路 flush·OFF 仍 recoverable]·r242-e2e 11/11·回归 r140/r161/r42/r93 不退）→ ~~索引 markdown 图片嵌入 `![](x)`~~（**R243 done·回收站组余 2 的 data-safety 前置·用户拍板 Option 1**·原定 slice 2a 删除附件处理·Step 0 发现 data-safety 阻断[孤儿判定需准确判断附件是否还被引用·但 parseNote 显式跳过 `![](x)`·wikilink 嵌入 `![[x]]` 早已索引]→**先补 markdown 嵌入索引**[顺带修 rename 不改写嵌入既有 bug]·`core/metadata.ts` parseNote 删 `if(m[1]=="!")continue`·嵌入 push 进 meta.links[kind markdown]·**LinkRef from=m.index+1 排除前导 `!`**→R16 改写校验 `^\[…\)$`/重建逐字不变即处理·`!` 在 span 外永不触碰[`![](old)`→`![](new)` 非降级为链接]·R16 引擎零改·**逻辑档[碰 metadata+影响 R16 rename·改 .md 字节面·底线①] data-safety skill 满跑·多维对抗评审揪 2 major+1 minor 全修**：①compat buildCache[`compat/obsidian/metadata.ts:538`] 嵌入分桶门原死写 `kind=="wikilink"`→md 嵌入误落 cache.links[丢 `!`·position 偏 1]→改 `content[from-1]=="!"` 不限 kind·r119 加 md 嵌入分桶断言锁[15/15·**widen-recheck 教训复发：初版只 grep core/ 消费者漏 compat/ 层**]；②getAttachmentMap 仍漏 frontmatter/property 附件引用[parseNote blank 整段 frontmatter]→引用**下界非完整集**·ARCHITECTURE 措辞降级+R244 前置约束；③r243 #2 弱断言隔离 other.md·r243-e2e 11/11[含 embed-into-metachar byte+`!`+percent-encode]·回归 r33 37/37[字节锁]/r70 23/23/r119 15/15/r101 23/23/r24 12/12/r126 11/11 不退·桌面 probe N/A[纯 TS·写 R16-vetted·字节平台无关]）→ ~~删除附件处理 下拉~~（**R244 done·回收站组余 slice 2a·data-safety 最高危轮**·Obsidian native Files&Links「删除文件时删除附件」reference 02:30·默认「每次都询问」·删笔记时按 attachmentDeleteMode[ask/delete/keep] 一并删【仅被该笔记引用】的孤儿附件·走 recoverable vault.trash·**绝不永久删**·**data-safety crux 兑现**：新 getOrphanedAttachments 算【完整 referrer 集 = 正文 getAttachmentMap ∪ frontmatter `[[att]]`(复用 WIKILINK_RE)】→ 别的笔记正文/frontmatter 引用的附件绝不被误删·新 core/attachmentDeletion.ts resolveAttachmentDeletion[keep→[]·deletedRoots 过滤防文件夹双删·ask→confirmAction 列清单]·3 删除点接线[App+Explorer 单/批·序 = R242 gate→resolve(ask 在 flush 前)→flushAll→trash(note)→per-att trash 孤儿]·**逻辑档→data-safety skill 满跑·简化门 clean·多维对抗 0 crit/0 major·四性坐实**[误删防护/recoverable 无永久删/flush 序/默认 ask 不破坏既有套件]·评审驱动硬化[孤儿 trash per-att try/catch]+2 deviation 记档[未加引号 frontmatter=Obsidian/compat R126 既有限制·未保存编辑新引用·纵深防御兜底]·r244-e2e 29/29·回归 r242/r101/r126/r42 不退·桌面 N/A-by-equivalence[纯 TS+trash R42 vetted]）→ ~~设置左导航选中项忠实度~~（**R245 done·G2-b 设置页视觉打磨 slice·机械档纯 CSS**·Step 0 实测纠误 settings.css 零硬编码颜色[R178 早立全变量]→真偏差=左导航选中项用 accent 紫高亮·而 Obsidian nav 选中是中性灰[reference 03 §三「灰底高亮」·§四 accent 仅 开关/主按钮/链接]→改 `.settings-nav-item.is-active` `--accent-muted`→`--bg-active`[=Obsidian `--background-modifier-active-hover`]·`--accent`→`--text-normal`·审计 settings.css 全部 accent 用处确认仅 nav 误用[toggle/slider/主按钮/focus/控件 active 正确保留]·scoped review 0 缺陷·r245-e2e 7/7[computed-style 探针解析变量精确比对·含防空断言]+截图实证·回归 r23 22/22·桌面 N/A）→ **🅿️【「删除文件去向」三档 = PARKED（用户 2026-06-26 拍板转视觉打磨·删除问题留档）**：撞硬边界（永久删不可恢复底线①·系统回收站可能需 Rust#5）·留 backlog·待授权·勿自主启动】 → ~~设置内容区分组 section 子标题（外观页）~~（**R246 done·G2-b 续·机械档纯 IA 重排**·reference 03 外观分组 主题→界面→字体·Geode 原扁平且顺序非忠实[theme→字体→界面]→重排进 主题(headerless)/界面/字体 三组 + 加 界面/字体 灰色子标题·新 `.settings-subheader` CSS + `AppearanceSection` 仅重排既有 14 块[testid/绑定逐字保留]+2 `<h3>`+i18n 2 键·scoped review 0 缺陷[byte 级 set-diff 坐实纯搬迁·R245 未回退]·r246-e2e 10/10[13 testid 全在·子标题文案/class·DOM 顺序]+截图实证·回归 r23 22/22·桌面 N/A）→ ~~编辑器页分组 section 子标题~~（**R247 done·G2-b 续·机械档纯 IA 重排**·reference 01 编辑器分组 顶部→显示→行为·Geode 原 17 项扁平交错→重排进 顶部(headerless)/显示/行为 + 加 显示/行为 子标题·复用 R246 `.settings-subheader`·`EditorSection` 仅重排 17 块[testid/绑定逐字保留]+2 `<h3>`+i18n 2 键·scoped review 0 缺陷[byte 级 set-diff·R246 未回退]·r247-e2e 11/11[21 testid·子标题·DOM 顺序]·回归 r246 10/10·r23 22/22·桌面 N/A·backlinks-indoc deviation 记档[归显示组]）→ ~~文件与链接页分组 section 子标题~~（**R248 done·G2-b 续·机械档纯 IA 重排**·reference 02 四组 默认位置/链接/回收站/高级·Geode 原 11 项扁平交错→重排进 顶部(headerless)/链接/回收站/高级 + 加 3 子标题·复用 R246 `.settings-subheader`·newnote 条件 path 块紧随 segmented·scoped review 0 缺陷[byte 级 set-diff 86=86·**R242/R244 数据安全绑定逐字未改**·r242 11/11·r244 29/29]·r248-e2e 13/13·回归 r247 11/11·r23 22/22）→ ~~设置下拉框原生观感 + section-子标题收官~~（**R249 done·G2-b 续·机械档纯 CSS**·收官核实剩余设置页全单组[Obsidian 无页内子标题]→三个多组页已覆盖·分组完成·下拉 `.settings-select` 加 `appearance:none`+双 gradient `v` chevron[`--text-muted` 双主题]替原生 OS 箭头·7 select 共用·scoped review 0 缺陷[功能不破·共用类零误用]·r249-e2e 7/7+截图实证·回归 r231/r234/r242/r244/r23 selectOption 全绿）→ **🅿️【R250 = 候选池盘点拐点（docs-only·等用户拍板）**：用户拍板的「回收站组余 + 设置页视觉打磨」两序均完成（R244 + R245-249）。R228 全表面穷举：命令矩阵实质枯竭[剩 missing 全 boundary-gated]·非命令 UI 层仅剩 ~3 bounded 项[**A1 热键页**(verify-first 纠误=扁平+源前缀+控件·非分组 section·防 phantom) / A2 编辑器右键 file-action / A3 默认打开文件] → 此后即 boundary-gated[字体枚举/删除去向 parked/多窗口/Vim 批 codemirror-vim/Stacked tabs data-safety 大轮/真插件迁移 批依赖]。**autonomous-safe bounded 池逼近见底**。**✅ 用户拍板（R250）：先做完剩余 bounded（A1 热键页保真→A2 编辑器右键→A3 默认打开文件），之后做 Vim 模式·🔓 Vim 依赖 `codemirror-vim`(硬边界#5)已授权**（仅 Vim·bounded 做完再启）。**✅ R251 done（A1 热键页命令名加来源前缀·插件命令「书签: xxx」·核心无前缀·扁平非分组·逻辑档）**：CMD_SOURCE_KEYS[9 插件前缀]+cmdLabel 幂等守卫[防 baked-name 双前缀]·ultracode 4-lens 评审揪 1 major[backlink/outline/outgoing-links 双前缀]全修+colon-width nit 证伪·r251-e2e 14/14·回归 r145/r142/r23 绿。**✅ R252 done（A2 编辑器右键菜单加 file-action 项·收藏/属性/导出/复制/[桌面]系统操作/重命名/删除·全 wire 既有 vetted 命令·逻辑档）**：`addFileActionItems` 3 组 + available 门控[桌面项浏览器隐藏] + getCommandName label[收藏翻转] + delete setWarning·底线①删除走 app:delete-file[R242+R244]/重命名走 R16·菜单零 vault 写·ultracode 4-lens 评审 0 confirmed[data-safety 坐实无旁路·label 偏长 nit 证伪]·r252-e2e 20/20·回归 r200 21/21·r131 8/8·r242 11/11。**✅ R253 done（A3 verify-first 证伪跳过 + Vim 模式·v0.248·🔓 `@replit/codemirror-vim` 用户 R250 授权仅此一依赖）**：A3「默认打开文件」WebSearch 证伪疑非 vanilla-native[forum=feature-request + Bug-graveyard 拒绝·疑 Homepage community plugin 漏入 reference]→跳过防 phantom→直接做 Vim。`vimMode` Store(默认 OFF)·`vimExtension(on)=on?Prec.high(vim()):[]`[**Prec.high 必需·否则 defaultKeymap Escape→simplifySelection(R63) 困死 insert 模式**·实测根因修复]·`vimCompartment` 基列表(live↔source 存活·R33 Prec.highest 之上仍赢)·EditorSection 新增「高级」subheader 下放 toggle·ZH 标签对齐官方「Vim 模式」·镜像 R232 compartment-toggle。逻辑档→data-safety skill 满跑[vim 编辑走 CM dispatch→docChanged→autosave·i+Esc 实测落盘·底线①]·简化门 clean·ultracode 4-lens 对抗评审 13 agents = 1 confirmed[toggle 属 Editor 高级非行为·reference 01:42 ground truth·已修]+8 refuted[data-safety all-clear·R33 precedence faithful·:w autosave-fallback 契约授权·bundle nit 均正确驳回]·CM6 单份 deduped·r253-e2e 10/10·回归 r232/r33/r88/r51/r191/r225/r226 全绿。**🏁 A 系列(A1=R251·A2=R252·A3 跳过)+ Vim(R253)全交付 → autonomous-safe bounded 池实质见底 → 盘点交用户·用户拍板「Stacked tabs 立项」**。**✅ R254 立项 done（Stacked tabs 契约冻结·docs-only·ARCHITECTURE「Round 254 additions」）**：Step 0 explorer 决定性发现 **Geode 已 split 多 EditorPane 同屏共存 → 多挂载 data-safety 早被 split 解决（autosave per-DocumentHandle 非 per-pane）→ Stacked 本质 = PaneLeafView 渲全部 tab·复用已验证机制·非新建数据安全面**·难度远低于初判。加性 `PaneLeaf.stacked?:boolean` + `toggleStacked` + sanitizeNode parity + App.tsx map 全 tab + 命令/菜单 + **EditorPane:593 focus 收窄 active-only** + app.css 横向 cascade·文件所有权 A/B/C·r254-e2e 计划。**✅ R255 done（Stacked tabs 实现·v0.249）**：即取 R254 冻结契约·加性零现有字段改动。`PaneLeaf.stacked?` + `toggleStacked` + sanitizeNode parity + App.tsx renderTab 提取 + map 全 tab 渲 column + 命令/菜单 + EditorPane autoFocus + 横向 cascade CSS。**逻辑档·data-safety 坐实零丢失**（多挂载 autosave per-handle[split 先例]·unstack 卸载列 release→flush[R16 继承]·stacked reload round-trip）·简化门 clean·**ultracode 4-lens 评审 16 agents = data-safety PASS + 3 confirmed[1 minor focus-on-activation 已修 + 2 nit]+9 refuted**·r255-e2e 20/20·回归 r81/r39/r88/r232/r33/r51/r229 全绿·桌面 N/A。**🏁 A 系列 + Vim + Stacked tabs 全交付 → 盘点交用户·用户拍板「收尾最后 bounded 小件」**。**✅ R256 done（view-header「…」more-options 聚合菜单·v0.250·机械档·零依赖）**：autonomous-safe 池最后一件。verify-first（ultracode 3-scout）= native-confirmed（pointer 纠误：reference 08:35 实为编辑器右键菜单·但「…」按钮 web 证实真 native·非 phantom·BUILD·R222/R223 先例）。TabBar 加「…」按钮（more-vertical·margin-left:auto）+ viewopts 菜单（视图模式+文件操作+tab/pane·wire 既有 vetted 命令·available() gate 桌面项·markdown/filePath gating·DEFER linked-view 子菜单）·App.tsx shell 不 import compat。机械档→跳简化门→scoped review（ultracode 3-lens·17 agents）= 1 confirmed[reveal-active-file 跨端 reveal 遗漏·浏览器下零 reveal 选项·已修]+13 refuted·r256-e2e 30/30·回归 r81/r131/r252/r33/r229 全绿·桌面 N/A-by-equivalence。**🏁🛑 A 系列 + Vim + Stacked tabs + view-header 菜单全交付 → autonomous-safe bounded 池真清空 → loop 停下盘点交用户**（剩全 boundary-gated：删除去向 parked 待授权 / Canvas·Bases·导入器·录音机·多窗口 须拍板立项 / 真插件迁移 须批依赖=硬边界#5 / F2 已降级）·见 HANDOFF START HERE】**。**✅ 用户拍板「选项1 真插件迁移商业主轴」→ ✅ R257 立项 done**（read-only 3-scout 决定性纠误：**Geode 早已加载真 Obsidian-格式插件·5 个在跑[loader.ts 完整 host]·非从零**·首目标 Sort&Permute MIT 0 dep·唯一 js-yaml 授权 deferred 到 R261 Dataview）**→ ✅ R258 done（v0.251·首 committed 真插件证明 + ES5/tslib 修复 + loader 硬化·零新依赖·逻辑档）**：vendor Sort&Permute 0.7.0 MIT 真 bundle·**首证明立抓真 gap = ES5/tslib base-class**（真插件 ES5+tslib 经 `Base.apply(this)` 继承·shim ES6 class 抛 → `es5compat.ts es5Callable` Proxy apply-trap 修·接 require("obsidian") bridge·**评审纠 throwaway-harvest 对 ctor-绑-listener base 静默失效→收窄包装集至 throwaway-safe only**）+ 硬化（`getLastError` 真 onload 错误进 report）·**data-safety PASS**（真插件 editor 写→autosave·未碰写路径·r258 排序落盘底线①）·ultracode 4-lens 13 agents = 1 confirmed[已修]+8 refuted·r258-e2e 12/12·回归 r221/r171/r167/r175/r33/r24 全绿·桌面 N/A-by-equivalence**→ ✅ R259 done（v0.252·第二 committed 真插件证明·read-time「Words to Read Time」1.0.0 MIT·ES6·零新依赖·零 src 改动·validation 轮）**：vendor read-time（release 原物 sha 同上游·LICENSE/README）·**互补覆盖面**（vs R258 commands+editor）：无命令·addStatusBarItem+registerInterval+registerDomEvent+getActiveViewOfType+真 PluginSettingTab·读 active note 词数→状态栏 read time。ES6 走既有 loader（验 harness 泛化·不需 ES5 fix）·read-only 插件 data-safety 非 scope·focused review CLEAN+1 nit[已校]·r259-e2e 7/7[260 词→~2min 字节级·dynamic recompute]·回归 r258 12/12**→ ✅ R260 done（v0.253·obsidian CM6 editor StateFields·editorInfoField/editorEditorField/editorLivePreviewField + editorViewField 别名·pre-Dataview 无依赖 gap·逻辑档 data-safety）**：三层桥 core/editorContext.ts(2 facet)+features/cmExtensions(editorModeExtensions 加 live-preview facet+基列表 path facet)+compat/editorFields.ts(3 StateField+ViewPlugin queueMicrotask effect-only 填充)+context.ts 注册+index.ts export。Editor.getDoc 早 done(R220)。type 纠误：editorEditorField=裸 EditorView·editorInfoField=轻 MarkdownFileInfo。**data-safety 满跑·零丢失**(填充 effect-only→无 autosave·markdown.ts 未碰·live-preview 套件全绿 decoration 不退)·简化门 clean·**ultracode 4-lens 评审 12 agents = 0 confirmed+8 refuted**(microtask-null-window 全证伪·真插件 reactive 读)·r260-e2e 12/12(真插件读 3 field·livePreview true→false reactive·effect-only seed 后 doc 字节不变)·回归 r106-128/r115/r33/r24/r258 全绿·桌面 N/A。**🛑 下一项 R261 = STOP-POINT：Dataview 类须 `js-yaml@^4.1.0`(parseYaml/stringifyYaml)=硬边界#5·须用户授权才能 npm i**·或先做无依赖余 gap(adapter.stat[Rust]/getResourcePath[Tauri]/ES5-suggest)·**loop 在 js-yaml 授权前不可自主 npm i**·见 HANDOFF START HERE】**。**🎉 用户拍板「授权 js-yaml·试载 Dataview」→ ✅ R261 done（v0.254·商业主轴 milestone：真 Dataview 0.5.70 载入并渲染查询·逻辑档 data-safety）**：🔓 `js-yaml@^4.1.0` 实装(4.3.0·+1 包·零 transitive)。`yaml.ts` parseYaml=load/stringifyYaml=dump(薄 wrapper)。**崩哪补哪 against 真 Dataview**(R258 硬化逐次 surface 真错误)：① `Workspace.updateOptions()`=bump editorExtensionsRevision(第一阻断)；② loader `window.CodeMirror` CM5 stub(defineMode/getMode no-op·第二阻断·⚠️真依赖 CM5 插件仍崩=边界)。**data-safety 零丢失**(R261 不加 Geode 写路径·parseYaml 导出工具·updateOptions 仅 bump·CM5 no-op·markdown.ts 未碰)·简化门 clean·**ultracode 4-lens 21 agents = 0 confirmed+17 refuted**·r261-e2e **13/13**(parseYaml + **真 Dataview status=enabled + DataviewAPI + `dataview LIST` 渲染 .dataview.list-view-ul >0 li 真结果 over metadataCache**)·回归 r115/r258/r259/r260/r221/r171/r33/r24 全绿·Dataview 2.4M 未 committed(gitignored)·桌面 N/A。**🏁 真插件迁移 = 5 手动+3 committed(Sort&Permute/read-time)真插件证明 + ES5/tslib 修复 + CM6 StateFields + parseYaml → 真 Dataview 查询渲染。下一项 R262 = Dataview 深用(TABLE/TASK/dataviewjs)崩哪补哪 或 Templater·🔓 js-yaml 已授权·第二个 dep 须再授权**·见 HANDOFF START HERE】** → ✅ **R262 done（Dataview 深用面 崩哪补哪 = compat metadataCache 类型化 frontmatter·v0.255·逻辑档·零新依赖·纯读投影）**：**Step 0 probe 纠误**——`r262-probe` 实测 LIST/TABLE/TASK/inline-field/inline-`=` **全已渲染**·唯一 ERROR=dataviewjs(Dataview 设置默认 off·faithful·注入 data.json 未被 loadData 接管=次要)；`r262-probe2` 用**判别性数据**揪真 bug：`TABLE rating WHERE rating > 3` 返**全部 3 行**(应 1·`rating`="5" 字符串令 Dataview 类型序 `string>number` 恒真→**静默错数据·无报错**=最恶劣失败类)。根因=frontmatter 字符串化(core `FrontmatterData.fields` 故意 `string|string[]` 撑字节级 properties 写)。**修=compat `metadata.ts` 单文件** 新 `typedFrontmatter(content,fm)` 用已授权 js-yaml(`parseYaml`)重解析原始 YAML 块为类型值(number/bool/date/null/nested)·try/catch+仅纯对象+content 缺省 fallback 字符串化→`out.frontmatter`；`frontmatterPosition`/`frontmatterLinks`(扫字符串 fields)**不变**·`FrontMatterCache` 本就 `unknown`=**零契约加宽**。逻辑档·**data-safety 维 6 refuted 坐实零写路径**(纯读投影·不碰 properties/documents/vault 写·markdown.ts 未碰)·简化门 clean(≤2 文件)·**ultracode 5-lens 对抗=2 confirmed(均 minor·均非回归·记档非改码：①ctx.frontmatter[markdownPostProcess·features 不可 import compat=分层]仍字符串化·Dataview 读 metadataCache 不受影响·统一须 core typed helper 将来轮·②warm-up read 不发 changed·仅 >30M 字符库冷文件边缘=同 sections/listItems 既有降级)+28 refuted·全 5 lens ship**·r262-e2e 18/18(Part A 类型投影 number=5/bool/float/null/nested+getAllTags 消费者不破+坏 YAML fallback 不抛·Part B 真 Dataview `WHERE rating>3`=1 行·NUMERIC SORT 10/2/1·boolean WHERE=2 行)·回归 r126 frontmatterLinks 11·r150/r151 tags 20/13·r33 字节 37·r261 Dataview LIST 13 全绿·桌面 N/A。**下一项 R263 = Dataview/compat 保真续深（先 Step 0 probe 复测再定切片）：A dataviewjs 启用面(查 loader 注入 data.json 链路是否真投递·JS 执行面 security-gated·faithful 默认 off) / B ctx.frontmatter 类型统一(core typed-frontmatter helper·闭合 R262 记档①·惠及所有 code-block processor) / C Templater / D 无依赖 gap(adapter.stat[Rust]/getResourcePath[Tauri]/CM5 stub 扩/suggest-modal ES5)·🔓 js-yaml 已授权·第二个 dep 须再授权**·见 HANDOFF START HERE】** → ✅ **R263 done（真 Templater 2.23 = Obsidian 第二大插件载入并执行模板·v0.256·逻辑档·零新依赖·补 newer-app API 面）**：**Step 0 probe 纠误**——候选 A dataviewjs 实为 faithful 非 gap(`__geodeObsidianConfig` seed data.json{enableDataviewJs:true} 后 JS 执行真工作·桌面读真 data.json 即生效·浏览器注入 dataJson 未接 configFiles=测试基建限制非产品 gap)→转 breadth 载 Templater。**崩哪补哪 9 blocker**(先 static-diff Templater obsidian-member 用法 vs shim 出口界定 gap 有界非 cascade)：补 `ConfirmationModal`/`SettingPage`/`SettingGroup`/`FileSystemAdapter`(marker·instanceof 正确 false)/`getFrontMatterInfo`/`resolveSubpath`/`Plugin.registerCliHandler`(no-op)/`SettingTab.update`(vault 事件防抖·守 display) + **CM5 stub null-mode**(关键:`getMode` 返 `{name:"null"}` 哨兵→插件查 `.name==="null"` 优雅跳过 CM5 高亮·否则从 getMode 建 CM6 StreamLanguage token 不前进→崩整个编辑器)。**逻辑档·data-safety 维 0 confirmed 零写路径**(全加性 compat shim·Templater 模板写经既有 editor→autosave[R258 先例]·纯读 getFrontMatterInfo/resolveSubpath·core/markdown.ts 未碰)·**简化门删 8 CM5 死成员**(null-mode 后不可达·0 caller)·**ultracode 5-lens 对抗=1 confirmed(nit·getFrontMatterInfo `to` 改 `close` byte-faithful·已修)+22 refuted·全 5 lens ship**·r263-e2e **10/10**(status=enabled+4 命令·tp.date/tp.file/tp.frontmatter.count=7[经 R262 typed]展开·**展开 PERSIST 落盘=data-safety**·0 page error)·回归 r261 Dataview 13/13[共用 CM5 stub]·r258/r259/r260/r262/r126/r150/r33 全绿·桌面 N/A-by-equivalence。**🏁 真插件迁移两大件 = Dataview(深用+类型化)+ Templater(载入+基础模板)**。**下一项 R264 = 真插件迁移续深（先 Step 0 probe 定切片）：A Templater 深用(settings UI render 补全·tp.user/tp.system=child_process 桌面 only·插模板 suggest·写回须 data-safety) / B ctx.frontmatter 类型统一(core typed helper·闭合 R262 记档①) / C 第三个真插件(Tasks/Calendar·breadth·zero-dep 优先) / D 无依赖 compat gap(adapter.stat/getResourcePath/suggest-modal ES5)·🔓 js-yaml 已授权·第二个 dep 须再授权·child_process/util 是 Node built-in stub 非 npm 不触边界**·见 HANDOFF START HERE】** → ✅ **R264 done（编辑器抗插件 inline-query ViewPlugin 崩溃硬化·v0.257·逻辑档·零新依赖·纯读）**：**Step 0 probe 改取向**——原拟 Templater 深用（settings render=1.13 声明式框架 9 类型太深·证伪跳过），改 **r264-probe 法**（载真 Templater+Dataview 跑基操·捕 reportGap+未捕异常）揪出**真崩溃**：Dataview inline-query ViewPlugin 开 live 模式崩——① `field(editorInfoField).file` 读 null（R260 字段初始 null·插件构造期在 microtask 前无守卫读）② `type.prop(tokenClassNodeProp)`（新版 @codemirror/language 删此导出→undefined→NodeType.prop 崩）。CM6 隔离 ViewPlugin 崩（编辑器存活）但每开笔记报错+inline 渲染断。**修两处**：① `editorFields.ts` editorInfoField.create() 返**非空** info（新 editorInfoHostFacet 携 {app,registry}+editorPathFacet·editor 省略[create 无 view·可选]·ViewPlugin microtask 仍 reseed 全 info·抽 makeFileInfo 二处共用）；② `loader.ts` 桥 @codemirror/language 加 `tokenClassNodeProp ?? new NodeProp()`（NodeProp 取已 bundle 的 @lezer/common[formatCommands.ts 已直接 import 先例·零新依赖·未动 package.json]·stub 从不 set→type.prop 返 undefined→Dataview ?.split 优雅）。**逻辑档·data-safety 4-lens 0 confirmed 零写路径**（create 只读 info[file getter]·microtask effect-only 不触 autosave[R260 不变]·纯读）·简化门 clean（≤2 文件·makeFileInfo 2 真调用点去重）·**ultracode 4-lens=3 confirmed（全文档级·均 ship）+15 refuted·全 4 lens ship**（@lezer/common phantom nit 不动 package.json·editorEditorField 留 null 真因更正[create 无 view+Dataview 不读·latent gap 记档]·reading-view inline 过度声明降级[r261/r262 验 BLOCK 查询·inline 未断言]）。r264-e2e **8/8**（probe 插件构造期读 editorInfoField 非空+.file 不抛·真 Dataview 开 live 渲染+**0 CodeMirror plugin crashed**[原 2]）·回归 r260 StateFields 12·r261 Dataview 13·r263 Templater 10·r262/r115/r134/r106/r33 全绿·桌面 N/A。**下一项 R265 = 真插件迁移续深/compat 硬化（先 r264-probe 法定切片）：A Dataview 编辑器 inline 真渲染（节点带 tokenClassNodeProp·碰 decoration）/ B 第三个真插件（Calendar VIEW/Tasks·load-test 定 gap）/ C ctx.frontmatter 类型统一（core typed helper·R262 记档①）/ D reportGap 高频项（App.internalPlugins/TFile.stat/dom.onNodeInserted）/ E Templater 1.13 声明式 settings 框架（深·须立项）·🔓 js-yaml 仅此·第二个新 npm dep 须再授权·已 bundle transitive+Node built-in stub 不触边界**·见 HANDOFF START HERE】** → ✅ **R265 done（TFile.stat 真 ctime/mtime/size·v0.258·逻辑档·零新依赖·只读元数据·跨 Rust+core+compat）**：r264-probe 法取最高值非投机 gap=TFile.stat（probe 实测 size===0·Dataview file.size/SORT file.ctime 静默错数据·像 R262）。Templater 设置渲染=1.13 声明式框架 9 类型证伪太深搁置。**修 3 层只读**：① Rust read_dir_recursive 读 fs::metadata→ctime(birthtime 退 mtime)/mtime/size 进 Node::File·invoke<FolderNode> 直反序列化进 FileNode；② core FileNode 加可选 ctime/mtime/size·MemoryVaultAdapter times map（touch/moveTimes/dropTimes）+ fileStat（size=byte length）·listTree 传；③ compat applyStat 复制·ensureFile(node?) rebuild 复制/live create 会话戳+sizeOf·handleModified 内部 sizeOf·**外部改 treeNode 兜底**（评审驱动·外部 sync 删缓存→走 re-list FS stat）·删 reportGap。**逻辑档（碰 core/vault 红线）·data-safety 4-lens 1 nit 0 写路径**（stat 全程只读·grep 无写路径读·touch 纯元数据·内容写未碰）·简化门删中间变量·**ultracode 4-lens=4 confirmed（1 minor+3 nit·均 ship·全已修）+17 refuted·全 4 lens ship**（修 writeBinary touch/remove+trash dropTimes 防泄漏/treeNode 兜底外部改）。r265-e2e 11/11（create size=501·modify 8+ctime 保留·rebuild stat 存活·Dataview file.size 非零）·回归 r261/r263/r264/r28/r93/r42/r33 全绿·cargo+release build·桌面 by-equivalence（headless WKWebView 不载·Rust fs::metadata cargo+build 验证·管线浏览器证明·只读无写面）。**下一项 R266 = 真插件迁移续深/compat 硬化（先 r264-probe 法）：A reportGap 余高频项（App.internalPlugins/dom.onNodeInserted）/ B 第三个真插件（Calendar/Tasks·load-test）/ C Dataview 编辑器 inline 真渲染 / D ctx.frontmatter 类型统一 / E Templater settings 框架（深·须立项）·🔓 js-yaml 仅此·第二个新 npm dep 须再授权**·见 HANDOFF START HERE】** → ✅ **R266 done（第三个真插件 Calendar 2.0[VIEW+Svelte] 载入并渲染月历 + `.app-container` 忠实 DOM 修复·v0.259·机械档·零新依赖）**：r266-probe 深探先证 Dataview/Templater 深用已稳（TASK/GROUP BY/dataviewjs/inline-reading-view 全渲染·tp.file.creation_date→真日期[R265 生效]·0 崩）→ 转 breadth 第三插件。load-test Calendar 2.0（GPL·489KB·零外部 require·Svelte bundled）=enabled + 月历 `<table.calendar>` Svelte 视图渲染 + 3 命令（新表面=自定义 VIEW+Svelte）。真 gap=Calendar `<Portal target=".app-container">`（弹层挂 Obsidian 标准根类）·Geode 根 `<div class="app">` 缺 → 修=App.tsx 根加 `app-container` 类（标准·惠及任何 .app-container portal/主题插件·additive 不破 .app/testid）。**记档 1 onload 瞬态**（Geode main.tsx plugins 载于 React commit .app-container 前=载序·Calendar eager onload portal 抢挂一次失败·非致命·视图+命令工作·重排 boot 有工作区恢复风险搁置）。机械档（additive DOM 类+插件验证·无逻辑/数据安全）→跳简化门+scoped review clean·无对抗 Workflow。r266-e2e 7/7（enabled·.app-container 在·命令·setViewState 开视图·月历 table+周行·除瞬态外 0 error）·回归 r23/r245/r100/r24/r261/r263/r33 全绿。**🏁 真插件迁移 3 大件=Dataview+Templater+Calendar**·gitignored。**下一项 R267=真插件迁移续深/compat 硬化（先 r264-probe 法）：A plugin load 序修（render-then-load·消 onload DOM 瞬态·惠及所有插件·碰 boot 须谨慎）/ B 第四插件 / Calendar 深用 / C Dataview 编辑器 inline 真渲染 / D ctx.frontmatter 类型统一 / E Templater settings（深·立项）/ F reportGap 余项（低值）·🔓 js-yaml 仅此·第二个新 npm dep 须再授权**·见 HANDOFF START HERE】** → ✅ **R267 done（静态 `.app-container` 包裹 #root → 彻底消 R266 onload-portal 瞬态·v0.260·机械档·零新依赖·DOM 结构+CSS）**：Step 0 纠误——A plugin load 序修实有真风险（plugins 故意载于 render 前·重排破工作区恢复插件视图）弃；B Calendar 深用 daily-note 点击实测不建笔记但无错（配置好·疑测试事件/Calendar Beta 行为·非明确 Geode gap 防 phantom 弃）→ 取彻底修 R266 瞬态。**修=index.html 静态 `<div class="app-container"><div id="root">`**（Obsidian 最外层根类·页面加载即在 DOM·早于 plugins 载入→Calendar eager onload Svelte Portal 找得到·wrapper 在 #root 外永不撞 React 树）·app.css `.app-container` 入 height:100% 链·revert R266 React-div 类（单一忠实 wrapper）。无 boot 重排/无 React 冲突/无工作区恢复风险。机械档→跳简化门+scoped review（截图实证布局完好）·无对抗 Workflow。r267-e2e 10/10（Part A 单 wrapper 包 #root 在外·布局满屏·0 error·Part B Calendar onload+view portal 0 .app-container error[R266 是 1]·view 渲）·回归 r261/r263/r266 7/7/r23/r245/r100/r106/r24/r33/r131 全绿·截图布局完好。**⚠️ 插件 compat 渐入收益递减**（3 大件 Dataview/Templater/Calendar 已强证商业主轴·余多 niche/深/低值）。**下一项 R268=真插件迁移续深/compat 硬化（先 r264-probe 法）：A 第四插件（Tasks/QuickAdd·load-test·第二 dep 须授权）/ B Dataview 编辑器 inline 真渲染（节点带 tokenClassNodeProp·架构略绕）/ C ctx.frontmatter 类型统一（R262 记档①·低值）/ D Templater settings 框架（深·立项）/ E reportGap 余项（低值）·可考虑盘点交用户或转向·🔓 js-yaml 仅此·第二 npm dep 须授权**·见 HANDOFF START HERE】** → ✅ **R268 done（第四个真插件 Tasks 8.2.2 载入并渲染任务查询 + `app.metadataTypeManager` shim + init-before-plugins 序修·v0.261·逻辑档·零新依赖）**：Tasks 8.2.2（GPL·882KB·仅 require obsidian+@codemirror/view·零新依赖）=enabled + ```tasks「not done」渲染 `.plugin-tasks-query-result` 4 项 + 7+ 命令（新表面=任务查询引擎）。真 gap=`app.metadataTypeManager.getAllProperties()`（缺 metadataTypeManager·崩·Tasks try/catch 捕非致命）。修=core `propertyTypes.getAll()` + compat App `metadataTypeManager`（getAllProperties 投影 R22 注册表·setType→vetted assign 写 types.json）。**Tasks 实注册 22 个 TQ_* 查询-widget 属性（checkbox/text·非 date·评审纠 overclaim）**·价值=消被捕错误+注册 widget 类型。**评审揪 1 major 已修=init-before-plugins 序**（init 原在 plugins 后→Tasks onload assign 跑时 regVault null 早返不写→init 重置 regMap→注册静默丢；修=await init 提前）。逻辑档（setType 写 config+boot 序改）·data-safety 3-lens 0 confirmed（assign R22 vetted·config 非笔记·序改是 config 读前置非 view-restore 风险）·简化门 clean·**ultracode 3-lens=2 confirmed(1 minor overclaim+1 major init 序·均已修)+11 refuted·data-safety 0**。r268-e2e 10/10（onload setType 持久化[序修]·真 Tasks ```tasks 渲染 4 项·0 error[原 1 崩]）·回归 r261/r263/r266/r267/r72 26/r73 23/r24/r33 全绿。**🏁 真插件迁移 4 大件=Dataview+Templater+Calendar+Tasks**。**⚠️🛑 R269 强提示：插件 compat 已强证商业主轴·收益明显递减·建议盘点交用户拍板转向**（表面复刻 / Canvas·Bases 立项 / 用户指定）。下一项 R269=候选 A Dataview 编辑器 inline 真渲染 / B ctx.frontmatter 统一 / C 第五插件 / D Templater settings 立项 / E reportGap·🔓 js-yaml 仅此·第二 npm dep 须授权·**见 HANDOFF START HERE】**。**🔑 R228 拐点纠误**：R227 收尾据 R226 explorer（仅扫 editor/appearance 两表面）误判「bounded 矿脉见底」——**Ultracode 6 表面并行穷举 scout 纠误：实剩 13 bounded-autonomous 项**（文件与链接/核心插件各设置 Tab/右键/Editor 顶部组等漏扫表面）→ **矿脉远未枯竭·loop 继续**。**纪律：宣布「bounded 枯竭/交回大方向」前必【全表面 Workflow 穷举】（所有 reference 01-08 + 核心插件每设置 Tab + 右键三类 + 候选池全节）·部分 scout 的「没找到」不算数。** 真·拐点（选项1 真插件迁移须批依赖 / Canvas·Bases·stacked-tabs 大件立项须拍板）= **backlog 全清后**。**⚠️ R222-R228 七轮教训：每项 Step 0 必 explorer 实查 + 读 reference 截图（PNG ground truth）+ WebSearch 辨「native 有 / community 加的 / Feature-archive 没做 / native 新版新增」——online 查不到 ≠ 非 native（R224/R226 截图证）、reference 截图滞后新版（R223 Footnotes 1.9.0）、相邻设置名易混（R225）；即取序项常已 done/需纠误·CM6 原生 bidi/补全优先复用（R226）·surface 既有 read-only 引擎=最低风险 bounded 轮（R228 rebuildAll）·只补 native 真缺口。**

...
| **D9** | ~~`loadMermaid` 再导出~~ + ~~`renderMath`/`finishRenderMath`~~ + `sanitizeHTMLToDom` | 中 | ~~loadMermaid~~ ✅ **R168**；~~renderMath/finishRenderMath~~ ✅ **R170**（+ `loadMathJax`，新 `compat/obsidian/math.ts` 复用 core loadKatex；**sync-return + async-finish** 复刻 Obsidian MathJax 模型：同步返 `<span>`+异步 KaTeX typeset 入队、finishRenderMath await；maxSize:100 rule-bomb 防护；r170-e2e 12/12）。**剩**：sanitizeHTMLToDom（安全敏感、自写保守白名单、建议单拆）。**图表/数学/web-clipper** 类插件 |
...
> **第八梯队取用顺序建议**：~~D1 版本号 / D2 parseLinktext / D9 loadMermaid / D10 base64~~（**R168 打包 done**）+ ~~D4 AbstractInputSuggest~~（R167）→ ~~D7 sleep/nextFrame/Document.on/off~~（**R169 done**）→ ~~D9 renderMath/finishRenderMath/loadMathJax~~（**R170 done**）→ ~~D3 prepareFuzzySearch/prepareSimpleSearch~~（**R171 done**）→ ~~D11 MarkdownPreviewRenderer 静态桥~~（**R172 done**）。**⚠️ 易摘零依赖速赢枯竭**——剩余分层：🛑 **D6 setIcon Lucide=硬边界 #5**（无 lucide、待用户拍板）/ **D9 sanitizeHTMLToDom**（零依赖但安全敏感、heavy XSS 评审、单拆 careful 轮，**下一项**）/ **D5 getResourcePath**（接 Tauri asset-protocol、动 Rust/host、单独成轮）/ **D8 editorInfoField 等 CM6 StateField**（需 editor 集成）/ **D12 FileManager.getAvailablePathForAttachment / D15 Workspace.getMostRecentLeaf / D16 getLanguage 等**（中型 API、无安全面）。**D5 getResourcePath 接 Tauri asset-protocol、D14 registerExtensions/protocol 派发** 涉宿主接线、单独成轮。**parseYaml（高价值）须用户拍板新依赖后再做**。全部为加性 display/读取面，无 vault 写、不触发 data-safety 字节级套件（除非实现触及 markdown.ts/写 .md）。

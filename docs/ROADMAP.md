...
| 功能 | 当前状态（已核实）| 范围与切入点提示 |
|---|---|---|
| **⑱ ~~Live preview 表格~~ / 跨行 `$$`·`%%` / ~~mermaid widget~~ / ~~Setext 标题~~** | **大部完成**（**Setext 折叠 R54** / **Live 表格 R55**[StateField block widget，Geode 首个 cursor-aware block widget] / **Live mermaid R56 已完成（v0.56，见上）**：```mermaid 围栏 live 渲染为图，**抽共享 `liveBlockWidget.ts`** 复用[liveTables 重构 + liveMermaid]，`findMermaidRanges`[FencedCode CodeInfo 首词 mermaid] + `MermaidWidget`[renderMarkdownToHtml 占位 + hydrateEmbeds 异步 SVG，复用 R19 串行链]；r56-e2e 13+probe 7，评审 0 真缺陷。**仍缺**：跨行 `$$`/`%%` 仅淡显不渲染——R18 偏差）| **余项复用 `liveBlockWidget`**（`spec.ranges` 任意函数）：跨行 `$$` 数学[katex，`$$` 可能需 scan-based ranges 非 lezer 节点]、`%%` 注释隐藏。已知偏差：嵌套(blockquote/list)表格/mermaid 不 live 渲染（行首守卫降级源码）；theme 切换 widget 不即时重渲染；视口外大文档迟渲染（自愈）。逐项可拆。 |

...
| 功能 | 当前状态（已核实）| 范围与切入点提示 |
|---|---|---|
| **⑱ ~~Live preview 表格~~ / 跨行 `$$`·`%%`·mermaid widget / ~~Setext 标题~~** | **部分**（**Setext 标题 R54 已完成**：Setext live 样式早已有[lezer heading tag→cm-md-h1/2]，本轮补**折叠**[folding.ts foldService 扩 SetextHeading1/2]，顺带修 ATX 折穿 Setext latent bug；r54-e2e 11+probe 8。**Live 表格 R55 已完成（v0.55，见上）**：GFM 管道表格 live 渲染真 `<table>`[复用 renderMarkdownToHtml]，`liveTables.ts` **StateField block-replace widget + atomicRanges + cursor/click 揭示源码**——Geode **首个 cursor-aware block widget**[block 装饰必须 StateField 非 ViewPlugin，否则 RangeSet.spans 崩溃]；行首守卫降级内嵌表格；r55-e2e 15+probe 6，评审 1 major+2 minor 修。**确立 block widget 范式**。**仍缺**：跨行 `$$`/`%%` 仅淡显不渲染；mermaid live 源码呈现——R18/R19 偏差）| **余项可复用 R55 block widget 范式**（StateField + atomicRanges + selection-reveal + click-edit）：mermaid live[widget 异步渲染 + R19 两坑]、跨行 `$$` 数学[katex]、`%%` 注释隐藏。另已知偏差：内嵌(blockquote/list)表格不 live 渲染；视口外大文档表格迟渲染（自愈）。逐项可拆。 |

...
|---|---|---|
| **㊵ 图谱设置完整化** | **部分**（**v1 显示+力面板 R78 完成 ✅**；过滤/分组着色/深度>2 续）| ~~**显示+力 持久化设置面板**~~（**R78 v0.75**：齿轮 → 设置浮层，4 力滑块（中心/斥力/链接力/链接距离，live-poke 运行中 d3 sim + alpha restart）+ 4 显示（节点大小/连线粗细/文本淡出阈值/箭头开关）→ 新 `features/graph/graphPrefs.ts`（GraphPrefs 扩 forces/display + parseGraphPrefs clamp/向后兼容旧 blob）+ GraphView 接线 + localStorage 持久化。纯前端、不写 .md、r26-bytes 0。对抗评审 7 lens 0 确认 + freeze DEFAULT_PREFS。16 e2e + 6 probe。**Gate**：grep 硬编码点 + WebSearch Obsidian 力/显示组）。**续缺口**：**过滤**（标签/附件作节点、孤立笔记、仅现有文件——动 `metadata.getGraph()`）· **分组着色**（按查询）· **局部图谱**深度>2 + in/out 方向 + 保存为默认。 |
...
| **㊼ Properties 增强** | **部分**（细化 ㉟；~~cssclasses 应用 R73 ✓~~）| ~~`cssclasses` 属性**应用** CSS 类到笔记视图容器~~（**R73 完成**，见上 v0.70 行）+ File properties 右侧栏（Hidden 模式下编辑入口，`RightPanelKind` 无 fileproperties）+ 键盘导航（Tab 跨行 / ↑↓ 切属性 / Cmd+Backspace 删属性）+ tags chip 点击搜索（`PropertiesPanel.tsx:265` 无 onClick）+ Date 值链接到对应日记 + hover/embeds 容器也应用 cssclasses（R73 限主 EditorPane 两模式）。 |

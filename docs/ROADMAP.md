...
| **D9** | ~~`loadMermaid` 再导出~~ + `renderMath`/`finishRenderMath` + `sanitizeHTMLToDom` | 中 | ~~loadMermaid~~ ✅ **R168 完成**（`index.ts` barrel `export {loadMermaid} from "@core/mermaid"`，类型更精确子类型、live 验 `.render` 是 function）。**剩**：renderMath/finishRenderMath（loadKatex 适配 + renderToString，零新依赖）；sanitizeHTMLToDom（安全敏感、自写保守白名单、建议单拆）。**图表/数学/web-clipper** 类插件 |
...
> **第八梯队取用顺序建议**：~~D1 版本号 / D2 parseLinktext / D9 loadMermaid / D10 base64~~（**R168 打包 done**）+ ~~D4 AbstractInputSuggest~~（R167）→ ~~D7 sleep/nextFrame/Document.on/off~~（**R169 done**）→ **下一批零依赖速赢**：**D9 renderMath/finishRenderMath**（loadKatex 适配 + renderToString，零新依赖；核 Obsidian 同步 vs Geode async 取舍）/ **D3 prepareFuzzySearch/prepareSimpleSearch**（ui.ts:483 私有 fuzzyMatch 上浮，纯函数）/ **D9 sanitizeHTMLToDom**（安全敏感、自写保守白名单、单拆）；再上 **D5/D6/D8** 等中等一轮项。**D5 getResourcePath 接 Tauri asset-protocol、D14 registerExtensions/protocol 派发** 涉宿主接线、单独成轮。**parseYaml（高价值）须用户拍板新依赖后再做**。全部为加性 display/读取面，无 vault 写、不触发 data-safety 字节级套件（除非实现触及 markdown.ts/写 .md）。

...
> **取序建议**：~~R229 视图模式切换~~ done · ~~R230 设为附件文件夹~~ done · ~~R231 快切 3 档~~ done · ~~R232 智能列表~~ done · ~~R233 始终聚焦新标签页~~ done（`focusNewTab` Store gate openFile + focus opt·default ON 零回归）；**R234 取 笔记重组 link/embed 下拉（05:11·editor 写·须 WebFetch 校 Obsidian 实际选项）或 笔记重组 合并提示+模板位置（05:11·merge data-safety 邻接谨慎）**；笔记重组两项可合一轮「笔记重组设置 Tab」；view-header 聚合菜单[~8 项偏大]/默认打开文件下拉/删除确认价值低靠后；F2 残片仅其它枯竭才取。**E1-E11 差异化系列仍卡设计空白（须先消解 `geode-设计讨论/99-待设计清单` 或拍板）·不在此 backlog。**

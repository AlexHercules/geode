...
| 功能 | 当前状态（已核实）| 范围与切入点提示 |
|---|---|---|
| **⑱ ~~Live preview 表格~~ / ~~跨行 `$$`~~·`%%` / ~~mermaid widget~~ / ~~Setext 标题~~** | **基本完成**（**Setext 折叠 R54** / **Live 表格 R55**[首个 cursor-aware block widget] / **Live mermaid R56**[抽共享 `liveBlockWidget`] / **跨行 `$$` 数学 R57 已完成（v0.57，见上）**：`liveMath.ts` 行扫描[镜像 markdown.ts 块规则]+renderMarkdownToHtml self-check[渲染器为权威]+复用 `HydratedBlockWidget`[R56 mermaid 同款，本轮抽 `liveHydratedWidget.ts`]+hydrateEmbeds KaTeX；r57-e2e 19+probe 9，评审 1 minor[D1 缩进守卫]修。**仅剩 `%%` 注释隐藏**）| **`%%` 注释隐藏**可复用 `liveBlockWidget`（block `%%…%%`）+ inline mark-hide（cursor reveal）= #⑱ 收尾。已知偏差：嵌套(blockquote/list/缩进 1-3)表格/mermaid/math 不 live 渲染（降级源码）；theme 切换 widget 不即时重渲染；视口外大文档迟渲染（自愈）。 |

...
> **取序建议**：~~R229 视图模式切换~~ done · ~~R230 设为附件文件夹~~ done · ~~R231 快切 3 档~~ done；**R232 取剩余最清爽 = 智能列表 toggle（01:38·镜像 R225 markdownKeymap→compartment·碰 editor 逻辑档）或 始终聚焦新标签页（01:12·碰 core openFile）**；笔记重组两项可合一轮「笔记重组设置 Tab」；删除确认/默认打开文件价值低靠后；F2 残片仅其它枯竭才取。**E1-E11 差异化系列仍卡设计空白（须先消解 `geode-设计讨论/99-待设计清单` 或拍板）·不在此 backlog。**

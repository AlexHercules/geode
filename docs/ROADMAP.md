...
| **G4** ⏳partial（R179=G4-a） | 文件树 / 编辑器右键菜单补齐 | **G4-a DONE（R179, v0.176）**：✅文件右键菜单 +「复制库内路径」（含扩展名）+「复制 Obsidian 链接」（`buildOpenUri` 去 .md、回环 parseObsidianUri+resolveLink），复用 R46 obsidianUri + R77 剪贴板 + 既有 toast，图标 copy/link 已注册。r179-e2e 10/10 + r93/r140/r97 回归全绿。**G4-b 余项**：文件夹右键同两项 / 列表中显示当前文件 / 打开历史入口 / 菜单分组·禁用态·宽度·阴影·浅色样式；**依赖宿主（→F1+P2）**：新窗口打开（pop-out）/ 默认应用打开（Tauri opener）/ 访达显示（系统 reveal）——宿主缺失项应显禁用。对照 `reference/08-右键菜单`（注：`reference/` 当前不在仓库） | 「默认应用打开」「访达显示」「新窗口」依赖 Electron shell / 多窗口 → 见 F1 + P2；其余纯前端 |
...
> **取用纪律 + 🧭 方向（✅ 2026-06-23 用户已拍板「完整复刻」）**：**表面复刻优先**，取用顺序 ~~**G1（设置三段式 IA · R177 done）**~~ → ~~**G2-a（设置视觉校准首片 · R178 done）**~~ → ~~**G4-a（文件右键 复制路径/Obsidian链接 · R179 done）**~~ → **下一项 = G3 命令矩阵（先建 Obsidian cmd→Geode cmd→status 矩阵，须先有 `reference/04-热键命令` 或自建权威源；R179 实查证命令面远比「90 vs 280」富，编辑格式/标签导航等早已存在，故 280 推进前必先矩阵防重复劳动）** 或 **G4-b（文件夹右键同项 + 列表显示当前文件 + 打开历史；宿主依赖项 F1）** 或 **G2-b（字体管理弹窗 + 控件原生观感）**；原「下一个 loop 落 E4/E9」**顺延至表面复刻主线推进后**。**⚠️ G3 阻塞点**：280 命令矩阵的权威源 `reference/04-热键命令` 当前不在仓库——须先补 reference 资产或据 Obsidian 实测自建命令清单。**G2-a 已交付**（弹窗 760→900/左栏 170→200/段标题分隔线/主题 segmented→下拉/修浅色整窗蓝 focus ring，颜色走 CSS 变量）；**G2-b 余项**＝字体「管理」弹窗（需系统字体枚举=功能）+ 控件原生阴影尺寸（无 `reference/` 截图故低置信、按需）。**⚠️ `reference/` 目录当前不在仓库**——G 系列的「逐像素验收」暂据审计文字 + Obsidian 实测，若要真·像素级需先补 reference 截图资产（可向用户索取或自截 Obsidian）。**G2/G3 换设置控件型必同步扫引用旧 testid 的 UI-驱动套件**（R177 立纪律、R178 复验）。依据：审计判定表面忠实度 = 最大迁移感知缺口。G1–G5（P0）纯前端、bounded、ROI 最高；G6–G10（P1）多含 data-safety 重轮或须定策略；P2/兼容层依既有梯队节奏。

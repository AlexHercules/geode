...
> **第八梯队取用顺序建议**：~~D1 版本号 / D2 parseLinktext / D9 loadMermaid / D10 base64~~（**R168 打包 done**）+ ~~D4 AbstractInputSuggest~~（R167）→ **下一批零依赖速赢**：**D7 sleep/nextFrame/Document.on/off**（crash-safety 高价值）/ **D9 renderMath/finishRenderMath**（KaTeX 适配）/ **D3 prepareFuzzySearch/prepareSimpleSearch**（ui.ts:483 私有 fuzzyMatch 上浮）/ **D9 sanitizeHTMLToDom**（安全敏感、单拆）；再上 **D5/D6** 等中等一轮项。**D5 getResourcePath 接 Tauri asset-protocol、D14 registerExtensions/protocol 派发** 涉宿主接线、单独成轮。**parseYaml（高价值）须用户拍板新依赖后再做**。全部为加性 display/读取面，无 vault 写、不触发 data-safety 字节级套件（除非实现触及 markdown.ts/写 .md）。

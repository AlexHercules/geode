...
| 核心插件 | `partial` | 多数中小能力存在 | R294 契约+目录补齐 29 行+R295 wave-1(Backlinks/Outgoing/Outline/Graph+Tags=9/29 真实启停·singleton placeholder 行为冻结)；剩 20 行 always-on 待 R296–R297 接入；Canvas/Bases/Audio/Converter 缺 | R296–R297 / R305+ |
...
- 9 行配置真实 `pluginId`：`random-note` / `daily-note` / `unique-note` / `word-count` / `tags`（R294）/ `outline` / `outgoing-links` / `backlinks` / `graph`（R295 wave-1）；其余 20 行为 always-on / not-yet-pluginified（disabled toggle，R296–R297 按 R294 冻结契约逐项接入）。
...
| 状态 | 项目 |
|---|---|
| `done`（可真实启停） | Random note、Daily notes、Unique notes、Word count、Tags（R294）、Outline、Outgoing links、Backlinks、Graph（R295 wave-1） |
| `partial`（能力在但非真插件，已入总表待 R296–R297 接入） | Note composer、Workspaces、Slides、Quick switcher、Command palette、Templates、File recovery、File explorer、Page preview、Search、Bookmarks、Properties view、Footnotes view |

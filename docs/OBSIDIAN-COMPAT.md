...
### R218 套件回归（2026-06-25，G3 §6 · 表面复刻 G 系列 · file-explorer:reveal-in-system + open-in-default-app[命令面板＋文件右键] · 薄 Rust 命令复用 safe_join → tauri-plugin-opener · 硬边界#5 用户授权解锁[仅此插件] · 逻辑档[非数据安全·只读 OS 动作] · 简化门 clean · reviewer deliverable·0 confirmed · 桌面 probe 5/5）

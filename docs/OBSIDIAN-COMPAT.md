...
**套件矩阵（R272·不回退）**：
- **r272-e2e 14/14 全绿**（新增）= 2 下拉型（view[edit/read]+editmode[live/source]）+ 旧 settings-newtab-{reading,live,source} segmented testid 消 · **read 保留 editMode + 回 edit 恢复 source · read 时改 editmode 不离开 read · consumer workspace.openFile 用 combined 开 source tab**。
- **修**：`appearance.ts`（+EditMode/defaultEditMode store/setter·setDefaultNewTabMode 加 editMode sync）· `SettingsModal.tsx`（2 `.settings-select`）· `dict.views.ts` +4 键×2 语言。
- **改控件型同步更新 tracked 套件**：r88 13（view selectOption）· r177 53 · r247 11（testid 列表）· **r195 11**（cycle 命令 setDefaultNewTabMode 同步 defaultEditMode·不退）· **r271 11**（R271 的 segmented-remains 断言翻为 segmented-gone+view-dropdown·原 10→11）· typecheck 0 · prod build✓。

### R271 套件回归（2026-06-28，设置控件原生化 segmented → Obsidian 真实控件型 = 表面复刻续第 3 项·机械档·零新依赖 · 跳简化门 + scoped review 5 窄域 0 confirmed · 桌面 by-equivalence · **R272 起 r271-e2e 10→11**[segmented-remains 断言翻 segmented-gone]）
...
**套件矩阵不回退**：`r290-e2e 5/5` · `r237-e2e 25/25` · `r160 26/26` · `r280 13/13` · `r288 17/17` · `r50 16/16` · `r24 12/12`。`npm run typecheck` 0 错误；`npm run build`✓；`PATH="$HOME/.cargo/bin:$PATH" cargo check --manifest-path src-tauri/Cargo.toml` 通过；Tauri release build 成功。桌面按 by-equivalence + release 二进制 smoke run。

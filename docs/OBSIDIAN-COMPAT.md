...
| ~~`MarkdownView.getMode/getViewData`~~ / `setViewData/setMode` + ~~`workspace.activeEditor`~~ | **部分**：~~getMode/getViewData~~（**R116 出队**：getMode hardcode "source"、getViewData=editor.getValue() live 源文本）+ ~~activeEditor~~（**R117 出队**：live getter→makeActiveMarkdownView，compat MarkdownView 即 MarkdownFileInfo 超集 editor/file/app）。`setViewData`/`setMode`（写=data-safety）+ `MarkdownFileInfo.hoverPopover` 仍缺。**已知偏离**：阅读视图非 CM-backed → reading mode `getActiveViewOfType(MarkdownView)`/`activeLeaf.view`/`activeEditor` 返 null（Obsidian 返 view+getMode "preview"） |
